 ...
 *   Por tanto offsetof(ProcessVM, registers) = VESTA_PROC_REGISTERS_OFFSET
 * - 32. Slot 16 ("rsp") -> registers + 0  = VESTA_PROC_REGISTERS_OFFSET - 32.
 *   Slot 17 ("rbp") -> registers + 8  = VESTA_PROC_REGISTERS_OFFSET - 24. */
inline int32_t vm_reg_offset(int slot) noexcept {
    if (slot == 16)
        return static_cast<int32_t>(VESTA_PROC_REGISTERS_OFFSET) - 32;
    if (slot == 17)
        return static_cast<int32_t>(VESTA_PROC_REGISTERS_OFFSET) - 24;
...
void load_op(MFunction &mf, ir::IrValueId vid, MReg dst) {
    mf.blocks.back().instrs.push_back(
        MInstr::make_unary(MOp::MOV, MOperand::make_reg(dst), slot_mem(vid)));
}
...
void load_op_rematerializable(MFunction &mf, const ir::IrFunction &fn,
    ...
    if (vid < fn.values.size()) {
        ...
        if (val.is_const) {
            ...
            mf.blocks.back().instrs.push_back(
                MInstr::make_unary(MOp::MOV, MOperand::make_reg(dst),
...
void store_op(MFunction &mf, ir::IrValueId vid, MReg src) {
    ...
    mf.blocks.back().instrs.push_back(
        MInstr::make_unary(MOp::MOV, slot_mem(vid), MOperand::make_reg(src)));
}
...
inline bool cb_is_leaf_safe_op(ir::IrOp op) noexcept {
    ...
    case ir::IrOp::ISNULL:
    case ir::IrOp::GEP:
    case ir::IrOp::GETPROC: return true;
...
MFunction Selector::select(const ir::IrFunction &ir_fn, bool *out_unsupported) {
    ...
    const uint64_t ic_fn_pc = (opts_.resolve_symbol && !ir_fn.name.empty())
    ...
    for (const auto &blk : ir_fn.blocks) {
        for (const auto &ins : blk.instrs) {
            if (ins.dst == ir::IR_NO_VALUE || ins.dst >= host_in_jit.size())
                continue;
            switch (ins.op) {
            ...
            case ir::IrOp::NEWOBJ:
            case ir::IrOp::RAW_ALLOC: host_in_jit[ins.dst] = 1u; break;
            ...
            for (const auto &ins : blk.instrs) {
                ...
                case ir::IrOp::TRUNC:
                    ...
                    if (ins.dst < skip_raw_free_vid.size() &&
                        !skip_raw_free_vid[ins.dst] && any_op_skip_free()) {
                        skip_raw_free_vid[ins.dst] = 1u;
                        if (ins.dst < alloca_size_by_vid.size() &&
                            alloca_size_by_vid[ins.dst] == 0) {
                            alloca_size_by_vid[ins.dst] = inherit_alloca_size();
                        }
                        changed = true;
                    }
                    break;
    ...
    for (const auto &blk : ir_fn.blocks) {
        for (const auto &ins : blk.instrs) {
            for (ir::IrValueId v : ins.operands) {
                if (v != ir::IR_NO_VALUE && v < use_count.size())
                    use_count[v]++;
            }
            for (const auto &pa : ins.phi_args) {
    ...
    auto warn_unsupported = [&](ir::IrOp op, uint32_t line,
                                const char *detail) {
    ...
    const bool cb_entry =
        opts_.callback_entry && opts_.mode == SelectorMode::VM_ABI;
    /* argc del callback = numero de params (cap 12 = calling convention VM). */
    const uint32_t cb_argc =
        cb_entry ? static_cast<uint32_t>(
                       ir_fn.params.size() > 12 ? 12 : ir_fn.params.size())
                 : 0u;
...
#endif
    const uint32_t cb_n_reg_args =
        cb_entry ? (cb_argc < CB_N_REG_ARGS ? cb_argc : CB_N_REG_ARGS) : 0u;
    ...
    const uint32_t cb_spill_bytes =
        (cb_entry && cb_use_call) ? ((cb_n_reg_args * 8u + 15u) & ~15u) : 0u;
    ...
    auto cb_spill_off = [&](uint32_t i) -> int32_t {
        return -(cb_work_base + static_cast<int32_t>(cb_save_bytes) + 8 +
...
#endif
    uint32_t frame_size = slot_bytes + VM_STACK_SLOTS_BYTES + cb_work_bytes +
                          ALIGN_PAD + SHADOW_SPACE;
    ...
    auto mark_slot_as_gc_if_needed = [&](ir::IrValueId vid,
                                         ir::IrType /*type*/) {
        if (vid == ir::IR_NO_VALUE) return;
        ...
        s.gc_kind = ir_fn.values[vid].is_host_ptr ? StackmapGcKind::HOSTPTR
    ...
    mf.blocks[prologue].instrs.push_back(
        MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RBP),
                           MOperand::make_reg(MReg::RSP)));
    if (frame_size > 0) {
        mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
            MOp::SUB, MOperand::make_reg(MReg::RSP),
     ...
     *     mov rax, &handler
     *     call rax                       ; c2_tier_up(proc, fn_pc):
     * recompila+swap tier_skip:
     *
    ...
    if (tier_counter_addr != 0) {
        ...
        /* tier_skip: */
        mf.blocks[prologue].instrs.push_back(MInstr::make_label_def(tier_skip));
    }
    ...
    if (cb_entry) {
        ...
        if (cb_use_call) {
            for (uint32_t i = 0; i < cb_n_reg_args; ++i) {
                mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RBP, cb_spill_off(i)),
        ...
        mf.blocks[prologue].instrs.push_back(MInstr::make_load_proc(
    ...
    if (fn_has_alloca && opts_.mode == SelectorMode::VM_ABI) {
        mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
            MOp::MOV, MOperand::make_reg(SCRATCH_A),
    ...
    if (!hoisted_order.empty() && opts_.mode == SelectorMode::VM_ABI) {
        ...
        for (const auto &kv : hoisted_order) {
            ...
            const int32_t off =
                static_cast<int32_t>(hoisted_total - accum_runup);
         ...
         * + add fixed_offset. */
        mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
            MOp::SUB, MOperand::make_reg(SCRATCH_A),
        ...
        mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
            ...
            MOperand::make_reg(SCRATCH_A)));
        mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
            MOp::MOV, MOperand::make_mem(MReg::RBP, hoisted_base_off),
            MOperand::make_reg(SCRATCH_A)));
    }
    ...
    std::unordered_map<ir::IrValueId, int32_t>
        hoisted_rbp_offset; /* unused, kept for ABI */
    ...
    if (opts_.mode == SelectorMode::NATIVE_ABI) {
...
#endif
        const int32_t N_cs =
            static_cast<int32_t>(regalloc.callee_saved_used.size());
        ...
        if (cb_save_all) {
            for (uint32_t r = 0; r < 16; ++r) {
                ...
                mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RBP, cb_save_off(r)),
                    MOperand::make_reg(SCRATCH_A)));
            }
        }
        ...
        for (uint32_t i = 0; i < cb_argc; ++i) {
            ...
            if (cb_save_all) {
                ...
                mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RBX, reg_off),
                    MOperand::make_reg(SCRATCH_A)));
            }
        ...
        if (cb_save_all) {
            ...
            mf.blocks[prologue].instrs.push_back(MInstr::make_unary(
                MOp::MOV, MOperand::make_mem(MReg::RBX, r15_off),
                MOperand::make_reg(SCRATCH_A)));
        }
    } else {
    ...
    for (size_t bi = 0; bi < ir_fn.blocks.size(); ++bi) {
        ...
        if (opts_.jit_instr_counter_addr != 0 && !ir_block.instrs.empty()) {
            const uint32_t cnt_idx =
                mf.intern_imm64(opts_.jit_instr_counter_addr);
            mf.blocks[mb].instrs.push_back(
                MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RAX),
                                   MOperand::make_imm64_idx(cnt_idx)));
            const int32_t n_instrs = static_cast<int32_t>(std::min(
                ir_block.instrs.size(), static_cast<size_t>(INT32_MAX)));
            mf.blocks[mb].instrs.push_back(
                MInstr::make_unary(MOp::ADD, MOperand::make_mem(MReg::RAX, 0),
        ...
        for (size_t ins_idx = 0; ins_idx < ir_block.instrs.size(); ++ins_idx) {
            ...
            case IrOp::STR_LIT_ADDR: {
                ...
                if (addr == 0) {
                    if (jit::g_jit_warn_unsupported) {
                        auto key = std::make_pair(static_cast<int>(ins.op),
                                                  ins.source_line);
                        if (warned_ops.insert(key).second) {
                            std::fprintf(stderr,
                                         "[jit] selector: STR_LIT_ADDR sin "
                                         "symbol '%s' en fn '%s' linea %u\n",
                                         sym.c_str(), ir_fn.name.c_str(),
                                         ins.source_line);
                        }
                    }
                    unsupported = true;
            ...
            case IrOp::LABEL_ADDR: {
                ...
                if (ins.func_name.empty()) {
                    if (jit::g_jit_warn_unsupported) {
                        std::fprintf(stderr,
                                     "[jit] selector: LABEL_ADDR sin func_name "
                                     "en fn '%s' linea %u\n",
            ...
            case IrOp::GETPROC: {
                ...
                if (opts_.mode != SelectorMode::VM_ABI) {
                    if (jit::g_jit_warn_unsupported) {
                        std::fprintf(stderr,
                                     "[jit] selector: GETPROC requiere VM_ABI "
                                     "en fn '%s' linea %u\n",
                                     ir_fn.name.c_str(), ins.source_line);
                    }
                    unsupported = true;
            ...
            case IrOp::CONST: {
                ...
                if (cv >= -0x80000000LL && cv <= 0x7FFFFFFFLL) {
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(SCRATCH_A),
                        MOperand::make_imm32(static_cast<int32_t>(ins.imm))));
                } else {
                    /* imm64 via pool. */
            ...
            case IrOp::XOR: {
                ...
                const bool op1_const =
                    op1 < ir_fn.values.size() && ir_fn.values[op1].is_const;
                bool used_imm_fold = false;
                if (op1_const) {
                    const int64_t cv =
                        static_cast<int64_t>(ir_fn.values[op1].const_val);
                    if (cv >= -0x80000000LL && cv <= 0x7FFFFFFFLL) {
                        load_op_rematerializable(mf, ir_fn, op0, SCRATCH_A);
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            m_op, MOperand::make_reg(SCRATCH_A),
                            MOperand::make_imm32(static_cast<int32_t>(cv))));
                        store_op(mf, ins.dst, SCRATCH_A);
                        used_imm_fold = true;
                    }
                ...
                if (!used_imm_fold &&
                    ...
                    const bool op0_const =
                        op0 < ir_fn.values.size() && ir_fn.values[op0].is_const;
                    if (op0_const) {
                        const int64_t cv =
                            static_cast<int64_t>(ir_fn.values[op0].const_val);
                        if (cv >= -0x80000000LL && cv <= 0x7FFFFFFFLL) {
                ...
                if (!used_imm_fold) {
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_unary(m_op, MOperand::make_reg(SCRATCH_A),
            ...
            case IrOp::MUL: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::IMUL, MOperand::make_reg(SCRATCH_A),
                                       MOperand::make_reg(SCRATCH_B)));
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            ...
            case IrOp::NEG: {
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_A);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::NEG, MOperand::make_reg(SCRATCH_A), {}));
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            case IrOp::NOT: {
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_A);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::NOT, MOperand::make_reg(SCRATCH_A), {}));
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            ...
            case IrOp::SAR: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(mop, MOperand::make_reg(SCRATCH_A),
            ...
            case IrOp::MOD: {
                ...
                if (!sr_off && ir_type_size_bytes(ins.type) == 4 &&
                    ...
                    if (ad >= 2 && !is_pow2) {
                        ...
                        load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                                 MReg::RAX);
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SHL, MOperand::make_reg(MReg::RAX),
                            MOperand::make_imm32(32)));
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SAR, MOperand::make_reg(MReg::RAX),
                            MOperand::make_imm32(32)));
                        /* RCX = n (para correccion + el mod). */
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_reg(MReg::RCX),
                        ...
                        if (d > 0 && mg.M < 0) {
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::ADD, MOperand::make_reg(MReg::RAX),
                                    MOperand::make_reg(MReg::RCX)));
                        } else if (d < 0 && mg.M > 0) {
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::SUB, MOperand::make_reg(MReg::RAX),
                                    MOperand::make_reg(MReg::RCX)));
                        }
                        /* q >>= s (arithmetic). */
                        if (mg.s > 0) {
                        ...
                        if (ins.op == IrOp::DIV) {
                            ...
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::SUB, MOperand::make_reg(MReg::RCX),
                                    MOperand::make_reg(MReg::RAX)));
                ...
                /* x86 IDIV usa RDX:RAX dividendo / src -> RAX cociente, RDX
                 * resto. Setup: load_op(arg0, RAX) load_op(arg1, RCX) CQO
                 * (sign-extend RAX -> RDX:RAX) IDIV RCX store_op(dst, RAX si
                 * DIV o RDX si MOD)
                 *
                ...
                /* IDIV RCX */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::IDIV, {}, MOperand::make_reg(MReg::RCX)));
            ...
            case IrOp::ALLOCA: {
                if (opts_.mode != SelectorMode::VM_ABI) {
                    ...
                    if (size_bytes > 0 && size_bytes < INT32_MAX) {
                        const uint64_t aligned = (size_bytes + 15ULL) & ~15ULL;
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SUB, MOperand::make_reg(MReg::RSP),
                            MOperand::make_imm32(
                ...
                if (hv_it != hoisted_vm_offset.end()) {
                    ...
                    if (off_in_block != 0) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::ADD, MOperand::make_reg(SCRATCH_A),
            ...
            case IrOp::BITCAST: {
                ...
                if (dst_bytes < src_bytes) {
                    ...
                    if (dst_bits < 64) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SHL, MOperand::make_reg(SCRATCH_A),
            ...
            case IrOp::LOAD: {
                ...
                if (!ptr_is_host && opts_.mode == SelectorMode::VM_ABI &&
                    ...
                    switch (lbytes) {
                    case 1:
                        fn_addr = (uint64_t)opts_.runtime->vm_read_u8;
                        break;
                    case 2:
                        fn_addr = (uint64_t)opts_.runtime->vm_read_u16;
                        break;
                    case 4:
                        fn_addr = (uint64_t)opts_.runtime->vm_read_u32;
                        break;
                    default:
                        fn_addr = (uint64_t)opts_.runtime->vm_read_u64;
                        break;
                    }
                    if (fn_addr == 0) {
                        if (jit::g_jit_warn_unsupported) {
                            std::fprintf(
                                stderr,
                                "[jit] selector: LOAD VM-ptr sin runtime entry "
                                "para size %llu en fn '%s' linea %u\n",
                                (unsigned long long)lbytes, ir_fn.name.c_str(),
                                ins.source_line);
                        }
                        if (out_unsupported) *out_unsupported = true;
                        return mf;
                    ...
                    if (inline_cache_ok) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_reg(SCRATCH_C),
                            MOperand::make_reg(SCRATCH_B)));
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::AND, MOperand::make_reg(SCRATCH_A),
                            MOperand::make_imm32(-4096)));
                        /* cmp rax, [rbx + page_v_disp] */
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::CMP, MOperand::make_reg(SCRATCH_A),
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::AND, MOperand::make_reg(SCRATCH_C),
                        ...
                        if (lbytes > 1) {
                            ...
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::CMP, MOperand::make_reg(SCRATCH_C),
                        ...
                        if (lbytes == 0 || lbytes >= 8) {
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::MOV, MOperand::make_reg(SCRATCH_A, 8),
                            ...
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::MOVSX,
                                    MOperand::make_reg(SCRATCH_A, 8), mem_op));
                        } else if (lbytes == 4) {
                            mf.blocks.back().instrs.push_back(
                                MInstr::make_unary(
                                    MOp::MOV, MOperand::make_reg(SCRATCH_A, 4),
                                    MOperand::make_mem(SCRATCH_A, 0)));
                        } else {
                            MOperand mem_op = MOperand::make_mem(SCRATCH_A, 0);
                            mem_op.flags = static_cast<uint8_t>(lbytes);
                        ...
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_jmp(done_label));
                        /* miss: */
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_label_def(miss_label));
                    }
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(NATIVE_ARG0),
                    ...
                    if (lbytes < 8 && lsigned) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOVSX, MOperand::make_reg(SCRATCH_A, 8),
                            src_reg));
                    }
                    if (inline_cache_ok) {
                        /* done: */
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_label_def(done_label));
                    }
                    store_op(mf, ins.dst, SCRATCH_A);
                    break;
                }
                ...
                if (lbytes == 0 || lbytes >= 8) {
                    ...
                    mem_op.flags = static_cast<uint8_t>(lbytes);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOVSX, MOperand::make_reg(SCRATCH_A, 8), mem_op));
                } else if (lbytes == 4) {
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(SCRATCH_A, 4),
                        MOperand::make_mem(SCRATCH_B, 0)));
                    ...
                    mem_op.flags = static_cast<uint8_t>(lbytes);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOVZX, MOperand::make_reg(SCRATCH_A, 8), mem_op));
                }
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            case IrOp::STORE: {
                ...
                if (!ptr_is_host && opts_.mode == SelectorMode::VM_ABI &&
                    ...
                    switch (sbytes) {
                    case 1:
                        fn_addr = (uint64_t)opts_.runtime->vm_write_u8;
                        break;
                    case 2:
                        fn_addr = (uint64_t)opts_.runtime->vm_write_u16;
                        break;
                    case 4:
                        fn_addr = (uint64_t)opts_.runtime->vm_write_u32;
                        break;
                    default:
                        fn_addr = (uint64_t)opts_.runtime->vm_write_u64;
                        break;
                    }
                    if (fn_addr == 0) {
                        if (jit::g_jit_warn_unsupported) {
                            std::fprintf(
                                stderr,
                                "[jit] selector: STORE VM-ptr sin runtime "
                                "entry para size %llu en fn '%s' linea %u\n",
                                (unsigned long long)sbytes, ir_fn.name.c_str(),
                                ins.source_line);
                        }
                        if (out_unsupported) *out_unsupported = true;
                        return mf;
                    ...
                    if (inline_cache_ok) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_reg(SCRATCH_B),
                            MOperand::make_mem(JIT_PROC_REG, page_h_disp)));
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::ADD, MOperand::make_reg(SCRATCH_B),
                            MOperand::make_reg(SCRATCH_C)));
                        ...
                        const uint8_t hw = (sbytes == 1 || sbytes == 2 ||
                                            sbytes == 4 || sbytes == 8)
                                               ? static_cast<uint8_t>(sbytes)
                                               : 8;
                        MOperand src_h = MOperand::make_reg(SCRATCH_A, hw);
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_mem(SCRATCH_B, 0), src_h));
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_jmp(done_label));
                        /* miss: */
                        mf.blocks.back().instrs.push_back(
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[1], SCRATCH_B);
                const uint8_t w =
                    (sbytes == 1 || sbytes == 2 || sbytes == 4 || sbytes == 8)
                        ? static_cast<uint8_t>(sbytes)
                        : 8;
                MOperand src_op = MOperand::make_reg(SCRATCH_A, w);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(SCRATCH_B, 0), src_op));
                break;
            }
            ...
            case IrOp::CMP_UGE: {
                 ...
                 * loops aritmeticos. */
                bool fuse = false;
                if (ins.dst != ir::IR_NO_VALUE && ins.dst < use_count.size() &&
                    use_count[ins.dst] == 1 &&
                    ins_idx + 1 < ir_block.instrs.size()) {
                    const auto &nxt = ir_block.instrs[ins_idx + 1];
                    if (nxt.op == IrOp::BR_COND && !nxt.operands.empty() &&
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::XOR, MOperand::make_reg(SCRATCH_A),
                                       MOperand::make_reg(SCRATCH_A)));
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::CMP, MOperand::make_reg(SCRATCH_B),
                                       MOperand::make_reg(SCRATCH_C)));
            ...
            case IrOp::CALL: {
                ...
                auto emit_tailcall_epilogue = [&]() {
                    ...
                    if (opts_.mode == SelectorMode::VM_ABI) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_reg(MReg::RAX),
                            MOperand::make_mem(MReg::RBX,
                                               VESTA_PROC_REGISTERS_OFFSET)));
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RSP),
                        MOperand::make_reg(MReg::RBP)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::POP, MOperand::make_reg(MReg::RBP), {}));
                    for (auto it = regalloc.callee_saved_used.rbegin();
                         it != regalloc.callee_saved_used.rend(); ++it) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::POP, MOperand::make_reg(*it), {}));
                    }
                    if (opts_.mode == SelectorMode::VM_ABI) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::POP, MOperand::make_reg(MReg::RBX), {}));
                    }
                ...
                if (fn_addr == 0 && opts_.mode == SelectorMode::VM_ABI &&
                    !ins.func_name.empty() && ins.func_name == ir_fn.name) {
                    is_self_ref = true;
                    is_user_call = true;
                ...
                if (fn_addr == 0 &&
                    !is_self_ref /* Sprint fib-recursion: self-call no usa
                                    trampoline */
                    && opts_.runtime != nullptr &&
                ...
                if (fn_addr == 0 && !is_self_ref) {
                    ...
                    if (jit::g_jit_warn_unsupported) {
                        ...
                        if (warned_ops.insert(key).second) {
                            std::fprintf(stderr,
                                         "[jit] selector: CALL a entry '%s' no "
                                         "resuelto en fn '%s' linea %u\n",
                                         ins.func_name.c_str(),
                                         ir_fn.name.c_str(), ins.source_line);
                        }
                    }
                ...
                if (is_bc_trampoline) {
                    ...
                    for (size_t a = 0; a < nargs && a < 12; ++a) {
                        load_op_rematerializable(mf, ir_fn, ins.operands[a],
                                                 SCRATCH_A);
                        const int32_t off =
                            regs_base + static_cast<int32_t>((a + 1) * 8);
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_mem(MReg::RBX, off),
                            MOperand::make_reg(SCRATCH_A)));
                    }
                    /* Paso 2: R15 = nargs. */
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
...
#endif
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_unary(MOp::MOV, MOperand::make_reg(n_arg0),
                                           MOperand::make_reg(MReg::RBX)));
                    const uint32_t bc_pool_idx = mf.intern_imm64(bc_entry_va);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(n_arg1),
                    ...
                    if (jit::g_jit_warn_unsupported) {
                        ...
                        if (warned_ops.insert(key).second) {
                            std::fprintf(stderr,
                                         "[jit] selector: CALL '%s' via "
                                         "trampoline JIT->interp (callee no "
                                         "JIT-able) en fn '%s' linea %u\n",
                                         ins.func_name.c_str(),
                                         ir_fn.name.c_str(), ins.source_line);
                        }
                    }
...
#endif
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_unary(MOp::MOV, MOperand::make_reg(arg0),
                                           MOperand::make_reg(MReg::RBX)));
...
#if defined(_WIN32)
                static const MReg ARG_REGS[] = {MReg::RCX, MReg::RDX, MReg::R8,
                                                MReg::R9};
                constexpr size_t MAX_REG_ARGS = 4;
#else
                static const MReg ARG_REGS[] = {MReg::RDI, MReg::RSI, MReg::RDX,
                                                MReg::RCX, MReg::R8,  MReg::R9};
                ...
                if (ins.operands.size() > MAX_REG_ARGS) {
                    ...
                    if (jit::g_jit_warn_unsupported) {
                        ...
                        if (warned_ops.insert(key).second) {
                            std::fprintf(
                                stderr,
                                "[jit] selector: CALL con %zu args (max %zu en "
                                "regs) en fn '%s' linea %u\n",
                                ins.operands.size(), MAX_REG_ARGS,
                                ir_fn.name.c_str(), ins.source_line);
                        }
                    }
                    unsupported = true;
            ...
            case IrOp::NEWOBJ: {
                ...
                if (!can_inline) {
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_unary(MOp::MOV, MOperand::make_reg(ABI0),
                                           MOperand::make_reg(MReg::RBX)));
                    const uint32_t pidx =
                        mf.intern_imm64(reinterpret_cast<uint64_t>(
                 ...
                 */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_reg(MReg::RCX, 4),
                    MOperand::make_mem(ABI1, 24)));
                /* add rcx, 15 (= 8 GcHeader + 7 padding) */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::ADD, MOperand::make_reg(MReg::RCX),
                                       MOperand::make_imm32(15)));
                /* and rcx, ~7 (align 8) */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::AND, MOperand::make_reg(MReg::RCX),
                    MOperand::make_imm32(static_cast<int32_t>(~7))));
                /* mov rax, [rbx + nursery_bump_off] */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_reg(MReg::RAX),
                    MOperand::make_mem(MReg::RBX, opts_.nursery_bump_offset)));
                /* mov r8, rax ; new_bump = bump */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::R8),
                                       MOperand::make_reg(MReg::RAX)));
                /* add r8, rcx */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_binary(MOp::ADD, MOperand::make_reg(MReg::R8),
                                        MOperand::make_reg(MReg::R8),
                                        MOperand::make_reg(MReg::RCX)));
                /* cmp r8, [rbx + nursery_end_off] */
                mf.blocks.back().instrs.push_back(MInstr::make_binary(
                    MOp::CMP, MOperand{}, MOperand::make_reg(MReg::R8),
                    MOperand::make_mem(MReg::RBX, opts_.nursery_end_offset)));
                /* ja lbl_slow */
                mf.blocks.back().instrs.push_back(
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RAX, 0),
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RAX, 8),
                    MOperand::make_reg(ABI1)));
                /* ObjectHeader.flags (offset 16) = OBJ_FLAG_GC_OWNED (=1). */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RAX, 16),
                    MOperand::make_imm32(1)));
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(ABI1),
                                       MOperand::make_reg(MReg::RAX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(ABI0),
                                       MOperand::make_reg(MReg::RBX)));
                {
                    const uint32_t pidx =
                        mf.intern_imm64(opts_.register_alloc_addr);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RAX),
                        MOperand::make_imm64_idx(pidx)));
                }
                ...
                /* jmp done */
                mf.blocks.back().instrs.push_back(MInstr::make_jmp(lbl_done));
            ...
            case IrOp::CALLN: {
                ...
                if (ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_abs" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_min" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_max" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_minu" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_maxu" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_ilog2" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_rotl" ||
                    ins.func_name ==
                        "stdlib/native/math/vesta_math:vmath_rotr") {
                    ...
                    if (fn.find("vmath_abs") != std::string::npos) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SAR, MOperand::make_reg(SCRATCH_C),
                            MOperand::make_imm32(63)));
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::XOR, MOperand::make_reg(SCRATCH_A),
                            MOperand::make_reg(SCRATCH_C)));
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::SUB, MOperand::make_reg(SCRATCH_A),
                            MOperand::make_reg(SCRATCH_C)));
                    ...
                    if (fn.find("vmath_ilog2") != std::string::npos) {
                        ...
                        load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                                 SCRATCH_B);
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::LZCNT, MOperand::make_reg(MReg::RAX),
                            MOperand::make_reg(SCRATCH_B)));
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::NEG, MOperand::make_reg(MReg::RAX),
                            MOperand::make_reg(MReg::RAX)));
                    ...
                    if (fn.find("vmath_rotl") != std::string::npos ||
                        ...
                        load_op_rematerializable(mf, ir_fn, ins.operands[1],
                                                 SCRATCH_B);
                        const MOp rot_op =
                            (fn.find("vmath_rotl") != std::string::npos)
                                ? MOp::ROL
                                : MOp::ROR;
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            rot_op, MOperand::make_reg(SCRATCH_A),
                            MOperand::make_reg(SCRATCH_B)));
                        store_op(mf, ins.dst, SCRATCH_A);
                        break;
                    }
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        ...
                        MOperand::make_reg(SCRATCH_B)));
                    MCond cc;
                    if (fn.find("vmath_minu") != std::string::npos)
                        cc = MCond::A;
                    else if (fn.find("vmath_maxu") != std::string::npos)
                        cc = MCond::B;
                    else if (fn.find("vmath_min") != std::string::npos)
                        cc = MCond::G;
                    else
                ...
                if (is_callni) {
                    ...
                    /* Native ABI arg regs. */
#if defined(_WIN32)
                    static const MReg N_ARG_REGS_NI[] = {MReg::RCX, MReg::RDX,
                                                         MReg::R8, MReg::R9};
                    constexpr size_t N_MAX_REG_ARGS_NI = 4;
#else
                    static const MReg N_ARG_REGS_NI[] = {MReg::RDI, MReg::RSI,
                                                         MReg::RDX, MReg::RCX,
                                                         MReg::R8,  MReg::R9};
                    ...
                    for (size_t a = 0; a < nargs_ni; ++a) {
                        load_op_rematerializable(mf, ir_fn, ins.operands[a + 1],
                                                 N_ARG_REGS_NI[a]);
                    }
                ...
                if (fn_addr == 0) {
                    if (jit::g_jit_warn_unsupported) {
                        ...
                        if (warned_ops.insert(key).second) {
                            std::fprintf(stderr,
                                         "[jit] selector: CALLN '%s' no se "
                                         "pudo resolver en fn '%s' linea %u\n",
                                         ins.func_name.c_str(),
                                         ir_fn.name.c_str(), ins.source_line);
                        }
                    }
                ...
                /* Native ABI arg regs. */
#if defined(_WIN32)
                static const MReg N_ARG_REGS[] = {MReg::RCX, MReg::RDX,
                                                  MReg::R8, MReg::R9};
                constexpr size_t N_MAX_REG_ARGS = 4;
#else
                static const MReg N_ARG_REGS[] = {MReg::RDI, MReg::RSI,
                                                  MReg::RDX, MReg::RCX,
                                                  MReg::R8,  MReg::R9};
                ...
                if (ins.operands.size() > N_MAX_REG_ARGS) {
                    warn_unsupported(ins.op, ins.source_line,
                                     "CALLN con demasiados args (stack args no "
                                     "implementados)");
                    unsupported = true;
                    mf.blocks.back().instrs.push_back(
                        {MOp::INT3, 0, 0, 0, {}, {}, {}});
                ...
                if (cn_need_shadow) {
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::ADD, MOperand::make_reg(MReg::RSP),
                        MOperand::make_imm32(0x20)));
                }
            ...
            case IrOp::CALLVIRT: {
                if (ins.operands.empty()) break;
                if (opts_.runtime == nullptr ||
                    opts_.runtime->callvirt == nullptr) {
                    warn_unsupported(ins.op, ins.source_line,
                 ...
                 *   2. Inline check: si method->jit_code != null -> direct
                 * call.
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_A);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_mem(MReg::RBX, regs_base + 8),
                    MOperand::make_reg(SCRATCH_A)));
                ...
                if (ic_slot_addr != 0 && opts_.runtime &&
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::R10),
                    ...
                    for (int e = 0; e < 4; ++e) {
                        ...
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::CMP, MOperand::make_reg(MReg::RCX),
                        ...
                        /* jmp hit_call */
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_jmp(hit_call_label));
                        /* skip_e: */
                        mf.blocks.back().instrs.push_back(
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jmp(ic_miss_label));
                    /* hit_call: convergencia de los 4 hits con rax = jit_code.
                     */
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_label_def(hit_call_label));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RCX),
                        MOperand::make_reg(MReg::RBX)));
                    {
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jmp(ic_continue_label));
                    /* ic_miss: */
                    mf.blocks.back().instrs.push_back(
...
#endif
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(ic_arg0),
                        MOperand::make_reg(MReg::RBX)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(ic_arg1),
                        MOperand::make_reg(MReg::RAX)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(ic_arg2),
                        MOperand::make_imm32(static_cast<int32_t>(vtbl_idx))));
                    const uint32_t slot_idx2 = mf.intern_imm64(ic_slot_addr);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(ic_arg3),
                        MOperand::make_imm64_idx(slot_idx2)));
                    const uint64_t ic_fn =
                        reinterpret_cast<uint64_t>(opts_.runtime->callvirt_ic);
                    ...
                    /* ic_continue: */
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_label_def(ic_continue_label));
                ...
                /* test rax,rax; jz fallback */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::TEST, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_reg(MReg::RAX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_jcc(MCond::E, fallback_label));
                /* mov rcx, [rax + 0]  (class_ptr) */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RCX),
                                       MOperand::make_mem(MReg::RAX, 0)));
                /* test rcx,rcx; jz fallback */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::TEST, MOperand::make_reg(MReg::RCX),
                                       MOperand::make_reg(MReg::RCX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_jcc(MCond::E, fallback_label));
                ...
                /* test rcx,rcx; jz fallback (no vtable = abstract o
                 * vtable_size=0) */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::TEST, MOperand::make_reg(MReg::RCX),
                                       MOperand::make_reg(MReg::RCX)));
                ...
                /* jmp continue */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_jmp(continue_label));

                /* fallback_label: */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(fallback_label));

                /* SLOW PATH: vrt_callvirt(proc, obj, vtbl_idx). */
                load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                         SCRATCH_A); /* obj host_ptr */
#if defined(_WIN32)
                const MReg arg0 = MReg::RCX;
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(arg1),
                                       MOperand::make_reg(SCRATCH_A)));
                /* mov arg2, vtbl_idx */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_reg(arg2),
                    MOperand::make_imm32(static_cast<int32_t>(vtbl_idx))));
                /* mov rax, vrt_callvirt; call rax */
                const uint64_t fn_addr =
                    reinterpret_cast<uint64_t>(opts_.runtime->callvirt);
                const uint32_t fn_pool_idx = mf.intern_imm64(fn_addr);
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_imm64_idx(fn_pool_idx)));
                ...
                /* continue_label: */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(continue_label));
            ...
            case IrOp::CALLM: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_A)));
                for (size_t a = 0; a < nargs; ++a) {
                    load_op_rematerializable(mf, ir_fn, ins.operands[a + 2],
                                             SCRATCH_A);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV,
                        MOperand::make_mem(
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_jmp(cm_continue));

                /* fallback: vrt_callm(proc, obj, method). */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(cm_fallback));
                /* Native ABI: arg0=proc(rbx), arg1=obj_payload,
                 * arg2=method_ptr. R10/R11 (caller-saved, no arg regs) como
                 * temporales para evitar clobber (RDX es cm_arg1 Win64 /
                 * cm_arg2 SysV). */
                load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                         MReg::R10); /* obj host_ptr */
                load_op_rematerializable(mf, ir_fn, ins.operands[1],
                                         MReg::R11); /* method ptr */
#if defined(_WIN32)
...
#endif
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cm_arg0),
                                       MOperand::make_reg(MReg::RBX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cm_arg1),
                                       MOperand::make_reg(MReg::R10)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cm_arg2),
                                       MOperand::make_reg(MReg::R11)));

                const uint64_t fn_addr =
                    reinterpret_cast<uint64_t>(opts_.runtime->callm);
                const uint32_t fn_pool_idx = mf.intern_imm64(fn_addr);
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_imm64_idx(fn_pool_idx)));
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(cm_continue));
                if (ins.dst != ir::IR_NO_VALUE) {
                    store_op(mf, ins.dst, MReg::RAX);
                }
                break;
            ...
            case IrOp::CALLITF: {
                if (ins.operands.size() < 2) break;
                if (opts_.runtime == nullptr ||
                    opts_.runtime->callitf == nullptr) {
                    warn_unsupported(ins.op, ins.source_line,
                                     "runtime->callitf null");
                    unsupported = true;
                    mf.blocks.back().instrs.push_back(
                        {MOp::INT3, 0, 0, 0, {}, {}, {}});
                    break;
                ...
                if (ic_slot_addr != 0) {
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::R11),
                        MOperand::make_imm64_idx(
                            mf.intern_imm64(ic_slot_addr))));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::R11),
                        MOperand::make_mem(MReg::R11, 0)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::R11),
                        MOperand::make_reg(MReg::R11)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_fallback));
                    /* obj -> R10; si null -> fallback (throw correcto alli). */
                    load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                             MReg::R10);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::R10),
                        MOperand::make_reg(MReg::R10)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_fallback));
                    /* cls = [obj + 0] (class_ptr) */
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RCX),
                        MOperand::make_mem(MReg::R10,
                                           VESTA_OBJ_HDR_CLASS_PTR_OFFSET)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::RCX),
                        MOperand::make_reg(MReg::RCX)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_fallback));
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::RAX),
                        MOperand::make_reg(MReg::RAX)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_fallback));
                     ...
                     */
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_label_def(ci_scan));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::R10),
                        MOperand::make_mem(MReg::RAX,
                                           VESTA_ITABLE_IFACE_OFFSET)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::R10),
                        MOperand::make_reg(MReg::R10)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_fallback));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::CMP, MOperand::make_reg(MReg::R10),
                        MOperand::make_reg(MReg::R11)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::E, ci_found));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::ADD, MOperand::make_reg(MReg::RAX),
                        MOperand::make_imm32(VESTA_ITABLE_ENTRY_SIZE)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jmp(ci_scan));
                    /* found: RAX = entry.methods; RCX = methods[idx];
                     * null -> fallback (rellena); advice_chain -> fallback
                     * (AOP); jit_code==0 -> fallback (compila); else call
                     * directo. */
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_label_def(ci_found));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RAX),
                        MOperand::make_mem(MReg::RAX,
                                           VESTA_ITABLE_METHODS_OFFSET)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RCX),
                        MOperand::make_mem(MReg::RAX,
                                           static_cast<int32_t>(method_index) *
                                               8)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::RCX),
                        MOperand::make_reg(MReg::RCX)));
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RAX),
                        MOperand::make_mem(
                            MReg::RCX, VESTA_METHODINFO_ADVICE_CHAIN_OFFSET)));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(MReg::RAX),
                        MOperand::make_reg(MReg::RAX)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::NE, ci_fallback));
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RAX),
                        MOperand::make_mem(MReg::RCX,
                                           VESTA_METHODINFO_JIT_CODE_OFFSET)));
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jmp(ci_continue));
                }

                /* FALLBACK: vrt_callitf(proc, obj_payload, params_addr,
                 * ic_slot_addr). Alcanzado por fall-through (sin inline) o jmp
                 * ci_fallback. R10/R11 temporales (caller-saved, no arg regs).
                 */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(ci_fallback));
                load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                         MReg::R10); /* obj */
                load_op_rematerializable(mf, ir_fn, ins.operands[1],
                                         MReg::R11); /* params_ptr */
...
#endif
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(ci_arg1),
                                       MOperand::make_reg(MReg::R10)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(ci_arg2),
                                       MOperand::make_reg(MReg::R11)));
                /* arg3 = ic_slot_addr (imm64, 0 si no hay slot). */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_reg(ci_arg3),
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(ci_arg0),
                                       MOperand::make_reg(MReg::RBX)));

                const uint64_t ci_fn =
                    reinterpret_cast<uint64_t>(opts_.runtime->callitf);
                ...
                /* continue: ambos paths convergen con resultado en RAX. */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_label_def(ci_continue));
                if (ins.dst != ir::IR_NO_VALUE) {
                    store_op(mf, ins.dst, MReg::RAX);
                }
                break;
            ...
            case IrOp::CALLCLOSURE: {
                if (ins.operands.empty()) break;
                if (opts_.runtime == nullptr ||
                    opts_.runtime->callclosure == nullptr) {
                    warn_unsupported(ins.op, ins.source_line,
                                     "runtime->callclosure null");
                    unsupported = true;
                    mf.blocks.back().instrs.push_back(
                        {MOp::INT3, 0, 0, 0, {}, {}, {}});
                    break;
                ...
                load_op_rematerializable(mf, ir_fn, ins.func_ptr,
                                         MReg::R10); /* fn_addr */
                load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                         MReg::R11); /* env_addr */
#if defined(_WIN32)
...
#endif
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cc_arg0),
                                       MOperand::make_reg(MReg::RBX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cc_arg1),
                                       MOperand::make_reg(MReg::R10)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(cc_arg2),
                                       MOperand::make_reg(MReg::R11)));

                const uint64_t fn_addr_cc =
                    reinterpret_cast<uint64_t>(opts_.runtime->callclosure);
            ...
            case IrOp::BR: {
                ...
                if (ins.target_block < ir_fn.blocks.size()) {
                    ...
                    for (const auto &pin : target.instrs) {
                        if (pin.op != ir::IrOp::PHI)
                            break; /* PHIs solo al inicio */
                        ...
                        for (const auto &arg : pin.phi_args) {
                            if (arg.block == static_cast<ir::IrBlockId>(bi)) {
                                load_op_rematerializable(mf, ir_fn, arg.value,
                                                         SCRATCH_A);
                ...
                if (opts_.mode == SelectorMode::VM_ABI &&
                    ...
                    safepoint_pool_idx_ != UINT32_MAX) {
                    MInstr sp_instr =
                        MInstr::make_safepoint(safepoint_pool_idx_);
            ...
            case IrOp::BR_COND: {
                ...
                const bool use_fused =
                    fused_cmp_active && fused_cmp_dst == ins.operands[0];
                ...
                if (use_fused) {
                    ...
                    load_op_rematerializable(mf, ir_fn, fused_cmp_op0,
                                             SCRATCH_B);
                    load_op_rematerializable(mf, ir_fn, fused_cmp_op1,
                                             SCRATCH_C);
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::CMP, MOperand::make_reg(SCRATCH_B),
                        MOperand::make_reg(SCRATCH_C)));
                } else {
                    load_op_rematerializable(mf, ir_fn, ins.operands[0],
                                             SCRATCH_A);
                ...
                const MCond skip_cond =
                    use_fused ? invert_cond(fused_cmp_cond) : MCond::E;
                if (!use_fused) {
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::TEST, MOperand::make_reg(SCRATCH_A),
                        MOperand::make_reg(SCRATCH_A)));
                }
                ...
                if (!t_has_phi && !f_has_phi) {
                    ...
                    if (ins.target_block < block_labels.size()) {
                        mf.blocks.back().instrs.push_back(MInstr::make_jcc(
                            taken_cond, block_labels[ins.target_block]));
                    }
            ...
            case IrOp::RET: {
                 ...
                 *           ret */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RSP),
                                       MOperand::make_reg(MReg::RBP)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::POP, MOperand::make_reg(MReg::RBP), {}));
            ...
            case ir::IrOp::FINDFIELD: {
                ...
                const uint64_t runtime_fn =
                    (ins.op == ir::IrOp::FINDMETHOD)
                        ? (opts_.runtime ? reinterpret_cast<uint64_t>(
            ...
            case ir::IrOp::CTZ: {
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_B);
                const MOp mop = (ins.op == ir::IrOp::POPCNT) ? MOp::POPCNT
                                : (ins.op == ir::IrOp::CLZ)  ? MOp::LZCNT
                                                             : MOp::TZCNT;
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(mop, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_reg(SCRATCH_B)));
                store_op(mf, ins.dst, MReg::RAX);
                break;
            }
            case ir::IrOp::BYTESWAP: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::BSWAP, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::RAX)));
                store_op(mf, ins.dst, MReg::RAX);
                break;
            }
            ...
            case ir::IrOp::IABS: {
                ...
                /* sar rdx, 63 -> rdx = -1 si neg, 0 si pos */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::SAR, MOperand::make_reg(SCRATCH_C),
                                       MOperand::make_imm32(63)));
                /* xor rax, rdx */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::XOR, MOperand::make_reg(SCRATCH_A),
                ...
                /* sub rax, rdx */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::SUB, MOperand::make_reg(SCRATCH_A),
                                       MOperand::make_reg(SCRATCH_C)));
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            ...
            case ir::IrOp::IMAXU: {
                ...
                /* cmp rax, rcx */
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::CMP, MOperand::make_reg(SCRATCH_A),
                                       MOperand::make_reg(SCRATCH_B)));
                /* cmov<cc> rax, rcx */
                const MCond cc = (ins.op == ir::IrOp::IMIN)    ? MCond::G
            ...
            case ir::IrOp::ILOG2: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::NEG, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_reg(MReg::RAX)));
                mf.blocks.back().instrs.push_back(
            ...
            case ir::IrOp::FDIV: {
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_B);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_GP_XMM, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(SCRATCH_B)));
                load_op_rematerializable(mf, ir_fn, ins.operands[1], SCRATCH_B);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_GP_XMM, MOperand::make_reg(MReg::XMM1),
                    MOperand::make_reg(SCRATCH_B)));
                const MOp mop = (ins.op == ir::IrOp::FADD)   ? MOp::ADDSD
                                : (ins.op == ir::IrOp::FSUB) ? MOp::SUBSD
                                : (ins.op == ir::IrOp::FMUL) ? MOp::MULSD
                                                             : MOp::DIVSD;
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(mop, MOperand::make_reg(MReg::XMM0),
                                       MOperand::make_reg(MReg::XMM1)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_XMM_GP, MOperand::make_reg(MReg::RAX),
            ...
            case ir::IrOp::UITOF: {
                ...
                load_op_rematerializable(mf, ir_fn, ins.operands[0], SCRATCH_B);
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::CVTSI2SD, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_XMM_GP, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::XMM0)));
            ...
            case ir::IrOp::FTOUI: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::CVTTSD2SI, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::XMM0)));
                store_op(mf, ins.dst, MReg::RAX);
                break;
            }
            ...
            case ir::IrOp::FSQRT: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::SQRTSD, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(MReg::XMM0)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_XMM_GP, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::XMM0)));
            ...
            case ir::IrOp::FMAX: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_B)));
                const MOp mop =
                    (ins.op == ir::IrOp::FMIN) ? MOp::MINSD : MOp::MAXSD;
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(mop, MOperand::make_reg(MReg::XMM0),
                                       MOperand::make_reg(MReg::XMM1)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
            ...
            case ir::IrOp::FTRUNC: {
                ...
                const uint8_t mode =
                    (ins.op == ir::IrOp::FROUND) ? 0 : /* nearest */
                        (ins.op == ir::IrOp::FFLOOR) ? 1
                                                     : /* down */
                        (ins.op == ir::IrOp::FCEIL) ? 2
                                                    : /* up */
                        3;                            /* trunc */
                MInstr round_instr;
            ...
            case ir::IrOp::FNEG: {
                 ...
                 */
                const MOp mop =
                    (ins.op == ir::IrOp::FABS) ? MOp::AND : MOp::XOR;
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(mop, MOperand::make_reg(SCRATCH_B),
                                       MOperand::make_reg(MReg::RAX)));
            ...
            case ir::IrOp::F32TOF64: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::CVTSS2SD, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(MReg::XMM0)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_XMM_GP, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::XMM0)));
            ...
            case ir::IrOp::F64TOF32: {
                ...
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    ...
                    MOperand::make_reg(SCRATCH_B)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::CVTSD2SS, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(MReg::XMM0)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOVQ_XMM_GP, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::XMM0)));
            ...
            case ir::IrOp::FCMP_GE: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::XOR, MOperand::make_reg(MReg::RAX),
                                       MOperand::make_reg(MReg::RAX)));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::XOR, MOperand::make_reg(MReg::RCX),
                                       MOperand::make_reg(MReg::RCX)));
                 ...
                 *   equal:           ZF=1, PF=0, CF=0  */
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::UCOMISD, MOperand::make_reg(MReg::XMM0),
                    MOperand::make_reg(MReg::XMM1)));
                ...
                if (is_ne) {
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::OR, MOperand::make_reg(MReg::RAX),
                        MOperand::make_reg(MReg::RCX)));
                } else {
                    ...
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::AND, MOperand::make_reg(MReg::RAX),
                        MOperand::make_reg(MReg::RCX)));
                    (void)is_eq;
                }
                store_op(mf, ins.dst, MReg::RAX);
            ...
            case ir::IrOp::ROTR: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(rot_op, MOperand::make_reg(SCRATCH_A),
                                       MOperand::make_reg(SCRATCH_B)));
                store_op(mf, ins.dst, SCRATCH_A);
                break;
            }
            ...
            case ir::IrOp::RAW_ALLOC: {
                /* RAW_ALLOC(size) -> host_ptr.  CALL a vrt_raw_alloc(proc,
                 * size). Equivale a @c malloc(size) del C runtime. El bloque
                 * vive hasta @c RAW_FREE manual. */
                if (ins.operands.size() == 1 && ins.dst != ir::IR_NO_VALUE &&
                    opts_.runtime != nullptr &&
                    opts_.runtime->raw_alloc != nullptr) {
                    const uint64_t fn_addr =
                        reinterpret_cast<uint64_t>(opts_.runtime->raw_alloc);
#if defined(_WIN32)
                    const MReg arg0 = MReg::RCX;
                    const MReg arg1 = MReg::RDX;
#else
            ...
            case ir::IrOp::RAW_FREE: {
                ...
                if (ins.operands.size() == 1 &&
                    ...
                    const uint64_t size =
                        (ins.operands[0] < alloca_size_by_vid.size())
                            ? alloca_size_by_vid[ins.operands[0]]
                            : 0;
                    if (size > 0 && size < INT32_MAX) {
                        const uint64_t aligned = (size + 15ULL) & ~15ULL;
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::ADD, MOperand::make_reg(MReg::RSP),
                            MOperand::make_imm32(
                                static_cast<int32_t>(aligned))));
                ...
                if (ins.operands.size() == 1 && opts_.runtime != nullptr &&
                    opts_.runtime->raw_free != nullptr) {
                    const uint64_t fn_addr =
                        reinterpret_cast<uint64_t>(opts_.runtime->raw_free);
#if defined(_WIN32)
                    const MReg arg0 = MReg::RCX;
                    const MReg arg1 = MReg::RDX;
#else
            ...
            case ir::IrOp::GC_DEREF_HOST: {
                ...
                if (ins.operands.size() == 1 && ins.dst != ir::IR_NO_VALUE &&
                    ...
                    const uint64_t fn_addr =
                        reinterpret_cast<uint64_t>(opts_.runtime->gc_deref);
#if defined(_WIN32)
                    const MReg arg0 = MReg::RCX;
                    const MReg arg1 = MReg::RDX;
#else
            ...
            case ir::IrOp::RAW_ASM: {
                ...
                if (asm_text == "gchandle {dst}, {src0}\n" &&
                    ins.operands.size() == 1 && ins.dst != ir::IR_NO_VALUE &&
                    opts_.runtime != nullptr &&
                 ...
                 */
                if (!matched && opts_.mode == SelectorMode::VM_ABI &&
                    opts_.runtime != nullptr &&
                    ...
                    auto stage_load_imm = [&](MReg dst, int64_t v) {
                        if (v >= INT32_MIN && v <= INT32_MAX) {
                            ...
                            const uint32_t pi =
                                mf.intern_imm64(static_cast<uint64_t>(v));
                            staged.push_back(MInstr::make_unary(
                                MOp::MOV, MOperand::make_reg(dst),
                    ...
                    for (size_t li = 0; li < lines.size() && all_ok; ++li) {
                        ...
                        if (opcode == "mov" && args.size() == 2) {
                            ...
                            int ph_src_idx_d = -1;
                            bool ph_is_dst_d = false;
                            int ph_src_idx_s = -1;
                            bool ph_is_dst_s = false;
                            const bool dst_is_ph = parse_placeholder(
                                dst, ph_src_idx_d, ph_is_dst_d);
                            const bool src_is_ph = parse_placeholder(
                                src, ph_src_idx_s, ph_is_dst_s);

                            if (dst_is_ph || src_is_ph) {
                                /* Helpers staged para SSA slots (en frame
                                 * nativo, accesibles via [rbp - 8*(vid+1)]). */
                                auto stage_load_ssa = [&](MReg dst_reg,
                                                          ir::IrValueId vid) {
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV, MOperand::make_reg(dst_reg),
                                        slot_mem(vid)));
                                };
                                auto stage_store_ssa = [&](ir::IrValueId vid,
                                                           MReg src_reg) {
                                    if (vid == ir::IR_NO_VALUE) return;
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV, slot_mem(vid),
                                ...
                                if (src_is_ph) {
                                    if (ph_is_dst_s) {
                                        ...
                                        if (ph_src_idx_s < 0 ||
                                            static_cast<size_t>(ph_src_idx_s) >=
                                                ins.operands.size()) {
                                            all_ok = false;
                                            break;
                                        }
                                    ...
                                    if (src_slot >= 0) {
                                         ...
                                         * por `mov {dst},
                                         * @Absolute("code.handler")` que el
                                         * frontend emite en lower_try para el
                                         * handler_pc de un catch block. */
                                        size_t lp = src.find('(');
                                        size_t rp = src.rfind(')');
                                ...
                                if (dst_is_ph) {
                                    if (ph_is_dst_d) {
                                        ...
                                        if (ph_src_idx_d < 0 ||
                                            static_cast<size_t>(ph_src_idx_d) >=
                                                ins.operands.size()) {
                                            all_ok = false;
                                            break;
                                        }
                                        stage_store_ssa(
                                            ins.operands[ph_src_idx_d],
                                            MReg::RAX);
                                    }
                                } else {
                                    /* dst es VMreg. */
                            ...
                            /* `mov [rN], rM` / `mov [rN], imm` / `mov [rN],
                             * @Absolute(...)`: vrt_vm_write_u64(proc, vaddr,
                             * value) */
                            if (!dst_mem.empty() && src_mem.empty()) {
                                ...
                                staged.push_back(MInstr::make_unary(
                                    MOp::MOV, MOperand::make_reg(ABI_ARG0),
                                    MOperand::make_reg(MReg::RBX)));
                            ...
                            if (dst_mem.empty() && !src_mem.empty()) {
                                ...
                                if (dst_slot == src_slot && li > 0) {
                                    ...
                                    std::string prev_op =
                                        (sp2 == std::string::npos)
                                            ? prev_line
                                            : prev_line.substr(0, sp2);
                                    if (prev_op == "mov") {
                                        std::string prev_rest =
                                            trim_str(prev_line.substr(sp2));
                                            ...
                                            if (try_resolve_at(prev_args[1],
                                                ...
                                                if (cached_value != 0) {
                                                    ...
                                                    stage_load_imm(
                                                        ...
                                                        static_cast<int64_t>(
                                                            cached_value));
                                                    stage_store_slot(dst_slot,
                                                                     MReg::RAX);
                                                    inlined = true;
                                                } else if (
                                                    opts_.reserve_ic_slot) {
                                                    /* Inline cache: reservar 8
                                                     * bytes en code cache, init
                                                     * a 0.  Primera vez popula
                                                     * via vm_read_u64; despues
                                                     * hot path. */
                                                    const uint64_t ic =
                                                        opts_.reserve_ic_slot(
                                                            0);
                        ...
                        /* @c findclass rN, rM:  vrt_findclass(proc,
                         * vaddr_from_rM) -> rN */
                         ...
                         *   2. handle   = vrt_gc_handle_for_ptr(proc, host_ptr)
                         *   3. r0       = handle
                         * El frontend siempre hace `gcderef cur0, r0; xchg
                         * cur0, rM` justo despues, lo que re-deriva host_ptr de
                         * forma correcta.
                         *
                        ...
                        if (opcode == "newobj" && args.size() == 1 &&
                            ...
                            if (li + 2 < lines.size() &&
                                ...
                                std::string op1 = (s1 == std::string::npos)
                                                      ? l1
                                                      : l1.substr(0, s1);
                                std::string op2 = (s2 == std::string::npos)
                                                      ? l2
                                                      : l2.substr(0, s2);
                                if (op1 == "gcderef" && op2 == "xchg") {
                                    std::string r1 =
                                        (s1 == std::string::npos)
                                            ? ""
                                            : trim_str(l1.substr(s1));
                                    std::string r2 =
                                        (s2 == std::string::npos)
                                            ? ""
                                            : trim_str(l2.substr(s2));
                                    auto a1 = split_csv(r1);
                                    auto a2 = split_csv(r2);
                                    if (a1.size() == 2 && a1[0] == "cur0" &&
                                        a1[1] == "r0" && a2.size() == 2 &&
                                        a2[0] == "cur0") {
                                        const int dest_slot =
                                            vm_reg_slot_index(a2[1]);
                                        if (dest_slot >= 0) {
                                            /* Emitir: vrt_newobj(proc, cls) ->
                                             * host_ptr en rax, y storear a
                                             * slot(rM) y slot(r0). */
                                            ...
                                            stage_call(
                                                reinterpret_cast<uint64_t>(
                                                    opts_.runtime->newobj));
                                            stage_store_slot(dest_slot,
                                                             MReg::RAX);
                                            stage_store_slot(
                                                0, MReg::RAX); /* r0 = host_ptr
                                                                  tambien */
                                            li +=
                                                2; /* skipear gcderef + xchg */
                                            fused = true;
                                        }
                                    }
                                }
                            ...
                            if (fused) continue;
                            /* Fallback: ruta original via newobj_handle +
                             * handle. */
                            stage_load_slot(ABI_ARG1, slot);
                            staged.push_back(MInstr::make_unary(
                                MOp::MOV, MOperand::make_reg(ABI_ARG0),
                        ...
                        /* @c deffield rN, rM: vrt_deffield(proc, rN, rM_vaddr)
                         * -> r0 */
                        ...
                        if (opcode == "findmethod" && args.size() == 2 &&
                             ...
                             * VMreg, SSA value placeholder ({dst}/{srcK}), o
                             * imm. */
                            auto stage_load_arg_to =
                                [&](MReg dst_reg,
                                ...
                                if (s >= 0) {
                                    stage_load_slot(dst_reg, s);
                                    return true;
                                }
                                return false;
                            };
                            auto stage_store_arg_from =
                                [&](const std::string &tok,
                                    MReg src_reg) -> bool {
                                if (tok == "{dst}") {
                                    if (ins.dst == ir::IR_NO_VALUE)
                                ...
                                if (tok.size() > 5 && tok[0] == '{' &&
                                    ...
                                    if (n < 0 || static_cast<size_t>(n) >=
                                        ...
                                        return false;
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV, slot_mem(ins.operands[n]),
                                        MOperand::make_reg(src_reg)));
                                    return true;
                                }
                                int s = vm_reg_slot_index(tok);
                                if (s >= 0) {
                                    stage_store_slot(s, src_reg);
                                    return true;
                                }
                                return false;
                            };

                            if (!stage_load_arg_to(ABI_ARG1, args[1])) {
                                all_ok = false;
                                break;
                            }
                            staged.push_back(MInstr::make_unary(
                            ...
                            stage_call(reinterpret_cast<uint64_t>(
                                opts_.runtime->findmethod));
                            if (!stage_store_arg_from(args[0], MReg::RAX)) {
                                all_ok = false;
                                break;
                            }
                            continue;
                        ...
                        if (opcode == "push" && args.size() == 1 &&
                            ...
                            stage_load_slot(ABI_ARG2, src_slot);
                            staged.push_back(MInstr::make_unary(
                                MOp::MOV, MOperand::make_reg(ABI_ARG1),
                                MOperand::make_reg(MReg::RAX)));
                            staged.push_back(MInstr::make_unary(
                                MOp::MOV, MOperand::make_reg(ABI_ARG0),
                                MOperand::make_reg(MReg::RBX)));
                        ...
                        if (opcode == "gcderef" && args.size() == 2 &&
                            args[0] == "cur0" && opts_.runtime->gc_deref) {
                         ...
                         * ================================================== */
                        auto resolve_arg_to_reg = [&](const std::string &tok,
                                                      MReg dst_reg) -> bool {
                            /* {dst} placeholder. */
                            if (tok == "{dst}") {
                        ...
                        /* Helper para almacenar RAX al destino (VMreg/SSA
                         * placeholder). */
                         ...
                         * + mov [rbp - off], rax). */
                        if (opcode == "mov" && args.size() == 2 &&
                            args[0] == "{dst}" && !args[1].empty() &&
                            args[1][0] == '@' && opts_.resolve_symbol) {
                         ...
                         * Si exc_frame_stack_offset==0 (no configurado),
                         * fallback a vrt_tryleave call (~10 instr). */
                        if (opcode == "tryleave" && args.empty()) {
                            if (opts_.exc_frame_stack_offset != 0) {
                                const uint32_t skip_lbl = mf.new_label();
                                const int32_t exc_off =
                                    opts_.exc_frame_stack_offset;
                                const int32_t free_off =
                                    opts_.exc_free_list_offset;
                                ...
                                /* je skip */
                                staged.push_back(
                                    MInstr::make_jcc(MCond::E, skip_lbl));
                                /* mov rcx, [rax + 168]        ; prev */
                                staged.push_back(MInstr::make_unary(
                                    MOp::MOV, MOperand::make_reg(MReg::RCX),
                                    MOperand::make_mem(
                                ...
                                if (free_off != 0) {
                                    /* mov rcx, [rbx + free_off]   ; old free
                                     * head */
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV, MOperand::make_reg(MReg::RCX),
                                        MOperand::make_mem(MReg::RBX,
                                                           free_off)));
                                    /* mov [rax + 168], rcx        ; popped.prev
                                     * = old head */
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV,
                                        MOperand::make_mem(
                                            ...
                                            VESTA_EXC_FRAME_PREV_OFFSET),
                                        MOperand::make_reg(MReg::RCX)));
                                    /* mov [rbx + free_off], rax   ; free head =
                                     * popped */
                                    staged.push_back(MInstr::make_unary(
                                        MOp::MOV,
                                ...
                                /* skip: */
                                staged.push_back(
                                    MInstr::make_label_def(skip_lbl));
                                continue;
                            }
                            /* Fallback: runtime call. */
                            if (!opts_.runtime->tryleave) {
                                all_ok = false;
                                break;
                            }
                            staged.push_back(MInstr::make_unary(
                        ...
                        auto stage_function_epilogue = [&]() {
                            ...
                            for (auto it = regalloc.callee_saved_used.rbegin();
                                 it != regalloc.callee_saved_used.rend();
                                 ++it) {
                                staged.push_back(MInstr::make_unary(
                                    MOp::POP, MOperand::make_reg(*it), {}));
                            }
                        ...
                        /* ==================================================
                         * throw {src0} -> vrt_throw_user(proc, handle) +
                         * epilogue
                         * ==================================================
                         ...
                         * el epilogue garantiza control transferido al
                         * scheduler. */
                        ...
                        if ((opcode == "monenter" || opcode == "monexit") &&
                            ...
                            const uint64_t fn_addr =
                                is_enter ? reinterpret_cast<uint64_t>(
                ...
                if (!matched) {
                    ...
                    if (jit::g_jit_warn_unsupported) {
                        ...
                        if (warned_ops.insert(key).second) {
                            ...
                            std::fprintf(stderr,
                                         "[jit] selector: raw_asm '%s...' no "
                                         "reconocido en fn '%s' linea %u\n",
                                         preview.c_str(), ir_fn.name.c_str(),
                                         ins.source_line);
                        }
                    }
                    unsupported = true;
            ...
            case ir::IrOp::FINDCLASS: {
                ...
                const uint64_t fn_addr =
                    opts_.runtime
                        ? reinterpret_cast<uint64_t>(opts_.runtime->findclass)
                        : 0;
                if (fn_addr == 0) {
                    warn_unsupported(ins.op, ins.source_line,
            ...
            case ir::IrOp::TRYENTER: {
                ...
                const uint64_t fn_addr =
                    opts_.runtime
                        ? reinterpret_cast<uint64_t>(opts_.runtime->tryenter)
                        : 0;
                if (fn_addr == 0) {
                    warn_unsupported(ins.op, ins.source_line,
            ...
            case ir::IrOp::TRYLEAVE: {
                const uint64_t fn_addr =
                    opts_.runtime
                        ? reinterpret_cast<uint64_t>(opts_.runtime->tryleave)
                        : 0;
                if (fn_addr == 0) {
                    warn_unsupported(ins.op, ins.source_line,
             ...
             *   mov rax, [cls_slot]                                ; rax =
             * ClassInfo* mov rax, [rax + VESTA_CLASSINFO_STATIC_DATA_OFFSET] ;
             * rax = static_data ptr mov rax, [rax + imm32_offset] ; rax = i64
             * value store_op(dst, rax)
             *
            ...
            case ir::IrOp::ISNULL: {
                ...
                mf.blocks.back().instrs.push_back(
                    MInstr::make_binary(MOp::XOR, MOperand::make_reg(SCRATCH_B),
                                        MOperand::make_reg(SCRATCH_B), {}));
                mf.blocks.back().instrs.push_back(MInstr::make_binary(
                    MOp::TEST, MOperand::make_reg(MReg::RAX),
                    MOperand::make_reg(MReg::RAX), {}));
            ...
            case ir::IrOp::THROW: {
                ...
                const uint64_t fn_addr =
                    opts_.runtime
                        ? reinterpret_cast<uint64_t>(opts_.runtime->throw_user)
                        : 0;
                if (fn_addr == 0) {
                    warn_unsupported(ins.op, ins.source_line,
            ...
            case ir::IrOp::GC_ALLOCP: {
                ...
                const uint64_t fn_addr =
                    opts_.runtime ? reinterpret_cast<uint64_t>(
                                        opts_.runtime->gc_alloc_payload)
                                  : 0;
                if (fn_addr == 0) {
                    warn_unsupported(
                        ins.op, ins.source_line,
            ...
            case ir::IrOp::MONNOTA: {
                ...
                if (opts_.runtime) {
                    ...
                    case ir::IrOp::MONWAIT:
                        fn_addr = reinterpret_cast<uint64_t>(
                            opts_.runtime->monitor_wait);
                        break;
                    case ir::IrOp::MONNOTI:
                        fn_addr = reinterpret_cast<uint64_t>(
                            opts_.runtime->monitor_notify);
                        break;
                    case ir::IrOp::MONNOTA:
                        fn_addr = reinterpret_cast<uint64_t>(
                            opts_.runtime->monitor_notify_all);
                        break;
                    default: break;
                    }
                }
            ...
            case ir::IrOp::STRCMP: {
                ...
                if (opts_.runtime) {
                    ...
                    case ir::IrOp::STRMAKE:
                        fn_addr =
                            reinterpret_cast<uint64_t>(opts_.runtime->str_make);
                        nargs = 2;
                        break;
                    case ir::IrOp::STRLEN:
                        fn_addr =
                            reinterpret_cast<uint64_t>(opts_.runtime->str_len);
                        nargs = 1;
                        break;
                    case ir::IrOp::STRGETBYTES:
                        fn_addr = reinterpret_cast<uint64_t>(
                            opts_.runtime->str_get_bytes);
                        nargs = 1;
                        break;
                    case ir::IrOp::STRRAW:
                        fn_addr =
                            reinterpret_cast<uint64_t>(opts_.runtime->str_raw);
                        nargs = 1;
                        break;
                    case ir::IrOp::STRCAT:
                        fn_addr =
                            reinterpret_cast<uint64_t>(opts_.runtime->str_cat);
                        nargs = 2;
                        break;
                    case ir::IrOp::STRCMP:
                        fn_addr =
                            reinterpret_cast<uint64_t>(opts_.runtime->str_cmp);
                        nargs = 2;
                        break;
                    default: break;
                    }
                ...
                if (nargs >= 1)
                    load_op_rematerializable(mf, ir_fn, ins.operands[0], arg1);
                if (nargs >= 2)
                    load_op_rematerializable(mf, ir_fn, ins.operands[1], arg2);
                ...
                if (use_ic) {
                    ...
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_unary(MOp::CMP, MOperand::make_reg(arg1),
                                           MOperand::make_mem(MReg::R10, 0)));
                    mf.blocks.back().instrs.push_back(
                        MInstr::make_jcc(MCond::NE, ic_miss_label));
                    if (nargs >= 2) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::CMP, MOperand::make_reg(arg2),
                            MOperand::make_mem(MReg::R10, 8)));
                        mf.blocks.back().instrs.push_back(
                            MInstr::make_jcc(MCond::NE, ic_miss_label));
                    }
                    // mov rax, [r10+16]; test rax,rax; je miss (handle 0 =
                    // empty)
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_reg(MReg::RAX),
                ...
                if (use_ic) {
                    ...
                    // mov [r10+0], arg1
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_mem(MReg::R10, 0),
                        MOperand::make_reg(arg1)));
                    // mov [r10+8], arg2 (or 0 if nargs==1)
                    if (nargs >= 2) {
                        mf.blocks.back().instrs.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_mem(MReg::R10, 8),
                            MOperand::make_reg(arg2)));
                    }
                    // mov [r10+16], r11 (cached handle)
                    mf.blocks.back().instrs.push_back(MInstr::make_unary(
                        MOp::MOV, MOperand::make_mem(MReg::R10, 16),
                        MOperand::make_reg(MReg::R11)));
            ...
            case ir::IrOp::UNWRAP: {
                ...
                const uint64_t throw_addr =
                    opts_.runtime
                        ? reinterpret_cast<uint64_t>(opts_.runtime->throw_fatal)
                        : 0;
                if (throw_addr == 0) {
                    warn_unsupported(ins.op, ins.source_line,
...
#if defined(_WIN32)
                const MReg targ0 = MReg::RCX, targ1 = MReg::RDX,
                           targ2 = MReg::R8;
#else
                const MReg targ0 = MReg::RDI, targ1 = MReg::RSI,
                           targ2 = MReg::RDX;
#endif
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(targ0),
                                       MOperand::make_reg(MReg::RBX)));
                mf.blocks.back().instrs.push_back(MInstr::make_unary(
                    MOp::MOV, MOperand::make_reg(targ1),
                    MOperand::make_imm32(
                        static_cast<int32_t>(VESTA_FATAL_NULL_POINTER))));
                mf.blocks.back().instrs.push_back(
                    MInstr::make_unary(MOp::MOV, MOperand::make_reg(targ2),
                                       MOperand::make_imm32(0)));
            ...
            case ir::IrOp::SETSTATIC: {
                ...
                const int32_t off = static_cast<int32_t>(ins.imm);
                mf.blocks.back().instrs.push_back(MInstr::make_binary(
                    MOp::MOV, MOperand::make_mem(MReg::RAX, off),
                    MOperand::make_reg(SCRATCH_B), {}));
                break;
            }
        ...
        if (!ir_block.instrs.empty()) {
            ...
            if (last_op != ir::IrOp::RET && last_op != ir::IrOp::BR &&
     ...
     * efectos de alignment/cache.  Mantenemos 1 sola iteracion. */
    {
         ...
         *   mov rax, [rbp-8]
         *   ...                       (instrs que no clobean rax ni escriben
         * [rbp-8]) mov rax, [rbp-8]   <-     ELIMINAR
         *
        ...
        for (size_t bi = 0; bi < N_BLOCKS; ++bi) {
                ...
                for (size_t p : preds[bi]) {
                    ...
                    if (first) {
                        ...
                        for (int r = 0; r < 64; ++r) {
                            if (reg_has[r].valid &&
                                (reg_has[r].mem_base !=
                                     exit_states[p][r].mem_base ||
                                 reg_has[r].mem_disp !=
                                     exit_states[p][r].mem_disp ||
                                 reg_has[r].mem_width !=
                                     exit_states[p][r].mem_width ||
            ...
            auto same_mem = [](const MOperand &op, uint8_t base, int32_t disp,
                               uint8_t width) -> bool {
                return op.kind == MOperandKind::MEM && op.reg == base &&
                       op.value == disp && op.width == width;
            };
            ...
            for (const auto &mi : block.instrs) {
                ...
                if (!skip && emit_mi.op == MOp::MOV &&
                    ...
                    emit_mi.dst.reg < 64 && is_slot_mem(emit_mi.src1)) {
                    ...
                    if (reg_has[dst_r].valid &&
                        ...
                        if (!regalloc.empty() && mem_b == reg_id(MReg::RBP)) {
                            const int32_t vid = slot_offset_to_vid(mem_d);
                            if (vid >= 0 &&
                                regalloc.vid_to_reg.count(
                                    static_cast<ir::IrValueId>(vid))) {
                 ...
                 */
                if (!skip && emit_mi.dst.kind == MOperandKind::REG &&
                    ...
                    if (emit_mi.op == MOp::SHL) {
                         ...
                         * count) es no-op. */
                        if (emit_mi.src1.value == 32 && reg_has[r].has_imm32) {
                            last_shl_reg = r;
                ...
                if (!skip && emit_mi.op != MOp::SHL && emit_mi.op != MOp::SAR &&
                    emit_mi.op != MOp::SHR) {
                    last_shl_reg = -1;
                }
                ...
                if (emit_mi.op == MOp::MOV) {
                    if (emit_mi.dst.kind == MOperandKind::REG &&
                        ...
                        if (is_slot_mem(emit_mi.src1)) {
                            reg_has[emit_mi.dst.reg] = {true, emit_mi.src1.reg,
                                                        emit_mi.src1.value,
                                                        emit_mi.src1.width};
                        } else {
                            invalidate_reg(emit_mi.dst.reg);
                               ...
                               emit_mi.src1.kind == MOperandKind::REG &&
                               emit_mi.dst.reg < 64 && emit_mi.src1.reg < 64) {
                        ...
                        if (is_slot_mem(emit_mi.dst)) {
                            ...
                            reg_has[emit_mi.src1.reg] = {true, emit_mi.dst.reg,
                                                         emit_mi.dst.value,
                                                         emit_mi.dst.width};
                        } else {
                ...
                } else if (emit_mi.op == MOp::CMP || emit_mi.op == MOp::TEST ||
                           emit_mi.op == MOp::JMP || emit_mi.op == MOp::JCC ||
                           emit_mi.op == MOp::RET) {
         ...
         * ------------------------------------------------------------- */
        {
            ...
            auto is_pure_slot = [&](const MOperand &op) -> bool {
                return op.kind == MOperandKind::MEM && op.reg == RBP_REG &&
                       (op.width & 0xFC) == 0xFC;
            };
            auto is_store_to_slot = [&](const MInstr &mi) -> bool {
                return mi.op == MOp::MOV && mi.dst.kind == MOperandKind::MEM &&
                       is_pure_slot(mi.dst) &&
                       mi.src1.kind == MOperandKind::REG;
            };
            auto is_load_from_slot = [&](const MInstr &mi) -> bool {
                return mi.op == MOp::MOV && mi.dst.kind == MOperandKind::REG &&
            ...
            for (auto &block : mf.blocks) {
                ...
                for (size_t i = 0; i < instrs.size(); ++i) {
                    ...
                    const size_t lim =
                        std::min(instrs.size(), i + 1 + LOOKAHEAD);
                    for (size_t j = i + 1; j < lim; ++j) {
                        const auto &mj = instrs[j];
                        /* Si STORE al MISMO slot, slot reescrito -> stop. */
                        if (is_store_to_slot(mj) &&
                            same_slot(mj.dst, slot_mem_op))
                            break;
                        /* Si STORE a memoria arbitraria (no slot),
                         * conservativo: stop. */
                        if (mj.op == MOp::MOV &&
                            mj.dst.kind == MOperandKind::MEM &&
                            !is_pure_slot(mj.dst))
                            break;
                        if (mj.op == MOp::CALL)
                            break; /* clobea regs + memoria */
                        ...
                        if (is_load_from_slot(mj) &&
                             ...
                             * sirve como respaldo. */
                            if (!rx_clobbered) {
                                /* Reemplazar load por mov rY, rX (sin
                                 * insertar). */
                                instrs[j].src1 =
                ...
                for (size_t i = 0; i < instrs.size(); ++i) {
                    while (ins_idx < inserts.size() &&
                        ...
                        rebuilt.push_back(MInstr::make_unary(
                            MOp::MOV, MOperand::make_reg(static_cast<MReg>(rY)),
                            MOperand::make_reg(static_cast<MReg>(rX))));
                        ++ins_idx;
                    }
                    if (kill[i]) continue;
         ...
         * solo se usan localmente (el peephole los mantuvo en reg). */
        {
            ...
            auto is_pure_slot = [&](const MOperand &op) -> bool {
                return op.kind == MOperandKind::MEM && op.reg == RBP_REG_DSE &&
                       (op.width & 0xFC) == 0xFC;
            };
            ...
            for (const auto &block : mf.blocks) {
                for (const auto &mi : block.instrs) {
                    ...
                    if (mi.src2.kind == MOperandKind::MEM &&
                        is_pure_slot(mi.src2)) {
            ...
            for (auto &block : mf.blocks) {
                ...
                out.reserve(block.instrs.size());
                for (const auto &mi : block.instrs) {
                    if (mi.op == MOp::MOV && mi.dst.kind == MOperandKind::MEM &&
                        is_pure_slot(mi.dst) &&
    ...
    if (!regalloc.empty()) {
        ...
        auto is_inplace_alu = [](MOp op) -> bool {
            return op == MOp::ADD || op == MOp::SUB || op == MOp::AND ||
                   op == MOp::OR || op == MOp::XOR || op == MOp::IMUL;
        };
            ...
            while (i < block.instrs.size()) {
                if (i + 2 >= block.instrs.size()) {
                    ...
                    if (mi.op == MOp::MOV && mi.dst.kind == MOperandKind::REG &&
                        mi.src1.kind == MOperandKind::REG && mi.dst.reg < 32 &&
                        mi.src1.reg < 32) {
                ...
                uint8_t src_root = m0_ok ? reg_equiv_root(m0.src1.reg) : 0;
                ...
                bool m2_ok =
                    ...
                    (m2.dst.reg == m0.src1.reg || m2.dst.reg == src_root) &&
                    m2.src1.reg == m0.dst.reg;
                bool m1_ok = m2_ok && is_inplace_alu(m1.op) &&
                ...
                if (m1_ok) {
                    ...
                    collapsed.dst = MOperand::make_reg(
                        static_cast<MReg>(regX_id), m2.dst.width);
                    ...
                    if (m0.dst.kind == MOperandKind::REG &&
                        m0.dst.reg != regX_id && m0.dst.reg < 32 &&
                        regX_id < 32) {
                        MInstr refresh;
                        refresh.op = MOp::MOV;
                        refresh.dst = MOperand::make_reg(
                            static_cast<MReg>(m0.dst.reg), m2.dst.width);
                        refresh.src1 = MOperand::make_reg(
                            static_cast<MReg>(regX_id), m2.dst.width);
                        refresh.source_pc = m2.source_pc;
                        out.push_back(refresh);
                        /* Tracking: m0.dst ahora equiv a regX. */
                        reg_equiv[m0.dst.reg] = static_cast<int8_t>(regX_id);
                    }
                    i += 3;
                    continue;
                }
                /* No matchea -- emitir m0 y actualizar tracking. */
                if (m0.op == MOp::MOV && m0.dst.kind == MOperandKind::REG &&
                    m0.src1.kind == MOperandKind::REG && m0.dst.reg < 32 &&
                    m0.src1.reg < 32) {
                    invalidate_reg(m0.dst.reg);
        ...
        for (auto &block : mf.blocks) {
            ...
            for (size_t k = 0; k < block.instrs.size(); ++k) {
                ...
                if (k + 1 < block.instrs.size()) {
                    ...
                    if (cur.op == MOp::MOV && nxt.op == MOp::MOV &&
                ...
                if (cur.op == MOp::MOV && cur.dst.kind == MOperandKind::REG &&
                    ...
                    for (size_t j = k + 1; j < block.instrs.size(); ++j) {
                        ...
                        if (later.dst.kind == MOperandKind::REG &&
                            ...
                            case MOp::LEA:
                            case MOp::POP:
                            case MOp::SETCC: pure_write = true; break;
                            default:
                        ...
                        if (later.op == MOp::CALL ||
                            ...
                            later.op == MOp::JMP || later.op == MOp::JCC ||
                            later.op == MOp::RET || later.op == MOp::IDIV ||
                            later.op == MOp::CQO) {
                ...
                if (i + 1 < n) {
                    ...
                    const bool nxt_dst_mem_bad =
                        ...
                        (cur.src1.kind == MOperandKind::MEM ||
                         cur.src1.kind == MOperandKind::IMM64_IDX);
                    const bool shape = cur.op == MOp::MOV &&
                                       cur.dst.kind == MOperandKind::REG &&
                                       ...
                                       !nxt_dst_uses_scr && !nxt_dst_mem_bad;
                    if (shape) {
                        ...
                        for (size_t j = i + 2; j < n; ++j) {
                            ...
                            bool reads =
                                cc_reads(L.src1, scr) || cc_reads(L.src2, scr);
                            ...
                            if (L.op == MOp::CALL || L.op == MOp::CALL_ABS ||
                                ...
                                L.op == MOp::SAFEPOINT ||
                                L.op == MOp::LOAD_PROC || L.op == MOp::IDIV ||
                                L.op == MOp::CQO) {
                                dead = false;
                                break;
                            }
    ...
    if (cb_entry && !cb_save_all) {
        ...
        for (const auto &blk : mf.blocks) {
            for (const auto &mi : blk.instrs) {
                if (mi.op == MOp::LOAD_PROC)
                    continue; /* el def que evaluamos */
                if (mi.op == MOp::PUSH && mi.src1.kind == MOperandKind::REG &&
                    mi.src1.reg == static_cast<uint8_t>(MReg::RBX))
                    continue;
                if (mi.op == MOp::POP && mi.dst.kind == MOperandKind::REG &&
                    mi.dst.reg == static_cast<uint8_t>(MReg::RBX))
                    continue;
                if (uses_rbx(mi.dst) || uses_rbx(mi.src1) ||
                    uses_rbx(mi.src2)) {
                    rbx_used = true;
                    break;
                }
