...
uint64_t BackendCaps::to_bits() const {
    ...
    if (sse2) b |= CF_SSE2;
    if (sse42) b |= CF_SSE42;
    if (popcnt) b |= CF_POPCNT;
    if (avx) b |= CF_AVX;
    if (avx2) b |= CF_AVX2;
    if (bmi1) b |= CF_BMI1;
    if (bmi2) b |= CF_BMI2;
    if (avx512f) b |= CF_AVX512F;
    if (erms) b |= CF_ERMS;
    if (fma) b |= CF_FMA;
    if (lzcnt) b |= CF_LZCNT;
    if (f16c) b |= CF_F16C;
...
uint64_t backend_caps_host_bits() {
    ...
    if (__get_cpuid(1u, &a, &b, &c, &d)) {
        if (d & (1u << 26)) f |= CF_SSE2;   // EDX.26 SSE2
        if (c & (1u << 20)) f |= CF_SSE42;  // ECX.20 SSE4.2
        if (c & (1u << 23)) f |= CF_POPCNT; // ECX.23 POPCNT
        if (c & (1u << 25)) f |= CF_AES;    // ECX.25 AES-NI
        if (c & (1u << 29)) f |= CF_F16C;   // ECX.29 F16C
        osxsave = (c & (1u << 27)) != 0;    // ECX.27 OSXSAVE
        // AVX (ECX.28) y FMA (ECX.12) requieren estado YMM habilitado por el
        // SO.
        ...
        if (avx_cpu && avx_os) f |= CF_AVX;
        if (fma_cpu && avx_os)
            f |= CF_FMA; // FMA usa el banco YMM -> requiere avx_os
    }
    if (__get_cpuid_count(7u, 0u, &a, &b, &c, &d)) {
        if (b & (1u << 3)) f |= CF_BMI1;             // EBX.3 BMI1 (incl. TZCNT)
        if (b & (1u << 8)) f |= CF_BMI2;             // EBX.8 BMI2 (PDEP/PEXT)
        if (b & (1u << 9)) f |= CF_ERMS;             // EBX.9 ERMS
        if (b & (1u << 29)) f |= CF_SHA;             // EBX.29 SHA
        if (avx_os && (b & (1u << 5))) f |= CF_AVX2; // EBX.5 AVX2 (banco YMM)
...
BackendCaps backend_caps_from_cpu(const std::string &cpu_name) {
    ...
    if (cpu_name.empty() || cpu_name == "generic") return c;
    const int32_t id =
        vx::instr_db::cpu_by_name(vx::instr_db::Isa::X86, cpu_name);
    if (id < 0) return c; // microarq desconocida -> baseline seguro
...
BackendCaps backend_caps_from_float_isa(FloatIsa fisa) {
    ...
    case FloatIsa::AVX512F:
        ...
        break;
    case FloatIsa::AVX:
        // El nivel `avx` de --float-isa = AVX2 256b (ver la ayuda).  AVX2
        // implica FMA3 en la practica (Haswell+; no hay CPU con AVX2 sin FMA3),
        // asi que el target AVX2 SI puede emitir VFMADD.
        c.avx = true;
        c.avx2 = true;
        c.fma = true;
        c.sse42 = true;
...
BackendCaps resolve_backend_caps(const std::string &cpu, bool jit_host,
    ...
    if (jit_host) return backend_caps_host(); // 2. host (JIT)
