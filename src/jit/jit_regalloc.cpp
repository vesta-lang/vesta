...
constexpr MReg CALLEE_SAVED_POOL[] = {MReg::R12, MReg::R13, MReg::R14,
                                      MReg::R15};
constexpr size_t POOL_SIZE =
    sizeof(CALLEE_SAVED_POOL) / sizeof(CALLEE_SAVED_POOL[0]);
...
std::vector<VidInfo> collect_vid_info(const ir::IrFunction &fn) {
    ...
    for (size_t b = 0; b < fn.blocks.size(); ++b) {
        const uint32_t bs =
            (b < live.block_start.size()) ? live.block_start[b] : 0;
        for (size_t i = 0; i < fn.blocks[b].instrs.size(); ++i) {
            ...
            for (const auto &arg : ins.phi_args) {
                if (arg.value == ir::IR_NO_VALUE || arg.value >= info.size())
                    continue;
            ...
            case ir::IrOp::THROW:
            case ir::IrOp::TAILCALL: is_call_like = true; break;
 ...
 */
void filter_candidates(const ir::IrFunction &fn, std::vector<VidInfo> &info) {
    (void)fn;
    for (auto &vi : info) {
 ...
 */
JitRegalloc assign_regs(const ir::IrFunction &fn, std::vector<VidInfo> &info) {
    JitRegalloc result;
    ...
    for (size_t cidx : candidates) {
        ...
        if (preferred_r >= 0) {
            ...
            result.vid_to_reg[info[cidx].vid] = CALLEE_SAVED_POOL[preferred_r];
            placed = true;
        }
        if (!placed) {
...
JitRegalloc compute_jit_regalloc(const ir::IrFunction &fn) {
    ...
    if (s_debug) {
        std::fprintf(stderr,
                     "[jit-regalloc] fn=%s assigned=%zu callee_saved=%zu\n",
                     fn.name.c_str(), result.vid_to_reg.size(),
                     result.callee_saved_used.size());
        for (const auto &p : result.vid_to_reg) {
            uint32_t uc = (p.first < info.size()) ? info[p.first].use_count : 0;
            bool xc =
                (p.first < info.size()) ? info[p.first].crosses_call : false;
            std::fprintf(stderr, "  vid=%u reg_id=%u use=%u xcall=%d\n",
                         p.first, static_cast<unsigned>(p.second), uc,
                         xc ? 1 : 0);
        }
    }
    return result;
