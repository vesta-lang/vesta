...
/// (SSE/AVX/AVX512: XMM/YMM/ZMM).  0 = DESCONOCIDO -> el llamador marca el rango
/// como NO exacto (aliasa conservador).  IMPRESCINDIBLE: sub-estimar un acceso
/// vectorial de 32 B como 8 perderia el solapamiento con un acceso a offset
/// 8..31 -> dependencia de memoria perdida -> lectura sin inicializar.
...
MemRef extract_memref(const MInstr &mi, const MEffects &e,
    ...
    case MOp::STORE_VM: {
        ...
        const int w = (mi.op == MOp::LOAD || mi.op == MOp::LOAD_VM)
                          ? (mi.flags >> 1)
                          : mi.flags;
        const bool w_known = (w == 1 || w == 2 || w == 4 || w == 8 ||
                              w == 16 || w == 32 || w == 64);
        r.size = w_known ? w : 8;
...
int schedule_region(std::vector<MInstr> &ins, int lo, int hi,
    ...
    for (int i = 0; i < n; ++i) ic[i] = cm.cost(ins[lo + i]);
    ...
    // Modelo de RECURSOS del core: ancho de emision (uops/ciclo) + capacidad por
    // grupo de puertos.  El scheduler no programa en el mismo ciclo mas uops de
    // los que el core puede emitir ni mas de los que caben en cada grupo de
    // puertos -> modela la contencion superescalar real (no solo la latencia).
    const int np = cm.port_count();
    const int iw = cm.issue_width() > 0 ? cm.issue_width() : 4;
    std::vector<int> pcap(np > 0 ? np : 1, 1);
    for (int g = 0; g < np; ++g) pcap[g] = std::max(1, cm.port_capacity(g));
    ...
    // ops LISTAS (indeg==0 -> todas sus dependencias colocadas; el orden SIEMPRE
    // respeta el DAG) prefiere las que quepan en el ciclo actual (issue width +
    // puertos), por camino critico.  GARANTIA DE PROGRESO: si ninguna ready-ya
    // cabe (contencion de recursos), FUERZA la de mayor prioridad -- nunca se
    // bloquea ni reordena fuera de las dependencias (correccion > calidad).
    ...
    // Compara la prioridad de dos nodos (camino critico, luego ready, luego id).
    ...
    // Env-gated (no #ifndef NDEBUG) para que sirva tambien en builds con NDEBUG.
    static const bool verify = std::getenv("VESTA_SCHED_VERIFY") != nullptr;
    if (verify) {
        std::vector<int> vpos(n);
        for (int k = 0; k < n; ++k) vpos[order[k]] = k;
        for (int i = 0; i < n; ++i)
            for (int j = i + 1; j < n; ++j)
...
int schedule_function(MFunction &mf, const SchedCostModel &cm, EffIsa isa) {
    ...
    for (MBlock &blk : mf.blocks) {
        ...
        //
        // PROCEDENCIA POSICIONAL: post-regalloc un registro fisico se REUSA para
        // objetos distintos, asi que el objeto de un @c base es el de su
        ...
        // por-bloque: reset al entrar (salvo RBP/RSP=FRAME; un base de un bloque
        // previo queda UNKNOWN -> aliasa, conservador), extraer la memref ANTES
        // de actualizar el dst, e INVALIDAR el objeto del dst en cualquier
        // escritura que no PROPAGUE procedencia (solo ALLOCA/MOV/LEA lo hacen).
