...
namespace vx {
        ...
        if (current_.kind == TokenKind::STRING_LIT ||
            ...
            if (current_.kind == TokenKind::DOT && lex_.peek_at(0).kind == TokenKind::LBRACE) {
                ...
                for (;;) {
                    ...
                    // `(fn_importada)(args)`.
                    declared_aliases_.insert(os.rename.empty()
                                                 ? os.name
                                                 : os.rename);
    ...
    std::unique_ptr<ast::Expr> Parser::parse_relational() {
        ...
        while (current_.kind == TokenKind::LT || current_.kind == TokenKind::LE ||
            ...
            switch (current_.kind) {
                case TokenKind::LT: op = ast::BinOp::Lt;
                    break;
                case TokenKind::LE: op = ast::BinOp::Le;
                    break;
                case TokenKind::GT: op = ast::BinOp::Gt;
                    break;
                case TokenKind::GE: op = ast::BinOp::Ge;
                    break;
                default: break;
            }
            const SourceLoc loc = current_.loc;
    ...
    std::unique_ptr<ast::Expr> Parser::parse_multiplicative() {
        ...
        while (current_.kind == TokenKind::STAR ||
            ...
            switch (current_.kind) {
                case TokenKind::STAR: op = ast::BinOp::Mul;
                    break;
                case TokenKind::SLASH: op = ast::BinOp::Div;
                    break;
                case TokenKind::PERCENT: op = ast::BinOp::Mod;
                    break;
                default: break;
            }
            const SourceLoc loc = current_.loc;
    ...
    std::unique_ptr<ast::Expr> Parser::parse_unary() {
        ...
        switch (current_.kind) {
            ...
            case TokenKind::STAR: {
                ...
                switch (current_.kind) {
                    case TokenKind::BANG: op = ast::UnOp::LogicalNot;
                        break;
                    case TokenKind::BANG_BANG: op = ast::UnOp::Unwrap;
                        break;
                    case TokenKind::TILDE: op = ast::UnOp::BitNot;
                        break;
                    case TokenKind::MINUS: op = ast::UnOp::Neg;
                        break;
                    case TokenKind::PLUS: op = ast::UnOp::Pos;
                        break;
                    case TokenKind::PLUS_PLUS: op = ast::UnOp::PreInc;
                        break;
                    case TokenKind::MINUS_MINUS: op = ast::UnOp::PreDec;
                        break;
                    case TokenKind::AMP: op = ast::UnOp::AddrOf;
                        break;
                    case TokenKind::STAR: op = ast::UnOp::Deref;
                        break;
                    default: break;
                }
                (void) consume();
            ...
            switch (current_.kind) {
                ...
                case TokenKind::QUESTION: {
                    ...
                    switch (next.kind) {
                        ...
                        case TokenKind::END_OF_FILE: is_postfix_q = true;
                            break;
                        default:
                        ...
                        while (true) {
                            ...
                            if (is_raw) {
                                ...
                                while (current_.kind != TokenKind::END_OF_FILE) {
                                    ...
                                    switch (current_.kind) {
                                        case TokenKind::LPAREN: ++paren_depth;
                                            break;
                                        case TokenKind::RPAREN: --paren_depth;
                                            break;
                                        case TokenKind::LBRACKET: ++brack_depth;
                                            break;
                                        case TokenKind::RBRACKET: --brack_depth;
                                            break;
                                        case TokenKind::LBRACE: ++brace_depth;
                                            break;
                                        case TokenKind::RBRACE: --brace_depth;
                                            break;
                                        default: break;
                                    }
                                    (void) consume();
