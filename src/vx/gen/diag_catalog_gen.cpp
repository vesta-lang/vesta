...
const CatEntry kEntries[] = {
    ...
    {"VX9252", {"the size of `.pdata` is not a multiple of 12: a RUNTIME_FUNCTION is three RVAs, so a half entry would be left at the end and the system would read it anyway.", "el tamano de `.pdata` no es multiplo de 12: un RUNTIME_FUNCTION son tres RVA, asi que quedaria una entrada a medias al final y el sistema la leeria igual."}},
    ...
    {"VX9258", {"symbols with no provider: {0}.  They are not defined in the image, and no `extern` block or plugin declares which library they come from, so there is no import to emit -- and none is invented, because a made-up one only moves the failure to load time on someone else's machine.  Every runtime service is a hook: it is provided by the stdlib module that covers it by default, by an override of your own (`@AllocatorOverride`, `@PanicHandler`, ...), by an `extern` naming the library, or by you under `--freestanding`.", "simbolos sin proveedor: {0}.  No estan definidos en la imagen, y ningun bloque `extern` ni plugin declara de que biblioteca salen, asi que no hay ningun import que emitir -- y no se inventa uno, porque inventarlo solo mueve el fallo al momento de CARGAR en la maquina de otro.  Cada servicio de runtime es un gancho: lo provee el modulo de la stdlib que lo cubre por defecto, un override tuyo (`@AllocatorOverride`, `@PanicHandler`, ...), un `extern` que diga la biblioteca, o tu mismo con `--freestanding`."}},
