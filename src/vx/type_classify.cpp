 ...
 * los structs Vesta no pueden contenerse a si mismos por valor (tamano
 * infinito, el type checker ya lo rechazaria), pero acotamos por si el resolver
...
bool is_c_representable_rec(const Type &t, const StructResolver &find_struct,
    ...
    case PrimitiveKind::PTR: return !t.is_virtual;
    ...
    case PrimitiveKind::STRUCT: {
        const StructLayout *lay =
            find_struct ? find_struct(t.struct_name) : nullptr;
...
bool is_managed_rec(const Type &t, const StructResolver &find_struct,
    ...
    case PrimitiveKind::UNIQUE_PTR:
    case PrimitiveKind::SHARED_PTR:
    case PrimitiveKind::GC_PTR: return true;
    // Array: gestionado si su elemento lo es.
    case PrimitiveKind::ARRAY:
        return t.pointee && is_managed_rec(*t.pointee, find_struct, depth + 1);
    ...
    case PrimitiveKind::STRUCT: {
        const StructLayout *lay =
            find_struct ? find_struct(t.struct_name) : nullptr;
        if (!lay)
            return false; // no resuelto -> no gestionado (conservador
