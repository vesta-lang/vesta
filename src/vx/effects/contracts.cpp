...
/// pure: sin efectos de dato observables en TODO el cierre (sound).
static bool p_pure(const FunctionSummary &s) {
    const SemanticEffects &c = s.semantic.closure;
    return s.completeness != AnalysisCompleteness::Unknown &&
           !c.mem.writes_memory() && !c.may_throw && !c.may_allocate &&
           !c.may_io && !c.may_block && c.tags.empty();
}

/// readonly: no escribe memoria en el cierre (puede leer).
static bool p_readonly(const FunctionSummary &s) {
    return s.completeness != AnalysisCompleteness::Unknown &&
...
static bool p_heap_free(const FunctionSummary &s) {
    return s.completeness != AnalysisCompleteness::Unknown &&
...
/// distinga GC de raw).
static bool p_gc_free(const FunctionSummary &s) { return p_heap_free(s); }

/// freestanding: no toca estado de maquina privilegiado ni I/O (candidato a Bare).
static bool p_freestanding(const FunctionSummary &s) {
    const SemanticEffects &c = s.semantic.closure;
    const bool machine =
...
std::vector<EvaluatedContract> derive_contracts(const FunctionSummary &s) {
    ...
    for (const ContractRule &r : rules)
        out.push_back({r.name, r.predicate(s)});
    return out;
}
