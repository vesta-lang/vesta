...
bool AuthManager::init(const std::string &db_path) {
    ...
    int rc =
        sqlite3_open_v2(resolved.string().c_str(), &g_auth_db, flags, nullptr);
    if (rc != SQLITE_OK) {
        std::ostringstream m;
        m << "no se pudo abrir SQLite en '" << resolved.string() << "': "
          << (g_auth_db ? sqlite3_errmsg(g_auth_db) : sqlite3_errstr(rc));
    ...
    const char *schema = "CREATE TABLE IF NOT EXISTS vm_users ("
...
bool AuthManager::derive_key(const std::string &password, const uint8_t *salt,
                             size_t salt_len, uint8_t *out_hash,
                             size_t out_len) {
    ...
    int rc =
        PKCS5_PBKDF2_HMAC(password.data(), static_cast<int>(password.size()),
                          salt, static_cast<int>(salt_len), PBKDF2_ITERS,
                          EVP_sha256(), static_cast<int>(out_len), out_hash);
    return rc == 1;
}
...
bool AuthManager::create_user(const std::string &username,
    ...
    const char *sql = "INSERT INTO vm_users (username, password_hash, salt, "
                      "role, created_at) "
                      "VALUES (?, ?, ?, ?, ?);";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(g_auth_db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
...
AuthResult AuthManager::login(const std::string &username,
...

    {
        ...
        sqlite3_stmt *stmt = nullptr;
        const char *sql = "SELECT password_hash, salt, role FROM vm_users "
                          "WHERE username = ?;";
        if (sqlite3_prepare_v2(g_auth_db, sql, -1, &stmt, nullptr) !=
            SQLITE_OK) {
        ...
        if (h_n != static_cast<int>(PBKDF2_HASH_LEN) ||
            s_n != static_cast<int>(PBKDF2_SALT_LEN) || !h_blob || !s_blob ||
            !r_text) {
            sqlite3_finalize(stmt);
    ...
    if (!derive_key(password, stored_salt, PBKDF2_SALT_LEN, computed,
                    PBKDF2_HASH_LEN)) {
