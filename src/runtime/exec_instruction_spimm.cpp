 ...
 * @brief Implementacion de las instrucciones de aritmetica de pila SUBSP y ADDSP.
 *
 ...
 *
 * Formato del bytecode (MIXED_SIZE, siempre 11 bytes para registros de 64 bits):
 *   [0x00][0x2E|0x2F][ctrl][imm64]
 *   ctrl: bits 7-6 = 3 (modo 64 bits), bits 1-0 = sp_bp (0=RSP, 1=RBP)
 */
...
namespace runtime {
    ...
    void exec_instr_subsp(ProcessVM *vm, const DecodedInstr &instr) {
        ...
        const uint64_t delta = instr.data_instruction.inmmed_data.inmmed; // valor a restar
    ...
    void exec_instr_addsp(ProcessVM *vm, const DecodedInstr &instr) {
        const uint8_t  sp_bp = instr.data_instruction.inmmed_data.reg;    // 0=RSP 1=RBP
        const uint64_t delta = instr.data_instruction.inmmed_data.inmmed; // valor a sumar
