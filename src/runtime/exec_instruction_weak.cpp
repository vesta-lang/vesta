 ...
 * la tabla de weak refs que apuntan a objetos muertos se anulan automaticamente.
 *
 *  WEAKREF   r_handle          (0x00 0x30): registra GcHandle; R0 = indice opaco.
 *  DEREF_WEAK r_dst, r_idx     (0x00 0x32): r_dst = GcHandle si vivo, GC_NULL_HANDLE si muerto.
 *  FREE_WEAK  r_idx            (0x00 0x34): libera la entrada de la tabla de weak refs.
 */
...
namespace runtime {
    ...
    void exec_instr_weakref(ProcessVM *vm, const DecodedInstr &instr) {
        ...
        const uint32_t handle   = static_cast<uint32_t>(vm->registers.regs[r_handle].qword());
    ...
    void exec_instr_deref_weak(ProcessVM *vm, const DecodedInstr &instr) {
        ...
        const uint32_t idx = static_cast<uint32_t>(vm->registers.regs[r_idx].qword());
