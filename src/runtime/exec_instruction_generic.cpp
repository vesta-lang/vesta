 ...
 * @brief Implementacion de la instruccion de instanciacion de genericos en runtime.
 *
 ...
 *
 * Operandos:
 *   r_class  -- registro con puntero host a ClassInfo generica (CLASS_FLAG_GENERIC).
 *   r_types  -- registro con puntero host a array de ClassInfo* (tipos concretos).
 *   count    -- nibble bajo de byte3; numero de parametros de tipo (0-15).
...
namespace runtime {
     ...
     * Si count excede type_param_count de la clase generica, se usa type_param_count.
     *
    ...
    void exec_instr_specialize(ProcessVM *vm, const DecodedInstr &instr) {
        // desempaquetar operandos del campo mem_data (reutiliza decode_instr_jumptable)
        const uint8_t r_dst   = instr.data_instruction.mem_data.reg_base;   // nibble alto byte2
        const uint8_t r_class = instr.data_instruction.mem_data.reg_index;  // nibble bajo byte2
        const uint8_t r_types = (instr.data_instruction.mem_data.scale >> 4) & 0x0Fu; // nibble alto byte3
        const size_t  count   = static_cast<size_t>(instr.data_instruction.mem_data.scale & 0x0Fu); // nibble bajo byte3
