...
namespace pkg {
    ...
    namespace {
        ...
        static DependencySpec parse_dep_spec_from_toml(const std::string &name,
            ...
            if (v.is_table()) {
                ...
                // tag/branch tambien soportados via git_rev (priority: rev > tag > branch)
                std::string tag    = v.get_string("tag");
                std::string branch = v.get_string("branch");
                if (d.git_rev.empty() && !tag.empty())    d.git_rev = "tag:" + tag;
                if (d.git_rev.empty() && !branch.empty()) d.git_rev = "branch:" + branch;
        ...
        static void parse_trust_from_toml(const toml::TomlValue &trust,
                                            Manifest &m) {
        ...
        ParseResult parse_json(const std::string &buffer,
            ...
            try {
                ...
                if (j.contains("package")) {
                    ...
                    if (p.contains("version")) m.version = p["version"].get<std::string>();
                    if (p.contains("license")) m.license = p["license"].get<std::string>();
                    if (p.contains("edition")) m.edition = p["edition"].get<int>();
                    if (p.contains("authors") && p["authors"].is_array()) {
                        for (auto &a : p["authors"]) {
                            if (a.is_string()) m.authors.push_back(a.get<std::string>());
                        }
                    }
                }
                if (j.contains("capabilities")) {
                    ...
                    if (c.contains("declared") && c["declared"].is_array()) {
                        for (auto &v : c["declared"]) {
                            if (v.is_string()) m.capabilities.declared.push_back(v.get<std::string>());
                        }
                    }
                }
                auto parse_deps = [&](const nlohmann::json &j_deps, bool is_dev) {
                    ...
                    for (auto it = j_deps.begin(); it != j_deps.end(); ++it) {
                        ...
                        if (v.is_string()) {
                            ...
                            if (v.contains("version"))   d.version = v["version"].get<std::string>();
                            if (v.contains("sha256"))    d.sha256  = v["sha256"].get<std::string>();
                            if (v.contains("git"))       d.git_url = v["git"].get<std::string>();
                            if (v.contains("rev"))       d.git_rev = v["rev"].get<std::string>();
                            if (v.contains("path"))      d.path    = v["path"].get<std::string>();
                            if (v.contains("trust"))     d.trust_key = v["trust"].get<std::string>();
                            std::string tag, branch, url;
                            if (v.contains("tag"))    tag    = v["tag"].get<std::string>();
                            if (v.contains("branch")) branch = v["branch"].get<std::string>();
                ...
                if (j.contains("dev-dependencies")) parse_deps(j["dev-dependencies"], true);
                ...
                if (j.contains("scripts") && j["scripts"].is_object()) {
                    for (auto it = j["scripts"].begin(); it != j["scripts"].end(); ++it) {
                        if (it.value().is_string()) {
                            m.scripts.emplace_back(it.key(), it.value().get<std::string>());
                        }
                    }
                }
                res.ok = true;
    ...
    std::string serialize_manifest_toml(const Manifest &m) {
        ...
        for (const auto &d : m.dependencies) {
            if (d.is_dev) deps_dev.push_back(&d);
            else          deps_normal.push_back(&d);
        }
        ...
        if (!deps_normal.empty()) {
            out << "[dependencies]\n";
            for (auto *d : deps_normal) emit_dep(d);
            out << "\n";
        }
        if (!deps_dev.empty()) {
            out << "[dev-dependencies]\n";
            for (auto *d : deps_dev) emit_dep(d);
            out << "\n";
        }
    ...
    ValidationResult validate_manifest(const Manifest &m) {
        ...
        for (const auto &d : m.dependencies) {
            if (d.path.empty()) {
                ...
                if (d.sha256.empty()) {
                    vr.warnings.push_back("dep '" + d.name +
                        "': sin 'sha256' (modo dev; falla en install si no hay vex.lock)");
            ...
            if (d.git_url.empty() && d.path.empty()) {
                ...
                if (!d.version.empty()) {
                    vr.warnings.push_back("dep '" + d.name +
                        "': solo 'version' sin 'git' ni 'path' requiere un registry (vacio por defecto)");
                } else {
                    vr.ok = false;
                    vr.errors.push_back("dep '" + d.name +
                        "': debe especificar 'git=', 'url=', 'path=' o 'version=' (con registry)");
                }
            }
        }
        for (const auto &t : m.trust_pins) {
            if (!is_valid_fingerprint(t.fingerprint)) {
                vr.ok = false;
                vr.errors.push_back("trust pin '" + t.glob +
