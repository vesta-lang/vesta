...
namespace pkg::cli {
    ...
    namespace {
        ...
        std::string find_project_manifest(std::string &out_format) {
            ...
            if (has_toml && has_json) {
                ui::error("ambos vex.toml y vex.json existen; elige uno y borra el otro");
                return std::string();
            }
            if (has_toml) { out_format = "toml"; return toml_p; }
            if (has_json) { out_format = "json"; return json_p; }
            return std::string();
        }
    ...
    static int cmd_init(const std::vector<std::string> &args) {
        ...
        std::string content = json
            ? serialize_manifest_json(m)
            : serialize_manifest_toml(m);
    ...
    static int cmd_install(const std::vector<std::string> &args) {
        ...
        for (const auto &w : vr.warnings) ui::warn(w);
        if (!vr.ok) {
            for (const auto &e : vr.errors) ui::error(e);
            return 1;
        }
        ...
        for (const auto &w : res.warnings) ui::warn(w);
        if (!res.ok) {
        ...
        if (rep.critical_count > 0 && !allow_unsigned) {
            ui::error("auditoria critica detecto issues; usa --allow-unsigned para forzar");
            return 1;
        }
    ...
    static int cmd_keygen(const std::vector<std::string> &args) {
        ...
        ui::info("comparte esta fingerprint con quienes vayan a verificar tus paquetes");
        return 0;
    }
    ...
    static int cmd_trust(const std::vector<std::string> &args) {
        ...
        if (sub == "list") {
            ...
            for (const auto &p : pins) {
                std::cout << "  " << ui::pkg_name(p.author_name.empty() ? "(anonimo)" : p.author_name)
                           << "  " << ui::short_hash(p.fingerprint);
                if (p.revoked) std::cout << " " << ui::red() << "[revoked]" << ui::reset();
    ...
    static int cmd_convert(const std::vector<std::string> &args) {
        ...
        std::string new_path = paths::join(paths::project_root(""),
                                            std::string("vex.") + fmt);
        std::string body = fmt == "toml"
            ? serialize_manifest_toml(pr.manifest)
    ...
    static int cmd_inspect(const std::vector<std::string> &args) {
        ...
        ui::kv("blob_pool_align",    std::to_string(
            static_cast<unsigned>(mod.blob_pool_alignment)));
        ...
        for (const auto &s : mod.symbols) {
            std::cout << "  "
                       << ui::magenta() << kind_to_str(s.kind) << ui::reset()
                       << "  "
                       << ui::pkg_name(s.name);
            // Tipo / firma.
            if (s.kind == vex::VexiSymbolKind::FUNCTION) {
                ...
                if (s.is_extern) {
                    std::cout << "  " << ui::yellow() << "[extern "
                              << s.extern_lib << "]" << ui::reset();
                }
            } else if (s.kind == vex::VexiSymbolKind::GLOBAL_VAR) {
                std::cout << "  " << ui::gray() << ": " << s.underlying_type
                           << ui::reset();
                if (s.is_const) std::cout << "  " << ui::cyan() << "const" << ui::reset();
                if (s.has_init_value) {
                    char buf[32];
                ...
                if (s.attr_align != 0)
                    std::cout << "  " << ui::cyan() << "@align("
                               << s.attr_align << ")" << ui::reset();
                ...
                if (!s.super_class.empty()) {
                    std::cout << "  " << ui::cyan() << "extends "
                               << s.super_class << ui::reset();
                }
            } else if (s.kind == vex::VexiSymbolKind::ENUM) {
                std::cout << "  " << ui::gray()
                           << "variants=" << s.variants.size() << ui::reset();
        ...
        if (!mod.blob_pool.empty()) {
            ...
            while (off + sizeof(vex::VexiBlobHeader) <= mod.blob_pool.size()) {
                const vex::VexiBlobHeader *bh =
                    vex::vexi_blob_read(mod.blob_pool,
    ...
    static int cmd_run(const std::vector<std::string> &args) {
        ...
        for (const auto &kv : pr.manifest.scripts) {
            if (kv.first == name) { cmdline = kv.second; break; }
        }
        if (cmdline.empty()) {
    ...
    int run_args(const std::vector<std::string> &args) {
        ...
        if (args.empty()) {
            ...
            ui::info("subcomandos: init, install, list, verify, audit, trust, keygen, convert, inspect, run");
            return 1;
        }
