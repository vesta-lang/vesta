...
SourceSpec resolve_github_import(const std::string &import_path) {
    ...
    if (!ref.empty()) {
        // Heuristica: si comienza con v y digito es tag, hexadecimal largo es
        // rev, si no asumimos branch.
        if (ref.size() >= 7) {
            bool all_hex = true;
            for (char c : ref) {
...
FetchResult fetch_git(const SourceSpec &spec, const std::string &dest_dir) {
    FetchResult r;
    if (!git_available()) {
        return fail("git no esta disponible en PATH; instala git para fetch de "
                    "paquetes");
    }
    ...
    if (!spec.rev.empty()) {
        ...
        CmdResult cr =
            run_cmd("cd " + sh_quote(dest_dir) + " && git rev-parse HEAD");
...
FetchResult fetch(const SourceSpec &spec, const std::string &dest_dir) {
    switch (spec.kind) {
    case SourceSpec::Git:
    case SourceSpec::Github: return fetch_git(spec, dest_dir);
    case SourceSpec::Path: return fetch_path(spec, dest_dir);
    case SourceSpec::Url:
    case SourceSpec::Zip:
        ...
        return fail("fetch URL/zip no implementado en MVP; usa git+https://... "
                    "o path local");
    }
