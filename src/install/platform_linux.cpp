...
static std::filesystem::path home_dir() {
    ...
    return pw ? std::filesystem::path(pw->pw_dir)
...
class PlatformLinux : public Platform {
  public:
    ...
    std::string elevation_hint() const override {
        return "Re-ejecuta este comando con sudo: sudo vesta install "
               "--system-wide";
    }
    ...
    std::filesystem::path
    default_bin_dir(const std::filesystem::path & /*prefix*/) const override {
        // bin separado: distinto del prefix
        if (geteuid() == 0) return "/usr/local/bin";
        return home_dir() / ".local" / "bin";
    }
    std::filesystem::path
    default_share_dir(const std::filesystem::path & /*prefix*/) const override {
    ...
    bool register_associations(const InstallOptions &opts,
        ...
        std::filesystem::path apps_dir =
            (geteuid() == 0) ? "/usr/share/applications"
                             : home_dir() / ".local/share/applications";
        std::filesystem::path mime_dir =
            (geteuid() == 0) ? "/usr/share/mime/packages"
    ...
    bool add_to_path(const InstallOptions &opts, Manifest &mf) override {
        ...
        // System-wide: si el bin_dir es /usr/local/bin, ya esta en PATH
        // default.
        ...
        for (const char *sh : shells) {
            ...
            if (content.find(marker_begin) != std::string::npos)
                continue; // ya esta
    ...
    bool create_shortcuts(const InstallOptions &opts, Manifest &mf) override {
        ...
        if (opts.create_desktop_shortcut) {
            ...
            if (!ec) {
                std::filesystem::permissions(
                    desktop, std::filesystem::perms::owner_all,
    ...
    bool unregister_associations(const Manifest &mf) override {
        std::error_code ec;
        for (auto &d : mf.desktop_files)
            std::filesystem::remove(d.path, ec);
        for (auto &m : mf.mime_files)
    ...
    bool remove_files(const Manifest &mf, bool /*keep_user_data*/) override {
        ...
        for (auto &s : mf.symlinks)
            std::filesystem::remove(s.link, ec);
        // Borrar ficheros
