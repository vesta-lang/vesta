 ...
 *   - Asociaciones via HKLM\Software\Classes (system) o HKCU\Software\Classes
 * (user)
...
static bool reg_delete_tree(HKEY root, const std::string &subkey) {
    ...
    LONG rc = RegOpenKeyExA(root, subkey.c_str(), 0, KEY_READ | KEY_WRITE, &k);
    ...
    for (;;) {
        ...
        FILETIME ft{};
        LONG er = RegEnumKeyExA(k, 0, name, &name_len, nullptr, nullptr,
                                nullptr, &ft);
...
static bool reg_set_sz(HKEY root, const std::string &subkey,
    ...
    LONG rc = RegSetValueExA(
        k, value_name.empty() ? nullptr : value_name.c_str(), 0, REG_SZ,
        reinterpret_cast<const BYTE *>(data.c_str()), (DWORD)data.size() + 1);
    RegCloseKey(k);
    return rc == ERROR_SUCCESS;
}
...
static bool reg_set_dword(HKEY root, const std::string &subkey,
    ...
    if (!reg_create_key(root, subkey, k)) return false;
    LONG rc =
        RegSetValueExA(k, value_name.c_str(), 0, REG_DWORD,
...
static bool reg_get_sz(HKEY root, const std::string &subkey,
    ...
    if (RegOpenKeyExA(root, subkey.c_str(), 0, KEY_READ, &k) != ERROR_SUCCESS)
        return false;
    DWORD type = 0, len = 0;
    if (RegQueryValueExA(k, value_name.c_str(), nullptr, &type, nullptr,
                         &len) != ERROR_SUCCESS) {
        RegCloseKey(k);
        return false;
    }
...
static bool reg_delete_value(HKEY root, const std::string &subkey,
                             const std::string &value_name) {
    HKEY k = nullptr;
    if (RegOpenKeyExA(root, subkey.c_str(), 0, KEY_ALL_ACCESS, &k) !=
        ERROR_SUCCESS)
        return false;
...
static bool is_build_garbage(const std::filesystem::path &p) {
    ...
    if (fn == "Makefile" || fn == "cmake_install.cmake" ||
        fn == "CMakeCache.txt" || fn == "CTestTestfile.cmake")
        return true;
    ...
    if (ext == ".obj" || ext == ".o" || ext == ".lib" || ext == ".exp" ||
...
static bool copy_dir_recursive(const std::filesystem::path &src,
                               const std::filesystem::path &dst, Manifest &mf,
                               bool overwrite) {
    std::error_code ec;
...
class PlatformWin : public Platform {
  public:
    ...
    std::string elevation_hint() const override {
        return "Re-ejecuta este comando desde una consola \"Run as "
               "Administrator\".";
    }
    ...
    std::filesystem::path
    default_bin_dir(const std::filesystem::path &p) const override {
        return p; // En Windows el binario y los assets viven juntos
    }
    std::filesystem::path
    default_share_dir(const std::filesystem::path &p) const override {
        return p;
    }
    ...
    bool copy_files(const InstallOptions &opts, Manifest &mf) override {
        ...
        // 2) assets relativos al binario actual: docs/, examples_codes_vsh/,
        // icons/
        //
        ...
        if (opts.copy_stdlib) {
            ...
            for (const auto &ps : plugins) {
                ...
                const std::filesystem::path candidates[] = {
                    ...
                    src_root / ps.file,
                    src_root.parent_path() / "stdlib" / "native" / ps.lib /
                        ps.file,
                    src_root.parent_path().parent_path() / "stdlib" / "native" /
                        ps.lib / ps.file,
                };
                bool copied = false;
                ...
                if (!copied) {
                    std::cerr
                        << "  [plugin] AVISO: " << ps.file
                        << " no encontrado en ninguna ubicacion conocida; "
                        << "los programas que lo requieran fallaran al "
                           "cargar.\n";
                }
            }
        }
        ...
        if (opts.copy_vscode_ext) {
            ...
            if (std::filesystem::exists(src)) {
                ...
                if (n > 0) {
                    ...
                    auto vsdir = home / ".vscode" / "extensions" /
                                 "desmonhak.vesta-shell-0.1.0";
    ...
    // ---- asociaciones ----
    bool register_associations(const InstallOptions &opts,
                               Manifest &mf) override {
        HKEY root = (opts.scope == Scope::SystemWide) ? HKEY_LOCAL_MACHINE
                                                      : HKEY_CURRENT_USER;
        std::string hive_name =
            (opts.scope == Scope::SystemWide) ? "HKLM" : "HKCU";
        ...
        for (auto &[ext, ao] : opts.associations) {
            ...
            if (!ao.command_template.empty()) {
                std::string ckey =
                    class_key + "\\shell\\" + ao.verb_open + "\\command";
                std::string cdata =
                    "\"" + exe.string() + "\" " + ao.command_template;
    ...
    bool create_shortcuts(const InstallOptions &opts, Manifest &mf) override {
        ...
        auto make_lnk = [&](const std::filesystem::path &lnk_path,
            ...
            auto icon = opts.prefix / "icono.ico";
            if (std::filesystem::exists(icon))
                sl->SetIconLocation(icon.string().c_str(), 0);

            IPersistFile *pf = nullptr;
            hr = sl->QueryInterface(IID_IPersistFile, (void **)&pf);
            bool ok = false;
            if (SUCCEEDED(hr)) {
                // Convertir la ruta UTF-8 a UTF-16 correctamente con la API
                // Win32. No vale `std::wstring(s.begin(), s.end())`: eso solo
                // promueve bytes a wchar_t y rompe ante cualquier caracter
                // no-ASCII; ademas el patron `path.string().begin() ...
                // path.string().end()` da UB porque cada llamada a string()
                // produce un temporal distinto.
                std::string narrow = lnk_path.string();
                std::wstring wp;
                int wlen = MultiByteToWideChar(CP_UTF8, 0, narrow.c_str(),
                                               (int)narrow.size(), nullptr, 0);
                if (wlen > 0) {
                    wp.resize((size_t)wlen);
                    MultiByteToWideChar(CP_UTF8, 0, narrow.c_str(),
                                        (int)narrow.size(), wp.data(), wlen);
                }
        ...
        if (opts.create_start_menu) {
            ...
            if (SHGetFolderPathA(nullptr, csidl, nullptr, 0, buf) == S_OK) {
                std::filesystem::path dir =
                    std::filesystem::path(buf) / "Vesta VM";
                std::error_code ec;
        ...
        if (opts.create_desktop_shortcut) {
            ...
            if (SHGetFolderPathA(nullptr, csidl, nullptr, 0, buf) == S_OK) {
                std::filesystem::path lnk =
                    std::filesystem::path(buf) / "Vesta VM.lnk";
    ...
    bool register_uninstaller(const InstallOptions &opts,
                              Manifest &mf) override {
        HKEY root = (opts.scope == Scope::SystemWide) ? HKEY_LOCAL_MACHINE
                                                      : HKEY_CURRENT_USER;
        std::string hive_name =
            (opts.scope == Scope::SystemWide) ? "HKLM" : "HKCU";
        std::string key =
            "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\VestaVM";
        auto exe = opts.prefix / "vm.exe";
        ...
        mf.registry.push_back(
            {hive_name, key, "DisplayVersion", mf.vesta_version});
        ...
        mf.registry.push_back(
            {hive_name, key, "InstallLocation", opts.prefix.string()});
        ...
        if (std::filesystem::exists(icon)) {
            ...
            mf.registry.push_back(
                {hive_name, key, "DisplayIcon", icon.string()});
        }
