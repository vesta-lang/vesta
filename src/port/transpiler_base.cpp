...
namespace port {
    ...
    bool parse_gc_mode(const std::string &s, GcMode &out) {
        if (s == "none")  { out = GcMode::None;  return true; }
        if (s == "vesta") { out = GcMode::Vesta; return true; }
        if (s == "boehm") { out = GcMode::Boehm; return true; }
        return false;
    }

    bool parse_exc_mode(const std::string &s, ExcMode &out) {
        if (s == "none")       { out = ExcMode::None;       return true; }
        if (s == "setjmp")     { out = ExcMode::SetJmp;     return true; }
        if (s == "returncode") { out = ExcMode::ReturnCode; return true; }
        return false;
    }

    bool parse_type_style(const std::string &s, TypeStyle &out) {
        if (s == "stdint")  { out = TypeStyle::StdInt;  return true; }
        if (s == "builtin") { out = TypeStyle::Builtin; return true; }
        return false;
    }
    ...
    void IPortBackend::emit_cond_branch(EmitContext &ctx,
                                        ir::IrValueId cond,
    ...
    void IPortBackend::emit_unsupported(EmitContext &ctx, const ir::IrInstr &instr) {
    ...
    void IPortBackend::emit_phi_copy(EmitContext &ctx,
                                     ir::IrValueId dst,
                                     ir::IrValueId src,
                                     ir::IrType t) {
    ...
    void Transpiler::emit_block(EmitContext &ctx, const ir::IrBlock &bb,
        ...
        for (const auto &ins : bb.instrs) {
            ...
            switch (ins.op) {
                ...
                case ir::IrOp::RET: {
                    backend_.emit_return(ctx,
                        ins.operands.empty() ? ir::IR_NO_VALUE : ins.operands[0]);
                    break;
                }
    ...
    bool Transpiler::block_has_phis(const ir::IrFunction &fn, ir::IrBlockId bid) const {
        ...
        switch (ins.op) {
            ...
            case IrOp::FDIV: case IrOp::FMIN: case IrOp::FMAX:
                if (ins.operands.size() >= 2) {
                    backend_.emit_binop(ctx, ins.op, ins.dst,
            ...
            case IrOp::FNEG: case IrOp::FABS: case IrOp::FSQRT:
                if (!ins.operands.empty()) {
                    backend_.emit_unop(ctx, ins.op, ins.dst,
                                       ins.operands[0], ins.type);
                }
                return;
            ...
            case IrOp::FCMP_LE: case IrOp::FCMP_GE: {
                if (ins.operands.size() >= 2) {
                    ...
                    backend_.emit_cmp(ctx, ins.op, ins.dst,
                                      ins.operands[0], ins.operands[1], op_t);
                }
                return;
            }
            ...
            // Memoria
            case IrOp::ALLOCA:
                backend_.emit_alloca(ctx, ins.dst, ins.imm);
                return;
            ...
            case IrOp::LOAD: {
                if (!ins.operands.empty() && ctx.fn) {
                    ...
                    backend_.emit_load(ctx, ins.dst, ins.operands[0],
                                       ins.type, host);
                }
                return;
            }
            ...
            case IrOp::STORE: {
                if (ins.operands.size() >= 2 && ctx.fn) {
                    ...
                    backend_.emit_store(ctx, ins.operands[0],
                                        ins.operands[1], ins.type, host);
                }
                return;
            }
    ...
    static bool block_ends_with_br_to(const ir::IrFunction &fn,
                                      ir::IrBlockId bid, ir::IrBlockId target) {
    ...
    static bool block_br_target(const ir::IrFunction &fn,
                                ir::IrBlockId bid,
                                ir::IrBlockId &out) {
        if (bid >= fn.blocks.size()) return false;
        const auto &b = fn.blocks[bid];
    ...
    void Transpiler::emit_region(EmitContext &ctx, const ir::IrFunction &fn,
        ...
        while (cur != ir::IR_NO_BLOCK && cur != stop) {
            ...
            if (info.kind == StructureInfo::SIMPLE_LOOP) {
                ...
                const auto &term = bb.instrs.back();
                ir::IrValueId cond_id = term.operands.empty()
                    ? ir::IR_NO_VALUE
                    : term.operands[0];
            ...
            if (info.kind == StructureInfo::IF_THEN
             || info.kind == StructureInfo::IF_THEN_ELSE) {
                ...
                // Las emitimos DENTRO del if {} para que solo corran si la cond es true.
                ctx.indent();
                ctx.out << "if (" << cond_expr << ") {\n";
                ctx.indent_level++;
                ...
                if (info.kind == StructureInfo::IF_THEN_ELSE) {
                    ...
                    emit_region(ctx, fn, info.else_block, info.merge_block, visited);
                    ctx.indent_level--;
                    ctx.indent();
                    ctx.out << "}\n";
                } else {
            ...
            switch (term.op) {
                ...
                case ir::IrOp::BR_COND: {
                    ...
                    if (term.false_block < fn.blocks.size()
                     && !visited[term.false_block]
                     && term.false_block != stop) {
