...
namespace vex {
    ...
    std::string asm_canonical_reg(const std::string &raw) {
        std::string r;
        r.reserve(raw.size());
        for (char c : raw) r.push_back((char) std::tolower((unsigned char) c));
        // Registros de proposito general: cada entrada lista todos los alias
        // de ancho que mapean al mismo fisico de 64 bits.
        struct GpEntry { const char *canon; const char *aliases[6]; };
        ...
        if (r.size() >= 2 && r[0] == 'r' && std::isdigit((unsigned char) r[1])) {
            std::string num;
            size_t i = 1;
            while (i < r.size() && std::isdigit((unsigned char) r[i])) num.push_back(r[i++]);
            std::string suf = r.substr(i);
            if (!num.empty() && (suf.empty() || suf == "d" || suf == "w" || suf == "b")) {
        ...
        for (const char *pfx : {"xmm", "ymm", "zmm"}) {
            ...
            if (r.size() > plen && r.compare(0, plen, pfx) == 0) {
                ...
                for (char c : num) {
                    if (!std::isdigit((unsigned char) c)) { all_dig = false; break; }
                }
                if (all_dig) {
                    int n = std::atoi(num.c_str());
    ...
    AsmEffects asm_effects_for(const std::string &mnemonic) {
        static const std::unordered_map<std::string, AsmEffects> table = [] {
            ...
            auto E = [](std::initializer_list<const char *> wr,
                        bool wfo, bool mem, bool flags, bool call = false) {
                AsmEffects e;
                for (const char *w : wr) e.implicit_write.emplace_back(w);
            ...
            add("imul", E({"rax", "rdx"}, false, false, true));  // forma 1-op (conservador)
            ...
            add("xchg",   E({}, true, false, false));  // escribe ambos (conservador: 1er op)
        ...
        for (char c : mnemonic) m.push_back((char) std::tolower((unsigned char) c));
        auto it = table.find(m);
        if (it != table.end()) return it->second;
        // setcc (sete/setne/...) y cmovcc se reconocen por prefijo.
        if (m.rfind("set", 0) == 0 && m.size() > 3) {
            AsmEffects e; e.writes_first_operand = true; e.known = true; return e;
        }
    ...
    namespace {
        ...
        std::string canon_to_gcc(const std::string &canon) {
            if (!canon.empty() && canon[0] == 'v'
                && canon.size() > 1 && std::isdigit((unsigned char) canon[1])) {
    ...
    AsmInferResult asm_infer_clobbers(const std::string &nasm_body,
        ...
        std::unordered_set<std::string> bound(bound_canon.begin(), bound_canon.end());
        ...
        static const char *caller_saved[] = {
        ...
        while (i <= b.size()) {
            ...
            while (ti < toks.size()) {
                ...
                if (!t.empty() && t.back() == ':') { ++ti; continue; }
                // Prefijos de instruccion.
                std::string lt;
                for (char c : t) lt.push_back((char) std::tolower((unsigned char) c));
                if (lt == "lock" || lt == "rep" || lt == "repe" || lt == "repz"
                    || lt == "repne" || lt == "repnz" || lt == "bnd") { ++ti; continue; }
                break;
            }
            ...
            for (const auto &w : eff.implicit_write) clob.insert(w);
            ...
            if (eff.is_call) {
                for (const char *cs : caller_saved) clob.insert(cs);
            }
        }
