...
namespace vex {
    ...
    namespace {
         ...
         * Los caracteres `{`, `}`, `|`, `<`, `>` son sintaxis del record y
         * deben ir escapados con `\` cuando aparecen como contenido literal.
         */
        ...
        const NodeStyle ST_MODULE     = { "box3d",        "#1e3a8a", "#ffffff", "#1e3a8a", "filled,bold",      2 };
        const NodeStyle ST_FUNC       = { "box",          "#15803d", "#ffffff", "#166534", "filled,rounded",   1 };
        const NodeStyle ST_CLASS      = { "box",          "#7e22ce", "#ffffff", "#581c87", "filled,rounded",   1 };
        const NodeStyle ST_STRUCT     = { "box",          "#b45309", "#ffffff", "#78350f", "filled,rounded",   1 };
        const NodeStyle ST_ENUM       = { "box",          "#0f766e", "#ffffff", "#134e4a", "filled,rounded",   1 };
        const NodeStyle ST_GLOBAL     = { "box",          "#475569", "#ffffff", "#1e293b", "filled,rounded",   1 };
        const NodeStyle ST_EXTERN     = { "box",          "#9d174d", "#ffffff", "#831843", "filled,rounded",   1 };
        const NodeStyle ST_PARAM      = { "ellipse",      "#dcfce7", "#15803d", "#166534", "filled",           1 };
        const NodeStyle ST_FIELD      = { "note",         "#fef3c7", "#92400e", "#78350f", "filled",           1 };
        const NodeStyle ST_METHOD     = { "box",          "#f3e8ff", "#7e22ce", "#581c87", "filled,rounded",   1 };
        const NodeStyle ST_VARIANT    = { "ellipse",      "#ccfbf1", "#0f766e", "#134e4a", "filled",           1 };
        const NodeStyle ST_EXTREF     = { "box",          "#fce7f3", "#9d174d", "#831843", "filled,dashed",    1 };
        // Stmts del control flow
        const NodeStyle ST_ENTRY      = { "circle",       "#1e293b", "#ffffff", "#0f172a", "filled,bold",      2 };
        const NodeStyle ST_VAR        = { "box",          "#fef3c7", "#92400e", "#92400e", "filled,rounded",   1 };
        const NodeStyle ST_EXPR       = { "box",          "#e0e7ff", "#3730a3", "#4338ca", "filled,rounded",   1 };
        const NodeStyle ST_CALL       = { "box",          "#dbeafe", "#1e40af", "#1d4ed8", "filled,rounded,bold", 2 };
        const NodeStyle ST_ASSIGN     = { "box",          "#fef3c7", "#854d0e", "#a16207", "filled,rounded",   1 };
        const NodeStyle ST_SPAWN      = { "doubleoctagon","#fae8ff", "#86198f", "#86198f", "filled,bold",      2 };
        const NodeStyle ST_IF         = { "diamond",      "#fef9c3", "#713f12", "#a16207", "filled,bold",      2 };
        const NodeStyle ST_LOOP       = { "diamond",      "#fed7aa", "#9a3412", "#c2410c", "filled,bold",      2 };
        const NodeStyle ST_LOOP_BODY  = { "box",          "#fed7aa", "#9a3412", "#c2410c", "filled,rounded",   1 };
        const NodeStyle ST_TERMINAL   = { "doublecircle", "#dcfce7", "#166534", "#15803d", "filled,bold",      2 };
        const NodeStyle ST_THROW      = { "doublecircle", "#fecaca", "#991b1b", "#b91c1c", "filled,bold",      2 };
        const NodeStyle ST_BRANCH     = { "octagon",      "#e9d5ff", "#6b21a8", "#7e22ce", "filled",           1 };
        const NodeStyle ST_LABEL      = { "tab",          "#cffafe", "#155e75", "#0e7490", "filled",           1 };
        const NodeStyle ST_TRY        = { "box",          "#fde68a", "#92400e", "#a16207", "filled,rounded,bold", 2 };
        const NodeStyle ST_CATCH      = { "box",          "#fca5a5", "#7f1d1d", "#991b1b", "filled,rounded",   1 };
        const NodeStyle ST_FINALLY    = { "box",          "#a7f3d0", "#065f46", "#047857", "filled,rounded",   1 };
        const NodeStyle ST_SYNC       = { "box",          "#fbcfe8", "#831843", "#9d174d", "filled,rounded,bold", 2 };
        const NodeStyle ST_AUX        = { "box",          "#f1f5f9", "#475569", "#94a3b8", "filled,dashed,rounded", 1 };
        // IR blocks
        const NodeStyle ST_IRBLOCK    = { "box",          "#dbeafe", "#1e3a8a", "#1e40af", "filled,rounded",   1 };
        const NodeStyle ST_IRBRANCH   = { "box",          "#fef9c3", "#854d0e", "#a16207", "filled,rounded",   1 };
        const NodeStyle ST_IREXIT     = { "box",          "#fee2e2", "#7f1d1d", "#991b1b", "filled,rounded,bold", 2 };
        // VEL blocks
        const NodeStyle ST_VELBLOCK   = { "box",          "#e0f2fe", "#075985", "#0369a1", "filled,rounded",   1 };
        const NodeStyle ST_VELEXIT    = { "box",          "#fee2e2", "#7f1d1d", "#991b1b", "filled,rounded,bold", 2 };
        const NodeStyle ST_VELOPT     = { "box",          "#fef3c7", "#92400e", "#92400e", "filled,rounded,bold", 2 };
        ...
        void emit_edge(std::ostringstream &os, const std::string &from,
            ...
            switch (kind) {
                ...
                case EdgeKind::Bold:   attrs.push_back("style=\"bold\""); attrs.push_back("penwidth=2"); break;
        ...
        void render_function_decl(std::ostringstream &os, const ast::FunctionDecl *fd,
            ...
            std::string tooltip = title + "\nFile: line " + std::to_string(fd->loc.line)
                                + "\n" + body_info;
            ...
            if (fd->body) {
                ...
                render_function_body_subgraph(os, body_cluster, body_title, fd->body.get());
        ...
        void render_class_decl(std::ostringstream &os, const ast::ClassDecl *cd,
            ...
            if (!cd->super_name.empty()) {
                ...
                emit_node(os, sid, cd->super_name, ST_EXTREF, "extends " + cd->super_name);
            ...
            for (size_t mi = 0; mi < cd->methods.size(); ++mi) {
                ...
                sig += ")";
                std::string body_info = m->body
                                ? (std::to_string(count_stmts(m->body.get())) + " stmts")
                                : std::string("(abstract)");
                std::string tooltip2 = sig + "\n" + body_info
                ...
                if (m->body) {
                    ...
                    render_function_body_subgraph(os, body_cluster, body_title, m->body.get());
                    emit_edge(os, mid, body_cluster + "_entry", "body", EdgeKind::Dotted);
                }
            }
        }
                ...
                case ast::NodeKind::ExprStmt: {
                    ...
                    if (expr->kind == ast::NodeKind::CallExpr
                     || expr->kind == ast::NodeKind::NewExpr) {
                        r.group = "call";   r.style = &ST_CALL;
                    } else if (expr->kind == ast::NodeKind::SpawnExpr
                            || expr->kind == ast::NodeKind::RSpawnExpr) {
                        r.group = "spawn";  r.style = &ST_SPAWN;
                    } else if (expr->kind == ast::NodeKind::AssignExpr) {
                        r.group = "assign"; r.style = &ST_ASSIGN;
                    } else {
                        r.group = "expr";   r.style = &ST_EXPR;
                    }
                    return r;
                }
                default:
        ...
        std::string render_stmt(StmtCtx &ctx, const ast::Stmt *s) {
            ...
            switch (s->kind) {
                ...
                case ast::NodeKind::IfStmt: {
                    ...
                    std::string cond = emit_stmt_node(ctx, "if",
                        "if " + fmt_expr(iss->cond.get()), ST_IF,
                        "L" + std::to_string(iss->loc.line));
                    std::string merge = emit_stmt_node(ctx, "ifend",
                        "(merge)", ST_AUX);
                ...
                case ast::NodeKind::WhileStmt: {
                    ...
                    std::string head = emit_stmt_node(ctx, "while",
                        "while " + fmt_expr(ws->cond.get()), ST_LOOP,
                        "L" + std::to_string(ws->loc.line));
                    std::string exit_id = emit_stmt_node(ctx, "wend",
                        "(loop exit)", ST_AUX);
                ...
                case ast::NodeKind::DoWhileStmt: {
                    ...
                    std::string entry = emit_stmt_node(ctx, "doentry",
                        "do {", ST_LOOP_BODY);
                    std::string cond = emit_stmt_node(ctx, "dowhile",
                        "while " + fmt_expr(dw->cond.get()), ST_LOOP,
                        "L" + std::to_string(dw->loc.line));
                    std::string exit_id = emit_stmt_node(ctx, "dwend",
                        "(loop exit)", ST_AUX);
                ...
                case ast::NodeKind::ForStmt: {
                    ...
                    std::string head = emit_stmt_node(ctx, "for",
                        fs->cond ? ("for " + fmt_expr(fs->cond.get())) : "for (true)",
                        ST_LOOP, "L" + std::to_string(fs->loc.line));
                    std::string step_id;
                    if (fs->step) {
                        step_id = emit_stmt_node(ctx, "step",
                            "step: " + fmt_expr(fs->step.get()), ST_LOOP_BODY);
                    }
                    std::string exit_id = emit_stmt_node(ctx, "fend",
                        "(loop exit)", ST_AUX);
                    ...
                    if (!bx.empty()) {
                        ...
                        emit_edge(ctx.os, head, step_id, "step (skipped)", EdgeKind::Dotted);
                        emit_edge(ctx.os, step_id, head, "back", EdgeKind::Bold);
                    }
                ...
                case ast::NodeKind::BreakStmt: {
                    ...
                    if (!ctx.loop_stack.empty()) {
                        emit_edge(ctx.os, n, ctx.loop_stack.back().break_target,
                                    "break", EdgeKind::Dashed);
                    }
                    return n + "|";
                }
                ...
                case ast::NodeKind::GotoStmt: {
                    ...
                    std::string n = emit_stmt_node(ctx, "goto",
                        "goto " + gs->label, ST_BRANCH);
                ...
                case ast::NodeKind::LabelStmt: {
                    ...
                    std::string n = emit_stmt_node(ctx, "lbl",
                        ls->name + ":", ST_LABEL);
                ...
                case ast::NodeKind::TryStmt: {
                    ...
                    std::string merge = emit_stmt_node(ctx, "tryend", "(after try)", ST_AUX);
                    ...
                    for (size_t ci = 0; ci < ts->catches.size(); ++ci) {
                        ...
                        std::string cls = c.exc_class_name.empty()
                                            ...
                                            : c.exc_class_name;
                        std::string handler = emit_stmt_node(ctx, "catch",
                            "catch " + cls
                        ...
                        emit_edge(ctx.os, be, handler, "throw " + cls,
                                    EdgeKind::Dotted, "#b91c1c");
                    ...
                    if (ts->finally_body) {
                        ...
                        std::string fnode = emit_stmt_node(ctx, "finally",
                            "finally", ST_FINALLY);
                        emit_edge(ctx.os, merge, fnode);
                        emit_edge(ctx.os, fnode, fe);
                        std::string after = emit_stmt_node(ctx, "tryafter",
                            "(after finally)", ST_AUX);
                ...
                case ast::NodeKind::SynchronizedStmt: {
                    ...
                    std::string entry = emit_stmt_node(ctx, "sync",
                    ...
                    std::string exit_id = emit_stmt_node(ctx, "syncend",
                        "monexit", ST_SYNC);
                    emit_edge(ctx.os, entry, be);
                ...
                default: {
                    return emit_stmt_node(ctx, "stmt",
        ...
        void render_function_body_subgraph(std::ostringstream &os,
            ...
            if (!body) {
                os << "        " << cluster_id << "_abstract [label=\"(metodo abstracto)\","
        ...
        void render_ir_block(std::ostringstream &os,
            ...
            lbl << "{ " << escape_record(bb.name + " (#" + std::to_string(bb.id)
        ...
        void render_ir_function(std::ostringstream &os,
            ...
            std::string title = fn.name + " ("
                              + std::to_string(fn.blocks.size()) + " bb, "
                              + std::to_string(fn.params.size()) + " params, ret "
                              + ir::ir_type_name(fn.ret_type) + ")";
    ...
    std::string graphviz_from_ast(const ast::ModuleNode &mod) {
        ...
        os << "//   funciones: nombre, return type, params, body con control flow\n";
        ...
        std::string root = "M";
        emit_node(os, root,
                    "Module\n" + std::to_string(mod.decls.size()) + " top-level decls",
                    ST_MODULE,
    ...
    std::string graphviz_from_ir_module(const ir::IrModule &mod,
        ...
        os << "// Cada cluster = una funcion.  Cada nodo = un IrBlock con sus instrs SSA.\n";
        os << "// Edges:\n";
        ...
        for (const auto &edge : intra_calls) {
            ...
            os << "    " << src_fn << "_b0 -> " << it->second << "_b0"
               << " [ltail=cluster_" << src_fn
               << ", lhead=cluster_" << it->second
    ...
    std::string graphviz_from_vel_text(const std::string &vel_text) {
        ...
        os << "// Tooltips contienen TODAS las instrucciones (no solo las primeras 10).\n";
        os << "// Edges:\n";
        ...
        os << "// Opcodes destacados (cmpjmp/cmpjmpu/decjnz/gcallocp/spawnargs/fulfillhlt)\n";
