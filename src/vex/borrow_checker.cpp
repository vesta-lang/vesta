...
namespace vex {
    ...
    OwnerKind BorrowChecker::owner_kind_of(const std::string &owner_name) const noexcept {
        auto it = owners_.find(owner_name);
        if (it == owners_.end()) return OwnerKind::Local;  // defensivo
        return it->second.owner_kind;
    }

    std::string BorrowChecker::root_owner_of(const std::string &borrower_name) const {
    ...
    bool BorrowChecker::suspend_for_reborrow(const std::string &source_borrower_name) {
    ...
    void BorrowChecker::on_borrow_drop(const std::string &borrower_name,
        ...
        if (!reborrow_source.empty()
         && !rec.suspend_stack.empty()
         && rec.kind == BorrowKind::None) {
    ...
    bool BorrowChecker::on_owner_use(const std::string &owner_name,
        ...
        if (!is_mutation) {
            if (rec.kind == BorrowKind::Mutable) {
                error_use_while_borrowed(loc_use, owner_name, rec, /*is_mutation=*/false);
                return false;
            }
    ...
    bool BorrowChecker::on_borrow_escape(const std::string &borrower_name,
        ...
        diags_.error(loc_escape,
            ...
            + "  " + escape_kind + " del borrow lo expone fuera del scope donde el local existe.");
        diags_.note(loc_escape,
            std::string("considera cambiar el tipo a `unique<T>` o `shared<T>` para transferir/clonar la posesion en lugar de prestar,\n")
    ...
    void BorrowChecker::error_aliasing(SourceLoc           loc_conflict,
        ...
        std::string note_msg = std::string("prestamo previo (") + kind_label(rec.kind) + ")";
        if (!rec.borrower_name.empty()) {
            note_msg += " por '" + rec.borrower_name + "'";
        }
        note_msg += " tomado aqui";
        diags_.note(rec.loc_taken, note_msg);
        diags_.note(loc_conflict,
            std::string("se intenta tomar prestamo ") +
            (trying_mut ? "mutable" : "shared") + " aqui");
    }
    ...
    void BorrowChecker::error_use_while_borrowed(SourceLoc           loc_use,
        ...
        diags_.note(rec.loc_taken,
            std::string("prestamo ") + kind_label(rec.kind) + " por '" +
            rec.borrower_name + "' tomado aqui");
        diags_.note(loc_use,
            std::string("conflicto: ") +
            (is_mutation ? "mutacion" : "lectura") + " del owner aqui");
    }

    void BorrowChecker::error_move_while_borrowed(SourceLoc           loc_move,
                                                  const std::string  &owner_name,
                                                  const BorrowRecord &rec) {
        diags_.error(loc_move,
            std::string("error: no se puede mover '") + owner_name +
            "' porque tiene un prestamo " + kind_label(rec.kind) + " activo");
        std::string note_msg = std::string("prestamo ") + kind_label(rec.kind);
        if (!rec.borrower_name.empty()) note_msg += " por '" + rec.borrower_name + "'";
        note_msg += " tomado aqui";
        diags_.note(rec.loc_taken, note_msg);
        diags_.note(loc_move, "move(...) del owner aqui");
        diags_.note(loc_move,
