...
static void emit_typedef_base(std::vector<uint8_t> &payload,
                              StringPoolBuilder &pool, const VexiSymbol &sym) {
...
std::vector<uint8_t> vexi_emit(const VexiModule &mod) {
    ...
    const uint64_t abi_hash =
        vexi_fnv1a(out.data() + HEADER_BYTES, out.size() - HEADER_BYTES);
...
VexiParseResult vexi_parse(const uint8_t *data, size_t size) {
    ...
    if (cvh != my_cvh) {
        std::ostringstream os;
        os << ".vexi generado por compilador distinto (esperado 0x" << std::hex
...
uint64_t vexi_compiler_version_hash() noexcept {
    ...
    static const uint64_t cached =
        vexi_fnv1a(kIdentity, std::strlen(kIdentity));
    return cached;
}
