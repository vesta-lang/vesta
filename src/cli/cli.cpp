...
namespace cli {
    ...
    /**
     * @brief Expande variables de entorno ($VAR) y "~" (HOME) en una cadena de ruta.
     *
     * Orden de expansion:
     *   1. Sustituye cada token $VAR o ${VAR} por el valor de la variable de entorno.
     *   2. Si la cadena resultante empieza por '~', la sustituye por HOME/USERPROFILE.
     *
    ...
    static std::filesystem::path expand_path(const std::string &p) {
        ...
        if (!expanded.empty() && expanded[0] == '~') {
            ...
            if (!home_s.empty())        return std::filesystem::path(home_s) / expanded.substr(2);
     ...
     */
    static distrib::NodeAuthConfig dist_build_auth(
    ...
    static void dist_print_help() {
        auto H = [](const char *s) {
            vesta::scout() << ansi::c(ansi::BOLD) << ansi::c(ansi::BR_CYAN)
                           << s << ansi::c(ansi::RESET) << "\n";
        };
        auto C = [](const char *usage, const char *desc) {
            vesta::scout() << "  " << ansi::c(ansi::BR_YELLOW)
                           << std::left << std::setw(52) << usage
                           << ansi::c(ansi::RESET) << " " << desc << "\n";
        };
    ...
    static void command_dist(const std::string &args) {
        ...
        // opciones de autenticacion y vm-id (comunes a todos los subcomandos con mgr_id)
        ...
        if (subcmd == "new") {
            ...
            for (size_t i = 1; i < words.size(); ++i) {
                if (words[i] == "--port"          && i+1 < words.size()) { try { port      = (uint16_t)std::stoul(words[++i]); } catch(...){} }
                if (words[i] == "--discover")                              discover  = true;
                if (words[i] == "--discover-port" && i+1 < words.size()) { try { disc_port = (uint16_t)std::stoul(words[++i]); } catch(...){} }
                if (words[i] == "--name"          && i+1 < words.size()) name       = words[++i];
                if (words[i] == "--node-id"       && i+1 < words.size()) { try { node_id_v = std::stoull(words[++i]);          } catch(...){} }
                if (words[i] == "--schedulers"    && i+1 < words.size()) { try { num_scheds= std::stoul(words[++i]);            } catch(...){} }
                if (words[i] == "--token"         && i+1 < words.size()) token_str  = words[++i];
            ...
            cfg.server_auth = dist_build_auth(token_str, use_tls, cert_str, key_str, ca_str);

            vm->dist_runtime = std::make_unique<distrib::DistRuntime>(*vm, cfg);
            bool ok = vm->dist_runtime->start();

            // arrancar schedulers en modo persistente para ejecutar procesos remotos (rspawn)
            ...
            vesta::scout() << oss.str() << "\n"
                           ...
                           << "  dist add-node " << new_id << " <ip> <p>  -> conectar nodo\n"
                           << "  dist stop "     << new_id << "           -> detener servidor VDP\n"
        ...
        // parsear mgr_id
        size_t mgr_id = 0;
        try { mgr_id = std::stoul(words[1]); }
        catch (...) {
            vesta::scout() << "[dist] mgr_id debe ser un numero entero\n"; return;
        }

        for (size_t i = 2; i < words.size(); ++i) {
            if (words[i] == "--vm-id"  && i+1 < words.size()) { try { vm_id = std::stoull(words[++i]); } catch(...) {} }
            if (words[i] == "--token"  && i+1 < words.size()) token_str = words[++i];
            if (words[i] == "--tls")                           use_tls = true;
            if (words[i] == "--cert"   && i+1 < words.size()) cert_str  = words[++i];
            if (words[i] == "--key"    && i+1 < words.size()) key_str   = words[++i];
        ...
        // ---- dist start ----
        // dist start <mgr_id> [--vm-id N] [--port P] [--discover] [--discover-port P]
        ...
        if (subcmd == "start") {
            ...
            for (size_t i = 2; i < words.size(); ++i) {
                ...
                if (words[i] == "--discover-port" && i+1 < words.size()) { try { disc_port = (uint16_t)std::stoul(words[++i]); } catch(...){} }
                if (words[i] == "--name"          && i+1 < words.size()) name       = words[++i];
                if (words[i] == "--node-id"       && i+1 < words.size()) { try { node_id   = std::stoull(words[++i]);          } catch(...){} }
            }
            ...
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                runtime::VM *vm = m->get_vm(vm_id);
                if (!vm) { msg = "VM " + std::to_string(vm_id) + " no encontrada en mgr " + std::to_string(mgr_id); return; }
                ...
                if (ok) {
                    ...
                    if (discover) msg += " + descubrimiento UDP (:" + std::to_string(disc_port) + ")";
                } else {
                    msg = "Error al iniciar DistRuntime (verificar puerto o certificados)";
                }
            });
            if (!found) vesta::scout() << "[dist] Manager " << mgr_id << " no encontrado\n";
            else        vesta::scout() << "[dist] " << msg << "\n";
            return;
        }
        ...
        if (subcmd == "stop") {
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                ...
                if (!vm || !vm->dist_runtime) {
                    vesta::scout() << "[dist] dist_runtime no disponible en mgr=" << mgr_id
                                   << " vm=" << vm_id << "\n"; return;
                }
        ...
        if (subcmd == "info") {
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                ...
                ss << ansi::c(ansi::BOLD) << ansi::c(ansi::BR_CYAN)
                   ...
                   << " ===" << ansi::c(ansi::RESET) << "\n"
                   << "  Node ID local : 0x" << std::hex << std::setw(16) << std::setfill('0')
                   << dr->local_node_id() << std::dec << std::setfill(' ') << "\n"
                ...
                for (uint32_t i = 0; i < (uint32_t)n; ++i) {
                    auto *ni = reg.get(i);
                    if (!ni) continue;
                    const char *col = (ni->state == distrib::NodeState::AUTHENTICATED)
                        ? ansi::c(ansi::BR_GREEN) : ansi::c(ansi::DIM);
                    ss << "    [" << i << "] " << col
                       << ni->ip << ":" << ni->port
        ...
        if (subcmd == "list" || subcmd == "nodes") {
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                ...
                size_t n  = reg.count();
                if (n == 0) { vesta::scout() << "[dist] Sin nodos registrados\n"; return; }

                std::ostringstream ss;
                ss << ansi::c(ansi::BOLD)
                   << std::left
                   << std::setw(5)  << "IDX"
                   << std::setw(22) << "IP:PUERTO"
                   << std::setw(18) << "NOMBRE"
                   << std::setw(15) << "ESTADO"
                   << "TIPO"
                   << ansi::c(ansi::RESET) << "\n"
                ...
                for (uint32_t i = 0; i < (uint32_t)n; ++i) {
                    ...
                    const char *col = (ni->state == distrib::NodeState::AUTHENTICATED)
                        ? ansi::c(ansi::BR_GREEN) : ansi::c(ansi::DIM);
                    std::string addr = std::string(ni->ip) + ":" + std::to_string(ni->port);
                    ss << col
                       << std::left
                       << std::setw(5)  << ni->idx
                       << std::setw(22) << addr
        ...
        if (subcmd == "add-node") {
            ...
            try { node_port = (uint16_t)std::stoul(words[3]); }
            catch (...) { vesta::scout() << "[dist] Puerto invalido: " << words[3] << "\n"; return; }
            for (size_t i = 4; i < words.size(); ++i)
                if (words[i] == "--name" && i+1 < words.size()) node_name = words[++i];

            distrib::NodeAuthConfig auth = dist_build_auth(token_str, use_tls, cert_str, key_str, ca_str);

            std::string msg;
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                runtime::VM *vm = m->get_vm(vm_id);
                if (!vm || !vm->dist_runtime) { msg = "dist_runtime no disponible"; return; }
                uint32_t idx = vm->dist_runtime->add_node(
                    node_ip.c_str(), node_port, auth,
                    node_name.empty() ? node_ip.c_str() : node_name.c_str());
                msg = "Nodo registrado: " + node_ip + ":" + std::to_string(node_port)
        ...
        if (subcmd == "connect") {
            if (words.size() < 3) {
                vesta::scout() << "Uso: dist connect <mgr_id> <node_idx> [--vm-id N]\n"; return;
            }
            uint32_t node_idx = 0;
            try { node_idx = (uint32_t)std::stoul(words[2]); }
            catch (...) { vesta::scout() << "[dist] node_idx invalido: " << words[2] << "\n"; return; }
            ...
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                ...
                msg = ok ? "Conectado a nodo idx=" + std::to_string(node_idx)
                         : "Fallo al conectar con nodo idx=" + std::to_string(node_idx);
            });
            if (!found) vesta::scout() << "[dist] Manager " << mgr_id << " no encontrado\n";
            else        vesta::scout() << "[dist] " << msg << "\n";
            return;
        ...
        if (subcmd == "discover") {
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                ...
                // start_discovery() es idempotente (exchange atomico lo protege)
                ...
                // emitir un broadcast inmediato sin esperar los 30 segundos del ciclo
                vm->dist_runtime->registry().broadcast_discover();
                vesta::scout() << "[dist] Descubrimiento UDP activo (puerto " << dport
        ...
        if (subcmd == "new") {
            ...
            vesta::scout() << oss.str() << "\n"
                           << "  Usa 'dist info " << new_id << "' para ver el estado.\n"
                           << "  Usa 'dist add-node " << new_id << " <ip> <puerto>' para conectar nodos.\n"
        ...
        // Carga el .velb en la VM local, obtiene la direccion de entrada y envia el bytecode
        // al nodo remoto indicado por node_idx mediante rspawn (VDP_RSPAWN).
        // Devuelve el GcHandle del FutureObject (0xFFFFFFFF si error).
        if (subcmd == "run") {
            if (words.size() < 4) {
                vesta::scout() << "[dist run] Uso: dist run <mgr_id> <node_idx> <archivo.velb>"
            ...
            for (size_t i = 4; i < words.size(); ++i) {
                ...
                if (words[i] == "--timeout" && i+1 < words.size()) { try { timeout_ms = std::stoull(words[++i]); } catch(...){} }
            }

            // resolver ruta absoluta del archivo
            auto velb_path = std::filesystem::weakly_canonical(
                std::filesystem::current_path() / velb_arg);
            if (!std::filesystem::exists(velb_path) || !std::filesystem::is_regular_file(velb_path)) {
                vesta::scout() << "[dist run] Archivo no encontrado: " << velb_path.string() << "\n";
                return;
            }
            std::string abs_path = velb_path.string();

            // estas variables se rellenan dentro del lambda para usarlas en el wait externo
            std::string            run_result;
            runtime::ProcessVM    *wait_proc    = nullptr;
            uint32_t               wait_handle  = 0xFFFFFFFFu;
            // variables para limpiar el proceso temporal de carga tras recibir el ACK
            ...
            bool found = mgr.modify_manager(mgr_id, [&](std::unique_ptr<runtime::ManageVM> &m) {
                runtime::VM *vm = m->get_vm(vm_id);
                if (!vm) {
                    run_result = "[dist run] VM " + std::to_string(vm_id) + " no encontrada en manager " + std::to_string(mgr_id) + "\n";
                    return;
                }
                if (!vm->dist_runtime) {
                    run_result = "[dist run] dist_runtime no disponible en esta VM\n";
                    return;
                }
                ...
                if (ni->state != distrib::NodeState::AUTHENTICATED) {
                    run_result = "[dist run] Nodo " + std::to_string(node_idx)
                                 ...
                                 + " " + std::to_string(node_idx) + "' para reconectar.\n";
                    return;
                }
                ...
                std::vector<uint8_t> velb_bytes;
                {
                    ...
                    if (!vf.is_open()) {
                        run_result = "[dist run] No se puede abrir: " + abs_path + "\n";
                        return;
                    }
                    auto fsz = vf.tellg();
                    if (fsz <= 0) {
                        run_result = "[dist run] Fichero vacio o error de lectura: " + abs_path + "\n";
                        return;
                    }
                ...
                // crear un proceso temporal solo para que su gc_heap albergue el FutureObject
                runtime::ProcessVM *proc = nullptr;
                try {
                    proc = m->loader.load_executable(*vm, abs_path);
                } catch (const std::exception &e) {
                    run_result = std::string("[dist run] Error creando proceso temporal: ") + e.what() + "\n";
                    return;
                }
                if (!proc) {
                ...
                // enviar el .velb completo al nodo remoto via rspawn_velb (VDP_RSPAWN con RSPAWN_FLAG_VELB)
                uint32_t gc_handle = vm->dist_runtime->rspawn_velb(proc, velb_bytes, node_idx);

                if (gc_handle == 0xFFFFFFFFu) {
                    run_result = "[dist run] Error: rspawn fallo al enviar a nodo[" + std::to_string(node_idx) + "]\n"
                                 "  La sesion puede haberse cerrado; usa 'dist connect "
                                 + std::to_string(mgr_id) + " " + std::to_string(node_idx) + "' para reconectar.\n";
                    // proc fue creado por load_executable pero rspawn fallo; eliminar ahora
                    vm->schedulers[proc->pid.scheduler_id]->kill(proc->pid);
                    return;
                }

                // guardar info de limpieza: el proceso local solo sirvio para leer el bytecode
                cleanup_vm   = vm;
                cleanup_proc = proc;
                ...
                std::ostringstream oss;
                oss << "[dist run] Bytecode enviado a nodo[" << node_idx << "] desde " << velb_arg << "\n"
                    << "  FutureObject GcHandle = 0x" << std::hex << gc_handle << std::dec << "\n";
                if (wait_mode) {
                    oss << "  Esperando resultado (timeout " << timeout_ms << " ms)...\n";
                    wait_proc   = proc;
                    wait_handle = gc_handle;
                } else {
                    oss << "  Usa '--wait' para bloquear hasta recibir el resultado del nodo.\n";
                }
                run_result = oss.str();
            });

            if (!found) {
                vesta::scout() << "[dist run] Manager " << mgr_id << " no encontrado\n";
                return;
            }
            ...
            if (wait_mode && wait_proc && wait_handle != 0xFFFFFFFFu) {
                ...
                if (final_state == loader::FutureState::RESOLVED) {
                    oss << "[dist run] Resultado: " << final_result
                        << " (0x" << std::hex << final_result << std::dec << ")\n";
                    ...
                    oss << "[dist run] Timeout: el nodo no respondio en " << timeout_ms << " ms\n";
                }
                vesta::scout() << oss.str();
            }
            ...
            if (cleanup_vm && cleanup_proc) {
                std::thread([vm = cleanup_vm, pid = cleanup_pid,
                    ...
                    // esperar hasta que el future deje de estar PENDING (ACK recibido)
                    // o hasta 5 s como maximo para evitar que el hilo quede huerfano
                    for (int i = 0; i < 500; i++) {
                    ...
                    // el proceso ya no esta en ningun estado activo (NEW); safe kill
        ...
        vesta::scout() << "[dist] Subcomando desconocido: '" << subcmd << "'\n"
                       << "  Subcomandos validos: new, start, stop, info, list, add-node, connect, discover, run\n"
    ...
    static void command_alias(const std::string &args) {
        ...
        if (args.empty()) {
            ...
            for (auto &[k, v] : g_aliases.map) {
                std::cout << "alias "
                          << ansi::c(ansi::BR_YELLOW) << k << ansi::c(ansi::RESET)
                          << "='"
        ...
        if (eq_pos != std::string::npos) {
            ...
            std::cout << "alias "
                      << ansi::c(ansi::BR_YELLOW) << name << ansi::c(ansi::RESET)
                      << "='"
                      << ansi::c(ansi::BR_GREEN)  << val  << ansi::c(ansi::RESET)
                      << "'\n";
        } else {
            ...
            if (it == g_aliases.map.end()) {
                ...
                std::cout << "alias "
                          << ansi::c(ansi::BR_YELLOW) << words[0] << ansi::c(ansi::RESET)
                          << "='"
                          << ansi::c(ansi::BR_GREEN)  << it->second << ansi::c(ansi::RESET)
                          << "'\n";
            }
        }
    }
    ...
    static bool cmd_table_has(const std::string &name); // comprueba si name esta en cmd_table

    // ---- helper interno: expande $VAR en una cadena arbitraria ----

    /**
     * @brief Sustituye cada token $VAR o ${VAR} en @p s por el valor de la variable de entorno.
    ...
    static void command_env(const std::string &args) {
        ...
        if (words.empty() || words[0] == "list") {
            ...
            char *block = GetEnvironmentStringsA();
            if (!block) { std::cout << "[env] Error al obtener el entorno\n"; return; }
            for (char *p = block; *p; p += std::strlen(p) + 1) {
                std::string entry(p);
                auto eq = entry.find('=');
                // ignorar entradas internas que empiezan por '=' (Windows las usa internamente)
                if (eq == std::string::npos || eq == 0) continue;
                std::string key = entry.substr(0, eq);
                std::string val = entry.substr(eq + 1);
                std::cout << ansi::c(ansi::BR_YELLOW) << key
                          << ansi::c(ansi::RESET) << "="
        ...
        if (words[0] == "set") {
            ...
            if (key.empty()) { std::cout << "[env] Nombre de variable vacio\n"; return; }
#if defined(_WIN32)
        ...
        // env unset KEY
        if (words[0] == "unset") {
            if (words.size() < 2) { std::cout << "Uso: env unset KEY\n"; return; }
#if defined(_WIN32)
        ...
        std::cout << "Uso: env [list|get KEY|set KEY VALUE|unset KEY|save [f]|load [f]]\n";
    }
    ...
    static void command_mkdir(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: mkdir <directorio> [directorio2 ...]\n"; return; }
        for (auto &w : words) {
            std::error_code ec;
    ...
    static std::string vel_highlight(const std::string &line) {
        ...
        auto semi = line.find(';');
        std::string code_part   = (semi != std::string::npos) ? line.substr(0, semi) : line;
        std::string comment_part = (semi != std::string::npos) ? line.substr(semi) : "";
        ...
        size_t tok_start = i;
        while (i < code_part.size() && !std::isspace((unsigned char)code_part[i]) && code_part[i] != ';') ++i;
        ...
        if (!first_tok.empty() && first_tok.back() == ':') {
            ...
            static const std::unordered_set<std::string> mnemonics = {
                "mov","movh","movc","movch","add","sub","mul","div","cmp","and","or","xor","not",
                "shl","shr","sar","jmp","jz","jnz","je","jne","jl","jle","jg","jge","ja","jb",
                "call","ret","hlt","nop","push","pop","inc","dec",
                "mkclosure","callclosure","mkrawclosure","callrawclosure","tailcall",
                "isnull","unwrap","jumptable","typeswitch","future","await","fulfill","reject",
                "subsp","addsp","weakref","deref_weak","free_weak",
                "monenter","monexit","monwait","monnoti","monnota","specialize",
                "rspawn","msgsend","msgrecv","memsync",
                "yield","resume","spawn","swapctx",
                "getproc","getvm","getmgr","calln","db","dw","dd","dq","align"
            };
            ...
            if (mnemonics.count(base) || mnemonics.count(lc))
                out += std::string(ansi::c(ansi::BOLD)) + ansi::c(ansi::BR_CYAN) + first_tok + ansi::c(ansi::RESET);
            else
                out += first_tok;
        }
        ...
        while (i < code_part.size()) {
            ...
            while (i < code_part.size() && (std::isspace((unsigned char)code_part[i]) ||
                   code_part[i] == ',' || code_part[i] == '[' || code_part[i] == ']' ||
                   code_part[i] == '+' || code_part[i] == '-' || code_part[i] == '*')) {
                out += code_part[i++];
            }
            ...
            if (is_reg) {
                out += ansi::c(ansi::BR_GREEN) + tok + ansi::c(ansi::RESET);
                       ...
                       (tok.size() > 1 && tok[0] == '-' && std::isdigit((unsigned char)tok[1])) ||
    ...
    static void command_cat(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: cat [-n] [-h] <archivo> [archivo2 ...]\n"; return; }

        bool line_nums = false;
        bool no_highlight = false;
        std::vector<std::string> files;
        for (auto &w : words) {
            if (w == "-n")          line_nums    = true;
            else if (w == "--raw")  no_highlight = true;
            else                    files.push_back(w);
        }
        if (files.empty()) { std::cout << "Uso: cat [-n] [--raw] <archivo>\n"; return; }

        for (auto &f : files) {
            auto path = expand_path(f);
            std::ifstream ifs(path);
            if (!ifs) {
                std::cout << ansi::c(ansi::BR_RED) << "cat: no se puede abrir: "
    ...
    static void command_rm(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: rm [-r] <ruta> [ruta2 ...]\n"; return; }

        bool recursive = false;
        std::vector<std::string> targets;
        for (auto &w : words) {
            if (w == "-r" || w == "-rf" || w == "-fr") recursive = true;
            else targets.push_back(w);
        }
        if (targets.empty()) { std::cout << "Uso: rm [-r] <ruta>\n"; return; }
        ...
        for (auto &t : targets) {
            ...
            if (is_dir && !recursive) {
                std::cout << "[rm] '" << path.string()
                          << "' es un directorio; usa -r para borrar recursivamente\n";
                continue;
            }
            if (is_dir) {
                // pedir confirmacion para directorios
                std::cout << ansi::c(ansi::BR_YELLOW)
                          << "Borrar directorio '" << path.string() << "' y todo su contenido? [s/N] "
                ...
                else    std::cout << "[rm] Eliminados " << removed << " elemento(s)\n";
            } else {
                std::filesystem::remove(path, ec);
                if (ec) std::cout << "[rm] Error: " << ec.message() << "\n";
                else    std::cout << "[rm] Eliminado: " << path.string() << "\n";
            }
        }
    }
    ...
    static void command_touch(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: touch <archivo> [archivo2 ...]\n"; return; }
        for (auto &w : words) {
            ...
            if (std::filesystem::exists(p, ec)) {
                ...
                if (!ofs) std::cout << "[touch] No se puede actualizar: " << p.string() << "\n";
            } else {
                std::ofstream ofs(p);
                if (!ofs) std::cout << "[touch] No se puede crear: " << p.string() << "\n";
                else      std::cout << "[touch] Creado: " << p.string() << "\n";
            }
        }
    }
    ...
    static void command_cp(const std::string &args) {
        ...
        std::vector<std::string> targets;
        for (auto &w : words) {
            if (w == "-r" || w == "-R") recursive = true;
            else targets.push_back(w);
        }
        if (targets.size() < 2) { std::cout << "Uso: cp [-r] <src> <dst>\n"; return; }
        ...
        if (std::filesystem::is_directory(src)) {
            if (!recursive) { std::cout << "[cp] Usa -r para copiar directorios\n"; return; }
        ...
        else    std::cout << "[cp] " << src.string() << " -> " << dst.string() << "\n";
    }
    ...
    static void command_mv(const std::string &args) {
        auto words = split_words(args);
        if (words.size() < 2) { std::cout << "Uso: mv <src> <dst>\n"; return; }
        ...
        if (ec) std::cout << "[mv] Error: " << ec.message() << "\n";
        else    std::cout << "[mv] " << src.string() << " -> " << dst.string() << "\n";
    }

    // ---- comando head ----
    /** @brief Muestra las primeras N lineas de un archivo. Formato: head [-n N] <archivo> */
    static void command_head(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: head [-n N] <archivo>\n"; return; }
        size_t n = 10;
        std::vector<std::string> files;
        for (size_t i = 0; i < words.size(); ++i) {
            if (words[i] == "-n" && i + 1 < words.size()) {
                try { n = std::stoul(words[++i]); } catch (...) {}
            } else {
        ...
        for (auto &f : files) {
            ...
            std::ifstream ifs(p);
            if (!ifs) { std::cout << "head: no se puede abrir: " << p.string() << "\n"; continue; }
            if (files.size() > 1)
                std::cout << ansi::c(ansi::BOLD) << "==> " << p.string() << " <==" << ansi::c(ansi::RESET) << "\n";
            std::string line;
    ...
    static void command_tail(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: tail [-n N] [-f] <archivo>\n"; return; }
        size_t n = 10;
        bool follow = false;
        std::vector<std::string> files;
        for (size_t i = 0; i < words.size(); ++i) {
        ...
        if (files.empty()) { std::cout << "Uso: tail [-n N] [-f] <archivo>\n"; return; }

        for (auto &f : files) {
            auto p = expand_path(f);
            std::ifstream ifs(p);
            if (!ifs) { std::cout << "tail: no se puede abrir: " << p.string() << "\n"; continue; }
            if (files.size() > 1)
                std::cout << ansi::c(ansi::BOLD) << "==> " << p.string() << " <==" << ansi::c(ansi::RESET) << "\n";
            ...
            for (size_t i = start; i < lines.size(); ++i) std::cout << lines[i] << "\n";

            if (follow) {
                std::cout << ansi::c(ansi::DIM) << "[tail -f] Siguiendo " << p.string()
                          << " (pulsa cualquier tecla para salir)..." << ansi::c(ansi::RESET) << "\n";
                ...
                while (true) {
                    ...
                    if (select(STDIN_FILENO + 1, &fds, nullptr, nullptr, &tv) > 0) {
                        char c; ::read(STDIN_FILENO, &c, 1); break;
                    }
#endif
    ...
    static void command_type(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: type <nombre> [nombre2 ...]\n"; return; }
        for (auto &name : words) {
            // comprobar alias primero
            {
                ...
                auto it = g_aliases.map.find(name);
                if (it != g_aliases.map.end()) {
                    std::cout << ansi::c(ansi::BR_YELLOW) << name << ansi::c(ansi::RESET)
                              << " es un alias -> '"
                              << ansi::c(ansi::BR_GREEN) << it->second << ansi::c(ansi::RESET) << "'\n";
                    continue;
                }
            ...
            static const char *specials[] = {"exit","help","history","interprete","complete",
                                              "source","!N",":prev",":next"};
            bool is_special = false;
            for (auto &s : specials) if (name == s) { is_special = true; break; }
            if (is_special) {
    ...
    static void command_source(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: source <archivo>\n"; return; }
        auto path = expand_path(words[0]);
        std::ifstream f(path);
        if (!f) { std::cout << "[source] No se puede abrir: " << path.string() << "\n"; return; }
        std::string line;
        size_t count = 0, ok = 0;
        while (std::getline(f, line)) {
            ++count;
        ...
        vesta::scout() << "[source] " << path.filename().string()
                       << ": " << ok << "/" << count << " lineas ejecutadas\n";
    }

    // ---- env persistente: subcomandos save / load ----
    // (la funcion command_env completa esta mas arriba; se amplia con estos helpers)
    ...
    static void command_set(const std::string &args) {
        ...
        if (args.empty() || args == "list") {
            if (!g_current_impl) { std::cout << "[set] REPL no activo\n"; return; }
            auto &cfg = g_current_impl->cfg;
            auto P = [](const char *k, const std::string &v) {
                std::cout << ansi::c(ansi::BR_YELLOW) << std::left << std::setw(20) << k
                          << ansi::c(ansi::RESET) << "= "
                          << ansi::c(ansi::BR_GREEN) << v << ansi::c(ansi::RESET) << "\n";
            };
        ...
        if (val.empty()) {
            // solo mostrar
            if      (key == "history_max")   std::cout << cfg.history_max   << "\n";
            else if (key == "multiline_end")  std::cout << cfg.multiline_end  << "\n";
            else if (key == "aliases_file")   std::cout << cfg.aliases_file   << "\n";
            else if (key == "env_file")       std::cout << g_env_file         << "\n";
            else std::cout << "[set] Clave desconocida: " << key << "\n";
            return;
        }

        // aplicar cambio
        if      (key == "history_max")  {
            try { cfg.history_max = std::stoul(val); std::cout << "[set] history_max=" << cfg.history_max << "\n"; }
            catch (...) { std::cout << "[set] Valor invalido\n"; }
        }
        else if (key == "multiline_end") { cfg.multiline_end = val; std::cout << "[set] OK\n"; }
        else if (key == "aliases_file")  {
            cfg.aliases_file = val; g_aliases.file = val;
            std::cout << "[set] OK (recarga con 'source " << val << "')\n";
        }
        else if (key == "env_file")     { g_env_file = val; std::cout << "[set] OK\n"; }
        else std::cout << "[set] Clave desconocida: " << key
                       << "\n  Claves: history_max, multiline_end, aliases_file, env_file\n";
    }
    ...
    static void command_jobs(const std::string &) {
        ...
        for (auto &[id, e] : g_jobs.jobs) {
            double elapsed = std::chrono::duration<double>(now - e->start_time).count();
            char   dur[32]; snprintf(dur, sizeof(dur), "%.1fs", elapsed);
    ...
    static void command_wait(const std::string &args) {
        auto words = split_words(args);
        if (words.empty()) { std::cout << "Uso: wait <job_id>\n"; return; }
        size_t id;
        try { id = std::stoul(words[0]); } catch (...) {
    ...
    static void command_watch(const std::string &args) {
        auto words = split_words(args);
        if (words.size() < 2) { std::cout << "Uso: watch <intervalo_ms> <comando ...>\n"; return; }
        size_t ms = 1000;
        try { ms = std::stoul(words[0]); } catch (...) {
        ...
        while (true) {
            ...
            char tbuf[64]; struct tm *tmi = std::localtime(&t);
            if (tmi) std::strftime(tbuf, sizeof(tbuf), "%H:%M:%S", tmi); else tbuf[0]=0;
            std::cout << ansi::c(ansi::BOLD) << ansi::c(ansi::DIM)
                      << "watch (" << ms << "ms)  " << tbuf << "  '" << watch_cmd << "'"
                      << ansi::c(ansi::RESET) << "\n\n";
...
#if defined(_WIN32)
                if (_kbhit()) { _getch(); goto watch_exit; }
#else
                struct timeval tv{0, 0};
                fd_set fds; FD_ZERO(&fds); FD_SET(STDIN_FILENO, &fds);
                if (select(STDIN_FILENO + 1, &fds, nullptr, nullptr, &tv) > 0) {
                    char c2; ::read(STDIN_FILENO, &c2, 1); goto watch_exit;
                }
#endif
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
            }
    ...
    /**
     * @brief Devuelve (o crea) el interprete VestaShell con el callback de despacho REPL.
     *
    ...
    static vsh::VshInterpreter &get_vsh_interp() {
        if (!g_vsh_interp) {
            g_vsh_interp = new vsh::VshInterpreter(
    ...
    static void command_vsh(const std::string &/*args*/) {
        try {
            ...
            std::cout << ansi::c(ansi::BR_RED) << "[vsh] " << e.what() << ansi::c(ansi::RESET) << "\n";
        }
    }
    ...
    static const CmdEntry cmd_table[] = {
        ...
        { "dist",     "dist <subcmd> <mgr_id> [opciones]  (dist help para mas info)",
          "Gestionar el runtime distribuido (start/stop/info/list/add-node/connect/discover/run)", command_dist },
        ...
        { "env",    "env [list|get KEY|set KEY=VALUE|unset KEY|save [f]|load [f]]",
          "Gestionar variables de entorno del proceso (save/load persiste en disco)",              command_env  },
        { "echo",   "echo <texto con $VARS>",
          "Imprimir texto expandiendo variables de entorno ($VAR, ${VAR})",                        command_echo },
        { "mkdir",  "mkdir <directorio> [directorio2 ...]",
          "Crear directorios (con padres intermedios)",                                            command_mkdir },
        { "cat",    "cat [-n] [--raw] <archivo> [archivo2 ...]",
          "Mostrar archivos (.vel con syntax highlight; --raw desactiva colores)",                 command_cat  },
        { "rm",     "rm [-r] <ruta> [ruta2 ...]",
        ...
        { "mv",     "mv <src> <dst>",
          "Mover o renombrar archivo/directorio",                                                  command_mv   },
        ...
        { "tail",   "tail [-n N] [-f] <archivo> [archivo2 ...]",
          "Mostrar las ultimas N lineas; -f sigue el archivo en tiempo real",                      command_tail },
        { "type",   "type <nombre> [nombre2 ...]",
          "Mostrar si un nombre es alias, comando interno o desconocido",                          command_type },
        { "source", "source <archivo>",
          "Ejecutar linea a linea un fichero de comandos del REPL",                                command_source },
        { "set",    "set [clave [= valor]]",
          "Leer o modificar parametros del REPL en caliente (history_max, multiline_end...)",      command_set  },
        { "jobs",   "jobs",
          "Listar tareas en background con estado y duracion",                                     command_jobs },
        { "wait",   "wait <job_id>",
          "Bloquear hasta que el job indicado termine",                                            command_wait },
        { "watch",  "watch <intervalo_ms> <comando ...>",
          "Ejecutar un comando repetidamente con intervalo; cualquier tecla detiene",              command_watch },
    ...
    static bool dispatch_table_cmd(const std::string &raw_line) {
        ...
        auto sp = line.find(' ');
        std::string cmd_name = (sp == std::string::npos) ? line : line.substr(0, sp);
        std::string cmd_args = (sp == std::string::npos) ? std::string{} : line.substr(sp + 1);
        ...
        for (int depth = 0; depth < 16; ++depth) {
            ...
            std::string full = cmd_args.empty() ? expanded : expanded + " " + cmd_args;
            auto sp2 = full.find(' ');
            cmd_name = (sp2 == std::string::npos) ? full : full.substr(0, sp2);
            cmd_args = (sp2 == std::string::npos) ? std::string{} : full.substr(sp2 + 1);
        }
    ...
    static void print_help_impl(const Impl &I) {
        ...
        // seccion de aliases solo si hay alguno definido
        {
            ...
            if (!g_aliases.map.empty()) {
                ...
                for (auto &[k, v] : g_aliases.map) {
                    ...
                    std::cout << "  "
                              << ansi::c(ansi::BR_YELLOW) << std::left << std::setw(46) << entry
                              << ansi::c(ansi::RESET) << "\n";
                }
            }
        }
        ...
        std::cout << "\n"
                  ...
                  << "      'dist help' muestra la ayuda completa de los comandos distribuidos.\n"
                  << "      Los aliases se guardan en '" << g_aliases.file << "' y persisten entre sesiones.\n"
                  << ansi::c(ansi::RESET);
    }
    ...
    static void handle_complete_impl(const Impl &I, const std::string &pref) {
        // candidatos: comandos especiales + tabla + aliases definidos por el usuario
        ...
        {
            std::lock_guard<std::mutex> lk(g_aliases.mtx);
            for (auto &[k, _] : g_aliases.map) candidates.push_back(k);
        }
    ...
    /**
     * @brief Lee una linea con edicion interactiva, historial con flechas y atajos bash.
     *
     * Teclas soportadas:
     ...
     *   Ctrl+C           : cancelar (activa interrupted_flag, devuelve cadena vacia)
     *   Ctrl+D           : EOF en linea vacia (pone eof_out = true)
     *   Enter            : confirmar y devolver la linea
     *
     ...
     */
    static std::vector<std::string> tab_completions(const Impl &I,
        ...
        if (is_first_token) {
            ...
            // comandos especiales del REPL
            for (auto &sp : {"exit","help","history","interprete","complete","source"}) {
                std::string n(sp);
                if (n.rfind(prefix, 0) == 0) out.push_back(n);
            }
            // aliases
            ...
            // completar con rutas: separar directorio y prefijo de nombre de archivo
            ...
            std::error_code ec;
            for (auto &entry : std::filesystem::directory_iterator(expand_path(search_dir), ec)) {
    ...
    static std::string readline_impl(Impl &I, const std::string &prompt_str, bool &eof_out) {
        eof_out = false;
        ...
        auto redraw = [&]() {
            ...
            if (after > 0) std::cout << "\x1B[" << after << "D"; // retroceder cursor
            std::cout << std::flush;
        };
        ...
        for (;;) {
...
#else
            {
                ...
                if (c == 0x1B) {
                    ...
                    if (nr >= 2 && seq[0] == '[') {
                        if      (seq[1] == 'A')                          key = RL_UP;
                        else if (seq[1] == 'B')                          key = RL_DOWN;
                        else if (seq[1] == 'C')                          key = RL_RIGHT;
                        else if (seq[1] == 'D')                          key = RL_LEFT;
                        else if (seq[1] == 'H')                          key = RL_HOME;
                        else if (seq[1] == 'F')                          key = RL_END;
                        else if (seq[1] == '1' && nr>=3 && seq[2]=='~') key = RL_HOME;
                        else if (seq[1] == '3' && nr>=3 && seq[2]=='~') key = RL_DEL;
                        else if (seq[1] == '4' && nr>=3 && seq[2]=='~') key = RL_END;
                    }
            ...
            if (key == 1)  { cur = 0; redraw(); continue; }
            // Ctrl+E: fin de linea
            if (key == 5)  { cur = buf.size(); redraw(); continue; }
            // Ctrl+K: borrar desde cursor hasta fin
            if (key == 11) { buf.erase(cur); redraw(); continue; }
            // Ctrl+U: borrar desde inicio hasta cursor
            if (key == 21) { buf.erase(0, cur); cur = 0; redraw(); continue; }
            // Ctrl+W: borrar palabra anterior
            if (key == 23) {
                if (cur > 0) {
                    size_t e = cur;
            ...
            if (key == 127 || key == 8) {
                if (cur > 0) { buf.erase(--cur, 1); redraw(); }
                continue;
            }

            // flechas y teclas de navegacion
            if (key == RL_LEFT)  { if (cur > 0)           { --cur; redraw(); } continue; }
            if (key == RL_RIGHT) { if (cur < buf.size())  { ++cur; redraw(); } continue; }
            if (key == RL_HOME)  { cur = 0;               redraw(); continue; }
            if (key == RL_END)   { cur = buf.size();      redraw(); continue; }
            if (key == RL_DEL)   {
                if (cur < buf.size()) { buf.erase(cur, 1); redraw(); }
                continue;
            }
            ...
            if (key == RL_UP) {
                ...
                if (hist_pos == -1) {
                    ...
                    hist_pos  = (int)I.history.size() - 1; // apuntar a la entrada mas reciente
                } else if (hist_pos > 0) {
                    --hist_pos;
                }
            ...
            if (key == 9) {
                ...
                std::string prefix = is_first ? before_cur : before_cur.substr(before_cur.rfind(' ') + 1);
                auto candidates = tab_completions(I, prefix, is_first);
                if (candidates.empty()) {
                    // sin completions: pitido visual (blink)
                    std::cout << "\x07" << std::flush;
                } else if (candidates.size() == 1) {
                    // completar directamente: reemplazar prefijo con el unico candidato
                    ...
                    if (cp.size() > prefix.size()) {
                        ...
                        std::cout << "\n";
                        for (auto &c : candidates)
                            std::cout << ansi::c(ansi::BR_CYAN) << c << ansi::c(ansi::RESET) << "  ";
                        std::cout << "\n";
                        redraw();
                    }
                }
...
#if defined(_WIN32)
                    k2 = _getch();
                    if (k2 == 0 || k2 == 224) { _getch(); k2 = 0; }
#else
                    unsigned char c2;
                    ssize_t nr2;
                    do { nr2 = ::read(STDIN_FILENO, &c2, 1); } while (nr2 < 0 && errno == EINTR);
                    k2 = (nr2 <= 0) ? 4 : (int)c2;
#endif
                    if (k2 == 18) {
                        ...
                        // Ctrl+G o ESC: cancelar busqueda, restaurar buffer original
                        std::cout << "\n" << std::flush;
                        break;
    ...
    void VestaViewManager::run() {
        ...
        // anclar ficheros de estado al directorio del ejecutable (evita que cd los mueva)
        {
            std::string exe_dir =
                std::filesystem::path(fs::get_executable_path()).parent_path().string();
        ...
        // exponer impl_ para command_set y otras funciones que necesitan acceso al cfg
        g_current_impl = impl_;

        load_history_impl(*impl_);
        aliases_load(g_aliases);
        env_load_file(g_env_file); // cargar variables de entorno persistentes al arrancar
        ...
        // ejecutar el script de inicio si existe (~/.vestarc o ./vm_init.vel/.vsh)
        {
            ...
            auto run_startup_repl = [](const std::string &path) {
                ...
                if (!f) return false;
                std::string line;
                while (std::getline(f, line)) dispatch_table_cmd(line);
                return true;
            };
            ...
            auto run_startup_vsh = [](const std::string &path) {
                std::ifstream f(path);
                if (!f) return false;
                try { get_vsh_interp().exec_file(path); } catch (...) {}
                return true;
            };
        ...
        while (impl_->running) {
            // construir el prompt dinamico como cadena ANSI (lo pinta readline_impl)
            std::string prompt_str;
            {
                ...
                try {
                    ...
                    if (cwd_name.empty())
                        cwd_name = cwd.root_name().string() + cwd.root_directory().string();
                } catch (...) { cwd_name = "?"; }

                prompt_str =
                    std::string(ansi::c(ansi::BOLD))    + ansi::c(ansi::BR_CYAN)  + "vesta"  +
                    ansi::c(ansi::RESET) + ansi::c(ansi::DIM)     + " ["    +
                    ansi::c(ansi::RESET) + ansi::c(ansi::BR_GREEN) + cwd_name +
                    ansi::c(ansi::RESET) + ansi::c(ansi::DIM)     + "]"     +
            ...
            // patron: "alias ls='ls -la'" -> cmd_name="ls" se sustituye por "ls -la"
            // autoreferencia ("alias ls='ls -l'"): se detecta comparando el primer
            // token de la expansion con el nombre del alias para romper el ciclo
            for (int _alias_depth = 0; _alias_depth < 16; ++_alias_depth) {
                std::string expanded;
                {
                    std::lock_guard<std::mutex> lk(g_aliases.mtx);
                    auto it = g_aliases.map.find(cmd_name);
                    if (it == g_aliases.map.end()) break; // no hay alias para este nombre
                    expanded = it->second;
                }
                // si la expansion empieza con el mismo nombre, es autoreferencia:
                // parar para no entrar en bucle infinito (permite "alias ls='ls -l'")
            ...
            if (!handled) {
                ...
                try {
                    ...
                    out = std::string(ansi::c(ansi::BR_RED)) +
                          e.what() + ansi::c(ansi::RESET);
                }
                if (!out.empty()) {
