...
namespace cli {
    ...
    void VestaInterprete::run_interprete() {
        ...
        // para garantizar que el proceso ya esta en la cola cuando el scheduler arranca.
        ...
        vesta::scout()
            << ansi::c(ansi::BR_CYAN) << "[interprete]" << ansi::c(ansi::RESET)
            << " Modo interactivo. Escribe 'quit' para salir, 'status' para ver la VM.\n";

        while (running) {
            try {
                // prompt del interprete con color
                vesta::scout()
                    << ansi::c(ansi::BOLD) << ansi::c(ansi::CYAN)
                ...
                if (tokens[0] == "bytecode") {
                    if (tokens.size() == 2 && tokens[1] == "on") {
                        ...
                        show_bytecode = false;
                        std::cout << ansi::c(ansi::YELLOW) << "[info] bytecode OFF\n" << ansi::c(ansi::RESET);
                    } else {
                        std::cout << ansi::c(ansi::RED) << "[error] uso: bytecode on | bytecode off\n" << ansi::c(ansi::RESET);
                    }
                    continue;
                }
                ...
                if (show_bytecode)
                    vesta::scout() << vesta::dump(bytecode.data(), bytecode.size()) << "\n";
                ...
                // (puede estar todavia en run_loop despues de que el proceso llegue a HALT)
                for (auto &f : vm->scheduler_futures) {
                    if (f.valid()) f.wait();
                }
                vm->scheduler_futures.clear(); // limpiar futures consumidos

                // invalidar el icache para que el decodificador no reutilice instrucciones
                // del fragmento anterior que ocupara la misma direccion de memoria
                process->reset_cache();
                ...
                // si se llamase antes, el scheduler podria ver alive_count==0 y terminar.
                vm->start();

                // esperar a que el proceso complete el fragmento (HALT) o falle (DEAD)
                while (process->state != runtime::HALT && process->state != runtime::DEAD) {
                    std::this_thread::sleep_for(std::chrono::milliseconds(1));
                }

                // leer RIP tras HLT: apunta al byte siguiente al HLT,
                // que es donde el proximo fragmento debe cargarse para preservar estado
                last_ip = process->registers.rip.raw();

            } catch (const std::exception &e) {
                std::cerr << ansi::c(ansi::BR_RED) << "[error] " << e.what() << ansi::c(ansi::RESET) << "\n";
            }
        }
