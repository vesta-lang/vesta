...
static void vsh_sock_close(vsh_sock_t s) {
    closesocket(s);
}

static bool vsh_sock_valid(vsh_sock_t s) {
...
static void flush_out_buffer() {
    ...
    if (g_is_console) {
        ...
        int n = utf8_to_utf16(g_out_buffer.data(),
                              static_cast<int>(g_out_buffer.size()), wide);
        if (n > 0) {
            DWORD written = 0;
            ...
            while (remaining > 0) {
                ...
                if (!WriteConsoleW(g_console_handle, p, to_write, &written,
                                   nullptr)) {
        ...
        // Pipe o redireccion: fwrite directo (no conviene WriteConsole con
        // pipes)
        fwrite(g_out_buffer.data(), 1, g_out_buffer.size(), stdout);
    }
#else
...
static void ensure_ssl_ctx() {
    std::call_once(g_ssl_init_flag, []() {
        ...
        if (g_ssl_ctx) SSL_CTX_set_verify(g_ssl_ctx, SSL_VERIFY_NONE, nullptr);
    });
}

static int64_t vsh_alloc_sock(vsh_sock_t fd, bool is_udp = false,
                              SSL *ssl = nullptr) {
...
static void vsh_resolve(const std::string &host, uint16_t port,
...
static VshHttpResp vsh_http_request(
    ...
    const std::string &path, bool use_tls, const std::string &body = "",
    ...
    if (!vsh_sock_valid(fd))
        throw vsh::VshRuntimeError("error creando socket HTTP");
    ...
    if (sp1 != std::string::npos) {
        ...
        std::string code_str = sp2 == std::string::npos
                                   ? r.headers.substr(sp1 + 1)
                                   : r.headers.substr(sp1 + 1, sp2 - sp1 - 1);
        try {
            r.status = std::stoi(code_str);
        } catch (...) {
        }
    }
    return r;
...
vsh_parse_url(const std::string &url, bool &use_tls, std::string &path_out) {
    use_tls = false;
    std::string work = url;
    ...
    std::string hostport =
        slash == std::string::npos ? work : work.substr(0, slash);
...
static int64_t vsh_ffi_sym(int64_t lib_id, const std::string &name) {
    ...
    if (it == g_ffi_libs.end())
        throw vsh::VshRuntimeError("ffi_sym: handle invalido");
#if defined(_WIN32)
    FARPROC p = GetProcAddress(it->second.handle, name.c_str());
    if (!p)
        throw vsh::VshRuntimeError("ffi_sym: simbolo no encontrado: " + name);
    int64_t result = 0;
...
static int64_t vsh_ffi_call(int64_t sym_addr,
                            const std::vector<vsh::Value> &args) {
    ...
    for (const auto &a : args) {
        if (a.is_string()) {
            ...
            throw vsh::VshRuntimeError(
                "ffi_call: tipo de argumento no soportado para FFI");
        }
    }
    ...
    typedef int64_t (*fn6)(int64_t, int64_t, int64_t, int64_t, int64_t,
                           int64_t);
    typedef int64_t (*fn7)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t,
                           int64_t);
    typedef int64_t (*fn8)(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t,
                           int64_t, int64_t);
    ...
    case 4:
        return reinterpret_cast<fn4>(raw)(iargs[0], iargs[1], iargs[2],
                                          iargs[3]);
    case 5:
        return reinterpret_cast<fn5>(raw)(iargs[0], iargs[1], iargs[2],
                                          iargs[3], iargs[4]);
    case 6:
        return reinterpret_cast<fn6>(raw)(iargs[0], iargs[1], iargs[2],
                                          iargs[3], iargs[4], iargs[5]);
    case 7:
        return reinterpret_cast<fn7>(raw)(iargs[0], iargs[1], iargs[2],
                                          iargs[3], iargs[4], iargs[5],
                                          iargs[6]);
    case 8:
        return reinterpret_cast<fn8>(raw)(iargs[0], iargs[1], iargs[2],
                                          iargs[3], iargs[4], iargs[5],
                                          iargs[6], iargs[7]);
    default:
        throw vsh::VshRuntimeError("ffi_call: maximo 8 argumentos soportados");
    }
}

// Igual que vsh_ffi_call pero el entero retornado se reinterpreta como double.
static double vsh_ffi_call_f(int64_t sym_addr,
                             const std::vector<vsh::Value> &args) {
...
std::string Value::to_string() const {
    ...
    case VshType::Instance: {
        ...
        // si tiene __str__, se llamara desde eval; aqui retornamos
        // representacion por defecto
...
char VshLexer::cur() const {
    return pos_ < src_.size() ? src_[pos_] : '\0';
}

char VshLexer::peek1() const {
...
const Token &VshLexer::peek(int offset) {
    while ((int)buf_.size() <= offset)
        buf_.push_back(read_one_token());
    return buf_[offset];
}
...
// empuja en buf_ la secuencia ISTR_TEXT / ISTR_EXPR_BEGIN ... ISTR_EXPR_END /
// ISTR_END
void VshLexer::push_interp_tokens(const std::string &raw, int base_line,
                                  int base_col) {
    std::string text;
    size_t i = 0;
    ...
    while (i < raw.size()) {
        ...
        if (raw[i] == '\\' && i + 1 < raw.size()) {
            char e = raw[i + 1];
            // \xHH : byte hexadecimal de 2 digitos
            if ((e == 'x' || e == 'X') && i + 3 < raw.size()) {
                int hi = hex_val(raw[i + 2]);
                int lo = hex_val(raw[i + 3]);
                if (hi >= 0 && lo >= 0) {
            ...
            case 'f': text += '\f'; break;
            case 'v': text += '\v'; break;
            case 'e': text += '\x1b'; break; // ESC: comodidad para ANSI
            case '"': text += '"'; break;
            case '\\': text += '\\'; break;
...
Token VshLexer::read_string_or_interp() {
                ...
                if (cur() == '\\') {
                    ...
                    if (pos_ < src_.size()) {
                        ...
                        if ((e == 'x' || e == 'X') && pos_ + 1 < src_.size()) {
                            ...
                            int hi = hex_v(src_[pos_]);
                            int lo = hex_v(src_[pos_ + 1]);
                            if (hi >= 0 && lo >= 0) {
                                advance();
                                advance();
    ...
    if (!has_interp) {
        ...
        for (size_t i = 0; i < raw.size(); ++i) {
            if (raw[i] == '\\' && i + 1 < raw.size()) {
                ...
                // \xHH : byte hexadecimal de 2 digitos
                if ((e == 'x' || e == 'X') && i + 3 < raw.size()) {
                    int hi = hex_v(raw[i + 2]);
                    int lo = hex_v(raw[i + 3]);
                    if (hi >= 0 && lo >= 0) {
                        out += static_cast<char>((hi << 4) | lo);
                        i += 3;
                        continue;
    ...
    // El parser vera ISTR_TEXT (o ISTR_EXPR_BEGIN como primer token) seguido de
    // ISTR_END Devolvemos un token ficticio que el parser identifica como
    // inicio de interp Usaremos ISTR_TEXT con lexeme especial "" como "ya esta
    // en buf_" En realidad simplemente devolvemos el primer token del buf_
    Token first = buf_.front();
    buf_.pop_front();
    return first;
}
...
bool VshParser::check(TK k) const {
    return cur_.kind == k;
}
...
Token VshParser::expect(TK k, const char *msg) {
    if (!check(k)) {
        throw VshParseError(std::string(msg) + " (se obtuvo '" + cur_.lexeme +
                                "')",
                            cur_.line, cur_.col);
    }
    return advance();
...
void VshParser::skip_nl() {
    while (check(TK::NEWLINE) || check(TK::SEMICOLON))
        advance();
}

void VshParser::end_stmt() {
    // termina sentencia con NEWLINE, ';', o implicitamente antes de '}'
    if (check(TK::NEWLINE) || check(TK::SEMICOLON)) {
        advance();
        return;
    }
...
AstNodePtr VshParser::let_decl() {
    ...
    advance(); // 'let'
    n->name =
        expect(TK::IDENT, "se esperaba nombre de variable despues de 'let'")
            .lexeme;
    if (match(TK::COLON)) {
        if (!is_type_name_token(cur_.kind))
            throw VshParseError("se esperaba nombre de tipo despues de ':'",
                                cur_.line, cur_.col);
        n->type_hint = cur_.lexeme;
        advance();
    }
    if (match(TK::EQ)) {
        n->right = expr();
...
std::vector<std::string> VshParser::params_list(AstNode *n) {
    ...
    while (match(TK::COMMA)) {
        ...
        p.push_back(
            expect(TK::IDENT, "se esperaba nombre de parametro").lexeme);
        t = {};
        if (match(TK::COLON)) {
            if (!is_type_name_token(cur_.kind))
                throw VshParseError("se esperaba nombre de tipo despues de ':'",
...
AstNodePtr VshParser::try_stmt() {
    ...
    n->name =
        expect(TK::IDENT, "se esperaba nombre de variable de error en catch")
            .lexeme;
    if (match(TK::COLON)) {
        n->catch_type_hint =
            expect(TK::IDENT, "se esperaba nombre de tipo en catch tipado")
                .lexeme;
    }
    n->catch_body = block();
    return n;
...
AstNodePtr VshParser::ret_stmt() {
    ...
    advance(); // 'return'
    if (!check(TK::NEWLINE) && !check(TK::SEMICOLON) && !check(TK::RBRACE) &&
...
AstNodePtr VshParser::import_stmt() {
    ...
    advance(); // 'import'
    auto tok =
        expect(TK::STRING_LIT, "se esperaba ruta de fichero en 'import'");
    n->name = tok.lexeme;
    end_stmt();
    return n;
}
...
AstNodePtr VshParser::class_decl() {
    ...
    if (match(TK::COLON)) {
        n->parent_class =
            expect(TK::IDENT, "se esperaba nombre de clase padre").lexeme;
    }
    skip_nl();
    ...
    while (!check(TK::RBRACE) && !check(TK::END_OF_FILE)) {
        ...
        } else if (check(TK::STRING_LIT) || check(TK::ISTR_TEXT) ||
                   check(TK::ISTR_EXPR_BEGIN)) {
            // docstring de clase: primer elemento string literal
            n->args.push_back(expr_stmt());
        } else {
            throw VshParseError(
                "se esperaba 'fn' o docstring dentro del cuerpo de clase",
                cur_.line, cur_.col);
        }
        skip_nl();
    }
...
AstNodePtr VshParser::assign_expr() {
    ...
    static const TK compound[] = {TK::PLUS_EQ, TK::MINUS_EQ, TK::STAR_EQ,
                                  TK::SLASH_EQ, TK::PERCENT_EQ};
    for (auto op : compound) {
        if (check(op)) {
            TK op2 = cur_.kind;
            int ln = cur_.line, co = cur_.col;
            advance();
            auto val = expr();
...
AstNodePtr VshParser::cmp_expr() {
    auto lhs = add_expr();
    while (check(TK::EQ_EQ) || check(TK::BANG_EQ) || check(TK::LT) ||
           check(TK::GT) || check(TK::LT_EQ) || check(TK::GT_EQ)) {
        TK op = cur_.kind;
        int ln = cur_.line, co = cur_.col;
        advance();
        auto rhs = add_expr();
...
AstNodePtr VshParser::postfix_expr() {
    ...
    for (;;) {
        if (check(TK::LPAREN)) {
            ...
            advance();
            std::string field =
                expect(TK::IDENT, "se esperaba nombre de campo despues de '.'")
                    .lexeme;
...
AstNodePtr VshParser::map_lit() {
    ...
    if (!check(TK::RBRACE)) {
        ...
        auto read_key = [&]() -> AstNodePtr {
            ...
            throw VshParseError(
                "se esperaba clave de mapa (string o identificador)", cur_.line,
                cur_.col);
        };
...
AstNodePtr VshParser::interp_string() {
    // El primer token ya esta en cur_ y es ISTR_TEXT o ISTR_EXPR_BEGIN o
    // ISTR_END Construimos un nodo InterpString cuyas partes son:
    ...
    while (!check(TK::ISTR_END) && !check(TK::END_OF_FILE)) {
        if (check(TK::ISTR_TEXT)) {
            ...
            n->args.push_back(expr());
            expect(TK::ISTR_EXPR_END,
                   "se esperaba '}' al cerrar expresion embebida en string");
        } else {
            break;
        }
    }
...
AstNodePtr VshParser::primary() {
    ...
    if (check(TK::INT_LIT)) {
        ...
        n->lit_val = Value(cur_.int_val);
        advance();
        return n;
    }
    if (check(TK::FLOAT_LIT)) {
        auto n = make_node(AstKind::Literal, cur_.line, cur_.col);
        n->lit_val = Value(cur_.flt_val);
        advance();
        return n;
    }
    if (check(TK::STRING_LIT)) {
        auto n = make_node(AstKind::Literal, cur_.line, cur_.col);
        n->lit_val = Value(cur_.lexeme);
        advance();
        return n;
    }
    if (check(TK::BOOL_LIT)) {
        auto n = make_node(AstKind::Literal, cur_.line, cur_.col);
        n->lit_val = Value(bool(cur_.int_val != 0));
        advance();
        return n;
    }
    ...
    if (check(TK::LBRACE)) {
        bool is_map = false;
        // t1: primer token no-NEWLINE tras '{'; busca en peek_ y luego en
        // lex_.peek(n)
        TK t1 = peek_.kind;
        ...
        throw VshParseError(
            "'{ }' inesperado en expresion; usa map() o lista de sentencias",
            cur_.line, cur_.col);
    }
    ...
    if (check(TK::LPAREN)) {
        ...
        auto n = expr();
        expect(TK::RPAREN,
               "se esperaba ')' para cerrar expresion entre parentesis");
        return n;
    }
    throw VshParseError("expresion inesperada: '" + cur_.lexeme + "'",
                        cur_.line, cur_.col);
}
...
void VshInterpreter::exec_string(const std::string &src,
...
/** @brief Extrae el docstring de un cuerpo de funcion (primera sentencia string
 * literal). */
...
Value VshInterpreter::eval(const AstNodePtr &n, std::shared_ptr<VshEnv> env) {
    ...
    default:
        throw VshRuntimeError("eval() llamado con nodo no-expresion", n->line,
                              n->col);
    }
}
...
Value VshInterpreter::eval_interp_str(const AstNode &n,
...
void VshInterpreter::set_argv(const std::vector<std::string> &argv) {
    ...
    for (auto &a : argv)
        lst->push_back(Value(a));
    global_->define("ARGV", Value(lst));
}

Value VshInterpreter::eval_ident(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    return env->get(n.name);
}

Value VshInterpreter::eval_binop(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    ...
    case TK::PLUS:
        ...
        if (lv.is_list()) {
            ...
            for (auto &e : *rv.as_list())
                lst->push_back(e);
            return Value(lst);
        }
        throw VshRuntimeError(
            "operador '+' no soportado para " + lv.to_string(), n.line);
    case TK::MINUS:
        if (lv.is_int() && rv.is_int()) return Value(lv.as_int() - rv.as_int());
        return Value(lv.as_number() - rv.as_number());
    case TK::STAR:
        ...
        if (lv.is_string() && rv.is_int()) {
            std::string r;
            for (int64_t i = 0; i < rv.as_int(); ++i)
                r += lv.as_string();
            return Value(r);
        }
        return Value(lv.as_number() * rv.as_number());
    case TK::SLASH:
        if (rv.is_int() && rv.as_int() == 0)
            throw VshRuntimeError("division por cero", n.line);
        if (rv.is_float() && rv.as_float() == 0.0)
            throw VshRuntimeError("division por cero", n.line);
        if (lv.is_int() && rv.is_int()) return Value(lv.as_int() / rv.as_int());
        return Value(lv.as_number() / rv.as_number());
    case TK::PERCENT:
    ...
    case TK::STARSTAR: return Value(std::pow(lv.as_number(), rv.as_number()));
    ...
    default: throw VshRuntimeError("operador binario desconocido", n.line);
    }
}

Value VshInterpreter::eval_unary(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    Value v = eval(n.left, env);
    if (n.op == TK::MINUS) {
        if (v.is_int()) return Value(-v.as_int());
        if (v.is_float()) return Value(-v.as_float());
        throw VshRuntimeError("'-' unario no aplicable a " + v.to_string(),
                              n.line);
    }
...
Value VshInterpreter::eval_assign(const AstNode &n,
                                  std::shared_ptr<VshEnv> env) {
    ...
    if (n.left->kind == AstKind::FieldAccess) {
        ...
        if (!obj.is_instance())
            throw VshRuntimeError(
                "asignacion de campo '.' solo para instancias", n.line);
    ...
    if (n.left->kind == AstKind::Index) {
        ...
        if (container.is_list()) {
            if (!key.is_int())
                throw VshRuntimeError("indice de lista debe ser entero",
                                      n.line);
        ...
        throw VshRuntimeError("asignacion por indice solo para listas y mapas",
                              n.line);
    }
    throw VshRuntimeError("el lado izquierdo de '=' no es asignable", n.line);
}

Value VshInterpreter::eval_compound(const AstNode &n,
                                    std::shared_ptr<VshEnv> env) {
    // Construir BinOp sintetico y luego asignar
    AstNode binop_node;
    binop_node.kind = AstKind::BinOp;
    binop_node.line = n.line;
    binop_node.col = n.col;
    binop_node.left = n.left;
    binop_node.right = n.right;
    switch (n.op) {
...
Value VshInterpreter::eval_call(const AstNode &n, std::shared_ptr<VshEnv> env) {
    ...
    if (n.callee->kind == AstKind::FieldAccess) {
        ...
        for (auto &a : n.args)
            extra_args.push_back(eval(a, env));
        if (obj.is_instance()) {
            ...
            if (it != inst->attrs.end()) {
                if (it->second.is_function())
                    return call_method_impl(it->second.as_fn(), obj,
                                            std::move(extra_args), n.line);
                throw VshRuntimeError(
                    "atributo '" + method_name + "' no es callable", n.line);
            }
            // metodo en la clase
            auto method = find_method(inst->klass, method_name);
            if (method)
                return call_method_impl(method, obj, std::move(extra_args),
                                        n.line);
        ...
        if (obj.is_class()) {
            ...
            if (method)
                return call_fn(Value(method), std::move(extra_args), n.line);
            throw VshRuntimeError("metodo no encontrado: '" + method_name +
                                      "' en clase " + obj.as_class()->name,
                                  n.line);
        }
        throw VshRuntimeError("acceso '.' solo para instancias y clases",
                              n.line);
    }
    ...
    if (is_ident && env->has(fn_name)) {
        ...
        if (callee.is_function())
            return call_fn(callee, std::move(args), n.line);
        if (callee.is_class()) {
        ...
        throw VshRuntimeError("'" + fn_name + "' no es una funcion ni clase",
                              n.line);
    }
    ...
    if (is_ident) {
        ...
        if (bit != builtins_.end()) {
            try {
                ...
                if (e.line == 0)
                    throw VshRuntimeError(e.vsh_msg, n.line, n.col);
                throw;
            }
        }
    }
    ...
    throw VshRuntimeError("funcion desconocida: '" + fn_name + "'", n.line,
                          n.col);
}

Value VshInterpreter::eval_index(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    ...
    if (container.is_list()) {
        if (!key.is_int())
            throw VshRuntimeError(
                "indice de lista debe ser entero, se obtuvo: " +
                    key.to_string(),
                n.line);
        auto &lst = *container.as_list();
        int64_t idx = key.as_int();
        if (idx < 0) idx += (int64_t)lst.size();
        if (idx < 0 || (size_t)idx >= lst.size())
            throw VshRuntimeError("indice fuera de rango: " +
    ...
    if (container.is_map()) {
        ...
        auto it = m.find(k);
        if (it == m.end())
            throw VshRuntimeError("clave no encontrada en mapa: '" + k + "'",
                                  n.line);
        return it->second;
    }
    ...
    throw VshRuntimeError("indexado no soportado para " + container.to_string(),
                          n.line);
}

Value VshInterpreter::eval_list_lit(const AstNode &n,
                                    std::shared_ptr<VshEnv> env) {
    auto lst = std::make_shared<std::vector<Value>>();
    for (auto &e : n.args)
        lst->push_back(eval(e, env));
    return Value(lst);
}

Value VshInterpreter::eval_map_lit(const AstNode &n,
                                   std::shared_ptr<VshEnv> env) {
...
void VshInterpreter::exec_let(const AstNode &n, std::shared_ptr<VshEnv> env) {
    ...
    if (!n.type_hint.empty()) {
        check_type_hint(n.type_hint, v, "variable '" + n.name + "'", n.line,
                        env);
    }
    env->define(n.name, std::move(v));
}

void VshInterpreter::exec_fn_decl(const AstNode &n,
                                  std::shared_ptr<VshEnv> env) {
    auto fn = std::make_shared<VshFunction>();
    fn->name = n.name;
    fn->params = n.params;
...
void VshInterpreter::exec_for_in(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    Value iter = eval(n.cond, env); // n.cond reutilizado como iterable
    auto loop_env = env->make_child();

    auto run_body = [&](Value v) {
        loop_env->vars[n.name] =
            std::move(v); // define/redefine la var del bucle
        try {
            exec(n.body, loop_env);
        } catch (ContinueSignal &) {
            /* siguiente iteracion */
        }
    };
    ...
    try {
        if (iter.is_list()) {
            for (auto &v : *iter.as_list())
                run_body(v);
        } else if (iter.is_map()) {
            for (auto &[k, _] : *iter.as_map())
                run_body(Value(k));
        } else if (iter.is_string()) {
            for (char c : iter.as_string())
                run_body(Value(std::string(1, c)));
        } else {
            throw VshRuntimeError(
                "'for in' requiere un iterable (lista, mapa o string)", n.line,
                n.col);
        }
    } catch (BreakSignal &) {
        /* terminar el bucle */
    ...
    try {
        ...
        if (!n.catch_type_hint.empty()) {
            ...
            if (e.thrown_val.is_instance()) {
                ...
                while (klass) {
                    if (klass->name == n.catch_type_hint) {
                        matches = true;
                        break;
                    }
                    klass = klass->parent;
        ...
        catch_env->define(
            n.name, e.thrown_val); // la variable catch recibe la instancia
        exec(n.catch_body, catch_env);
    } catch (VshRuntimeError &e) {
        if (!n.catch_type_hint.empty())
            throw; // catch tipado no captura errores de string
        auto catch_env = env->make_child();
...
std::string
VshInterpreter::resolve_import_path(const std::string &path,
    ...
    if (!importer_file.empty()) {
        ...
        if (!importer_dir.empty()) {
            if (auto r = try_candidate(importer_dir / path); !r.empty())
                return r;
        }
    }
    ...
    if (!exe_dir.empty()) {
        ...
        if (auto r = try_candidate(exe_dir / "stdlib" / path); !r.empty())
            return r;
        ...
        if (!parent.empty()) {
            ...
            if (auto r = try_candidate(parent / "stdlib" / path); !r.empty())
                return r;
            if (auto r =
                    try_candidate(parent / "share" / "vm" / "stdlib" / path);
                !r.empty())
                return r;
        }
    }
...
void VshInterpreter::exec_import(const AstNode &n,
                                 std::shared_ptr<VshEnv> /*env*/) {
    ...
    for (auto &p : import_stack_)
        if (p == resolved)
            throw VshRuntimeError("importacion circular detectada: " + resolved,
                                  n.line);
    import_stack_.push_back(resolved);
    try {
        exec_file(resolved); // ejecuta en el scope global (efecto: define
                             // funciones globales)
    } catch (...) {
...
Value VshInterpreter::call_fn(const Value &callee, std::vector<Value> args,
                              int line) {
...
Value VshInterpreter::eval_field(const AstNode &n,
                                 std::shared_ptr<VshEnv> env) {
    ...
    if (obj.is_instance()) {
        ...
        throw VshRuntimeError("atributo no encontrado: '" + field + "' en " +
                                  inst->klass->name,
                              n.line);
    }
    if (obj.is_class()) {
...
void VshInterpreter::exec_class_decl(const AstNode &n,
                                     std::shared_ptr<VshEnv> env) {
    ...
    if (!n.parent_class.empty()) {
        if (!env->has(n.parent_class))
            throw VshRuntimeError(
                "clase padre no definida: '" + n.parent_class + "'", n.line);
        Value parent_val = env->get(n.parent_class);
        if (!parent_val.is_class())
            throw VshRuntimeError("'" + n.parent_class + "' no es una clase",
                                  n.line);
    ...
    for (auto &child : n.args) {
        if (child->kind == AstKind::FnDecl) {
            ...
            fn->closure_env = env;
            fn->doc =
                extract_docstring(std::static_pointer_cast<AstNode>(fn->body));
...
std::shared_ptr<VshFunction>
VshInterpreter::find_method(const std::shared_ptr<VshClass> &klass,
                            const std::string &name) {
...
Value VshInterpreter::call_method_impl(
    const std::shared_ptr<VshFunction> &method, const Value &self_val,
    std::vector<Value> extra_args, int line) {
    if (method->params.empty())
        throw VshRuntimeError(
            "metodo '" + method->name + "' requiere al menos 'self'", line);
    ...
    if (extra_args.size() < method->params.size() - 1)
        throw VshRuntimeError(
            "metodo '" + method->name + "': argumentos insuficientes", line);
    for (size_t i = 1; i < method->params.size(); ++i) {
        ...
        if (i < method->param_types.size() && !method->param_types[i].empty()) {
            check_type_hint(method->param_types[i], extra_args[ai],
                            "parametro '" + method->params[i] + "'", line,
                            method->closure_env);
        }
...
void VshInterpreter::check_type_hint(const std::string &hint, const Value &val,
    ...
    if (hint == "null")
        ...
        ok = val.is_float();
    else if (hint == "str" || hint == "string")
        ok = val.is_string();
    else if (hint == "list")
        ok = val.is_list();
    else if (hint == "map")
        ok = val.is_map();
    else if (hint == "fn" || hint == "function")
        ok = val.is_function();
    else if (hint == "class")
        ok = val.is_class();
...
void VshInterpreter::run_interactive(ReadlineFn readline_fn) {
    ...
    for (;;) {
        ...
        try {
            ...
            if (ast->args.size() == 1 &&
                ast->args[0]->kind == AstKind::ExprStmt) {
                Value result = eval(ast->args[0]->left, global_);
                // auto-invocar stubs de builtins (ej: 'help' sin () ejecuta
                // help())
                if (result.is_function() && !result.as_fn()->body &&
                    !result.as_fn()->name.empty()) {
            ...
            std::cerr << e.thrown_val.as_instance()->klass->name << ": "
                      << e.what() << '\n';
        } catch (VshRuntimeError &e) {
...
void VshInterpreter::register_builtins() {
    ...
    builtins_["print"] = [](std::vector<Value> args) -> Value {
        ...
        // El flush ocurre por threshold (64KB) o explicitamente con
        // flush_output().
        return {};
    };
    ...
    builtins_["println"] = [](std::vector<Value> args) -> Value {
        ...
        buffered_write_char('\n');
        flush_out_buffer(); // println es para output del usuario, flush es
                            // razonable
        return {};
    };
    ...
    builtins_["int"] = [](std::vector<Value> args) -> Value {
        ...
        if (args[0].is_string()) {
            // Soporte de base opcional (segundo argumento) y auto-deteccion
            // de prefijo "0x"/"0b"/"0o" cuando no se da base.  Sin esto el
            // cliente del debugger tiene que parsear hex a mano.
            int base = 0; // 0 = auto-detect via std::stoll
            if (args.size() >= 2 && args[1].is_int()) {
                base = static_cast<int>(args[1].as_int());
                if (base != 0 && (base < 2 || base > 36)) {
                    throw VshRuntimeError("int(): base fuera de rango [2..36]");
                }
            }
            const std::string &s = args[0].as_string();
            // std::stoll con base 0 auto-detecta "0x" como hex.  Anadimos
            // soporte explicito para prefijos "0b" (binario) y "0o" (octal)
            // cuando base = 0, ya que stoll no los reconoce.
            std::string trimmed = s;
            size_t pos0 = trimmed.find_first_not_of(" \t\n\r");
            if (pos0 == std::string::npos) {
                throw VshRuntimeError("int(): cadena vacia");
            }
            trimmed = trimmed.substr(pos0);
            int sign = 1;
            ...
            try {
                ...
                int64_t v = static_cast<int64_t>(
                    std::stoll(trimmed, &consumed, eff_base));
                return Value(sign * v);
            } catch (...) {
                throw VshRuntimeError("int(): no se puede convertir '" + s +
                                      "'");
            }
        }
    ...
    builtins_["float"] = [](std::vector<Value> args) -> Value {
        ...
        if (args[0].is_string()) {
            try {
                ...
                throw VshRuntimeError("float(): no se puede convertir");
            }
        }
    ...
    builtins_["len"] = [](std::vector<Value> args) -> Value {
        ...
        if (args[0].is_string())
            return Value(int64_t(args[0].as_string().size()));
    ...
    builtins_["values"] = [](std::vector<Value> args) -> Value {
        ...
        auto lst = std::make_shared<std::vector<Value>>();
        for (auto &[_, v] : *args[0].as_map())
            lst->push_back(v);
        return Value(lst);
    };
    builtins_["contains"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: contains(coleccion, valor)");
        if (args[0].is_list()) {
        ...
        if (args[0].is_string()) {
            return Value(args[0].as_string().find(args[1].to_string()) !=
                         std::string::npos);
        }
        throw VshRuntimeError("contains() no soportado para " +
                              args[0].to_string());
    };
    ...
    builtins_["split"] = [](std::vector<Value> args) -> Value {
        ...
        if (sep.empty()) {
            for (char c : s)
                lst->push_back(Value(std::string(1, c)));
            return Value(lst);
        }
        size_t pos = 0, found;
        while ((found = s.find(sep, pos)) != std::string::npos) {
            lst->push_back(Value(s.substr(pos, found - pos)));
            pos = found + sep.size();
        }
    ...
    builtins_["trim"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string())
            throw VshRuntimeError("Uso: trim(string)");
        std::string s = args[0].as_string();
        auto l = s.find_first_not_of(" \t\r\n");
        auto r = s.find_last_not_of(" \t\r\n");
        return Value(l == std::string::npos ? std::string{}
                                            : s.substr(l, r - l + 1));
    };
    builtins_["upper"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string())
            throw VshRuntimeError("Uso: upper(string)");
        std::string s = args[0].as_string();
    ...
    builtins_["lower"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string())
            throw VshRuntimeError("Uso: lower(string)");
        std::string s = args[0].as_string();
        std::transform(s.begin(), s.end(), s.begin(),
    ...
    builtins_["replace"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 3 || !args[0].is_string())
            throw VshRuntimeError("Uso: replace(s, viejo, nuevo)");
        std::string s = args[0].as_string(), viejo = args[1].as_string(),
                    nuevo = args[2].as_string();
        size_t pos = 0;
        while ((pos = s.find(viejo, pos)) != std::string::npos) {
            s.replace(pos, viejo.size(), nuevo);
            pos += nuevo.size();
        }
        return Value(s);
    };
    builtins_["substr"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: substr(s, inicio [, len])");
        const auto &s = args[0].as_string();
        int64_t start = args[1].as_int();
        if (start < 0) start += (int64_t)s.size();
        size_t len =
            args.size() >= 3 ? (size_t)args[2].as_int() : std::string::npos;
    ...
    builtins_["range"] = [](std::vector<Value> args) -> Value {
        ...
        if (args.size() == 1) {
            end = args[0].is_int() ? args[0].as_int()
                                   : (int64_t)args[0].as_float();
        } else if (args.size() >= 2) {
            start = args[0].is_int() ? args[0].as_int()
                                     : (int64_t)args[0].as_float();
            end = args[1].is_int() ? args[1].as_int()
                                   : (int64_t)args[1].as_float();
        }
        if (args.size() >= 3)
            step = args[2].is_int() ? args[2].as_int()
                                    : (int64_t)args[2].as_float();
    ...
    builtins_["basename"] = [](std::vector<Value> args) -> Value {
        if (args.empty()) return Value(std::string{});
        return Value(
            std::filesystem::path(args[0].as_string()).filename().string());
    };
    ...
    builtins_["stem"] = [](std::vector<Value> args) -> Value {
        if (args.empty()) return Value(std::string{});
        return Value(
            std::filesystem::path(args[0].as_string()).stem().string());
    };
    builtins_["extension"] = [](std::vector<Value> args) -> Value {
        if (args.empty()) return Value(std::string{});
        return Value(
            std::filesystem::path(args[0].as_string()).extension().string());
    };
    builtins_["glob"] = [](std::vector<Value> args) -> Value {
        ...
        try {
            ...
            auto match_pat = [](const std::string &name,
    ...
    builtins_["read_file"] = [](std::vector<Value> args) -> Value {
        ...
        if (!f)
            throw VshRuntimeError("read_file(): no se puede abrir: " +
                                  args[0].as_string());
    ...
    builtins_["write_file"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: write_file(ruta, contenido)");
        std::ofstream f(args[0].as_string());
        if (!f)
            throw VshRuntimeError("write_file(): no se puede abrir: " +
                                  args[0].as_string());
        f << args[1].to_string();
        return {};
    };
    ...
    builtins_["rename_path"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: rename_path(origen, destino)");
        std::error_code ec;
    ...
    builtins_["copy_file"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: copy_file(origen, destino)");
        std::error_code ec;
    ...
    builtins_["normpath"] = [](std::vector<Value> args) -> Value {
        ...
        return Value(std::filesystem::path(args[0].as_string())
                         .lexically_normal()
                         .string());
    };
...
#endif
        if (!f)
            throw VshRuntimeError("shell_ex(): no se pudo ejecutar: " + cmd);
        char buf[256];
        while (fgets(buf, sizeof(buf), f))
            out += buf;
        if (!out.empty() && out.back() == '\n') out.pop_back();
    ...
    builtins_["sleep"] = [](std::vector<Value> args) -> Value {
        int64_t ms = 0;
        if (!args.empty())
            ms = args[0].is_int() ? args[0].as_int()
                                  : (int64_t)(args[0].as_number());
    ...
    builtins_["input"] = [](std::vector<Value> args) -> Value {
        ...
        if (!std::getline(std::cin, line)) {
            // EOF o error: devolver null para que el script pueda detectarlo.
            return Value(); // Value() = null en VSH
        }
        return Value(line);
    };
    builtins_["exit"] = [](std::vector<Value> args) -> Value {
        int code = args.empty()
                       ? 0
                       : (int)(args[0].is_int() ? args[0].as_int()
                                                : (int64_t)args[0].as_number());
        std::exit(code);
        return {};
    };
    builtins_["assert"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("assert(): se requiere condicion");
        if (!args[0].truthy()) {
            std::string msg =
                args.size() >= 2 ? args[1].to_string() : "asercion fallida";
            throw VshRuntimeError(msg);
        }
        return {};
    };
    builtins_["error"] = [](std::vector<Value> args) -> Value {
        std::string msg =
            args.empty() ? "error en el script" : args[0].to_string();
        throw VshRuntimeError(msg);
        return {};
    };
    ...
    builtins_["min"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("min() requiere 2 argumentos");
        return args[0] < args[1] ? args[0] : args[1];
    };
    builtins_["max"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("max() requiere 2 argumentos");
        return args[0] > args[1] ? args[0] : args[1];
    };
    ...
    builtins_["pow"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("pow() requiere 2 argumentos");
    ...
    // ---- utilidades de tipo ----
    builtins_["is_null"] = [](std::vector<Value> args) -> Value {
        return Value(args.empty() || args[0].is_null());
    };
    builtins_["is_bool"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_bool());
    };
    builtins_["is_int"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_int());
    };
    builtins_["is_float"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_float());
    };
    builtins_["is_str"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_string());
    };
    builtins_["is_list"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_list());
    };
    builtins_["is_map"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_map());
    };
    builtins_["is_fn"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_function());
    };
    builtins_["is_class"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_class());
    };
    builtins_["is_instance"] = [](std::vector<Value> args) -> Value {
        return Value(!args.empty() && args[0].is_instance());
    };

    // ---- OOP ----
    builtins_["classname"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("classname() requiere argumento");
    ...
    builtins_["isinstance"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: isinstance(obj, clase)");
        if (!args[0].is_instance()) return Value(false);
        if (!args[1].is_class())
            throw VshRuntimeError("isinstance(): segundo arg debe ser clase");
    ...
    builtins_["help"] = [](std::vector<Value> args) -> Value {
        if (args.empty()) {
            std::cout << "Uso: help(funcion_o_clase)  ->  muestra firma y "
                         "docstring\n";
            return Value{};
        }
    ...
    builtins_["rand_seed"] = [](std::vector<Value> args) -> Value {
        uint64_t seed =
            args.empty()
                ...
                : (uint64_t)(args[0].is_int() ? args[0].as_int()
                                              : (int64_t)args[0].as_number());
        g_vsh_rng.seed(seed);
        return {};
    };
    builtins_["rand"] = [](std::vector<Value> /*args*/) -> Value {
        return Value(
            std::uniform_real_distribution<double>(0.0, 1.0)(g_vsh_rng));
    };
    builtins_["rand_int"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2) throw VshRuntimeError("Uso: rand_int(min, max)");
        int64_t lo =
            args[0].is_int() ? args[0].as_int() : (int64_t)args[0].as_number();
        int64_t hi =
            args[1].is_int() ? args[1].as_int() : (int64_t)args[1].as_number();
    ...
    builtins_["rand_choice"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: rand_choice(lista)");
        auto &lst = *args[0].as_list();
        if (lst.empty()) throw VshRuntimeError("rand_choice(): lista vacia");
        size_t idx =
            std::uniform_int_distribution<size_t>(0, lst.size() - 1)(g_vsh_rng);
        return lst[idx];
    };
    builtins_["rand_shuffle"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: rand_shuffle(lista)");
    ...
    builtins_["sort"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: sort(lista)");
        auto copy = std::make_shared<std::vector<Value>>(*args[0].as_list());
    ...
    builtins_["reverse"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: reverse(lista)");
        auto copy = std::make_shared<std::vector<Value>>(*args[0].as_list());
    ...
    builtins_["slice"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: slice(lista, inicio [, fin])");
        ...
        for (int64_t i = lo; i < hi; ++i)
            out->push_back(src[(size_t)i]);
        return Value(out);
    };
    builtins_["flat"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: flat(lista)");
        auto out = std::make_shared<std::vector<Value>>();
        for (auto &v : *args[0].as_list()) {
            if (v.is_list())
                for (auto &e : *v.as_list())
                    out->push_back(e);
            else
                out->push_back(v);
        }
    ...
    builtins_["zip"] = [](std::vector<Value> args) -> Value {
        ...
        auto &a = *args[0].as_list();
        auto &b = *args[1].as_list();
    ...
    builtins_["enumerate"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: enumerate(lista)");
        auto out = std::make_shared<std::vector<Value>>();
        int64_t i = 0;
        for (auto &v : *args[0].as_list()) {
            auto pair = std::make_shared<std::vector<Value>>();
    ...
    builtins_["sum"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: sum(lista)");
        ...
        for (auto &v : *args[0].as_list()) {
            if (v.is_float()) {
                ...
                throw VshRuntimeError("sum(): elemento no numerico: " +
                                      v.to_string());
        }
    ...
    builtins_["any_of"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: any_of(lista)");
        for (auto &v : *args[0].as_list())
            if (v.truthy()) return Value(true);
        return Value(false);
    };
    builtins_["all_of"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: all_of(lista)");
        for (auto &v : *args[0].as_list())
            if (!v.truthy()) return Value(false);
        return Value(true);
    };
    builtins_["unique"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_list())
            throw VshRuntimeError("Uso: unique(lista)");
        auto out = std::make_shared<std::vector<Value>>();
        for (auto &v : *args[0].as_list()) {
            bool found = false;
            for (auto &u : *out)
    ...
    builtins_["index_of"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_list())
            throw VshRuntimeError("Uso: index_of(lista, valor)");
        auto &lst = *args[0].as_list();
        for (size_t i = 0; i < lst.size(); ++i)
            if (lst[i] == args[1]) return Value(int64_t(i));
    ...
    builtins_["lstrip"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string())
            throw VshRuntimeError("Uso: lstrip(s)");
        std::string s = args[0].as_string();
    ...
    builtins_["rstrip"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string())
            throw VshRuntimeError("Uso: rstrip(s)");
        std::string s = args[0].as_string();
    ...
    builtins_["find_str"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: find_str(s, sub [, inicio])");
    ...
    builtins_["count_str"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: count_str(s, sub)");
        const auto &s = args[0].as_string(), &sub = args[1].as_string();
    ...
    builtins_["repeat"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: repeat(s, n)");
        std::string s = args[0].as_string();
    ...
    builtins_["pad_left"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: pad_left(s, n [, char])");
        std::string s = args[0].as_string();
        int64_t n = args[1].as_int();
        char ch = args.size() >= 3 && args[2].is_string() &&
                          !args[2].as_string().empty()
                      ? args[2].as_string()[0]
                      : ' ';
    ...
    builtins_["pad_right"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string())
            throw VshRuntimeError("Uso: pad_right(s, n [, char])");
        std::string s = args[0].as_string();
        int64_t n = args[1].as_int();
        char ch = args.size() >= 3 && args[2].is_string() &&
                          !args[2].as_string().empty()
    ...
    builtins_["clamp"] = [](std::vector<Value> args) -> Value {
        ...
        if (args[0].is_int() && args[1].is_int() && args[2].is_int()) {
            int64_t x = args[0].as_int(), lo = args[1].as_int(),
                    hi = args[2].as_int();
            return Value(x < lo ? lo : (x > hi ? hi : x));
        }
        double x = args[0].as_number(), lo = args[1].as_number(),
               hi = args[2].as_number();
        return Value(x < lo ? lo : (x > hi ? hi : x));
    };
    builtins_["sign"] = [](std::vector<Value> args) -> Value {
        ...
        if (args[0].is_int()) {
            ...
            return Value(x > 0 ? int64_t(1)
                               : (x < 0 ? int64_t(-1) : int64_t(0)));
        }
        double x = args[0].as_number();
        return Value(x > 0.0 ? 1.0 : (x < 0.0 ? -1.0 : 0.0));
    };
    ...
    builtins_["time_now"] = [](std::vector<Value> args) -> Value {
        ...
        const char *fmt =
            args.empty() ? "%Y-%m-%d %H:%M:%S" : args[0].as_string().c_str();
        char buf[128];
    ...
    builtins_["setenv"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: setenv(nombre, valor)");
#if defined(_WIN32)
    ...
    builtins_["format"] = [](std::vector<Value> args) -> Value {
        ...
        for (size_t i = 0; i < tmpl.size(); ++i) {
            if (tmpl[i] == '{' && i + 1 < tmpl.size()) {
                size_t j = i + 1;
                while (j < tmpl.size() && tmpl[j] != '}')
                    ++j;
                if (j < tmpl.size()) {
                    std::string key = tmpl.substr(i + 1, j - i - 1);
                    size_t idx =
                        key.empty() ? arg_idx++ : (size_t)std::stoul(key) + 1;
    ...
    builtins_["json_str"] = [](std::vector<Value> args) -> Value {
        if (args.empty()) return Value(std::string("null"));
        std::function<std::string(const Value &)> to_json =
    ...
    // ffi_open(ruta) -> handle: carga la libreria dinamica y devuelve su id
    // opaco.
    ...
    // ffi_call_sym(ruta, nombre [, arg1, ...]): atajo open+sym+call+close en
    // una llamada.
    builtins_["ffi_call_sym"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError(
                "Uso: ffi_call_sym(ruta, nombre [, arg1, ...])");
    ...
    // ffi_str(ptr_int) -> string: interpreta el entero como char* y copia el
    // contenido. Util para leer strings de retorno de funciones Win32/POSIX.
    ...
    // colorize(texto, fg [, bg [, negrita]]): envuelve el texto con escapes de
    // color. fg/bg son nombres de color en minusculas: "red", "green",
    // "br_blue", etc.
    builtins_["colorize"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: colorize(texto, fg [, bg [, bold]])");
        static const std::unordered_map<std::string, int> fg_map = {
            {"black", 30},   {"red", 31},        {"green", 32},
            {"yellow", 33},  {"blue", 34},       {"magenta", 35},
            {"cyan", 36},    {"white", 37},      {"br_black", 90},
            {"br_red", 91},  {"br_green", 92},   {"br_yellow", 93},
            {"br_blue", 94}, {"br_magenta", 95}, {"br_cyan", 96},
            {"br_white", 97}};
        static const std::unordered_map<std::string, int> bg_map = {
            {"black", 40},    {"red", 41},         {"green", 42},
            {"yellow", 43},   {"blue", 44},        {"magenta", 45},
            {"cyan", 46},     {"white", 47},       {"br_black", 100},
            {"br_red", 101},  {"br_green", 102},   {"br_yellow", 103},
            {"br_blue", 104}, {"br_magenta", 105}, {"br_cyan", 106},
            {"br_white", 107}};
        ...
        if (it != fg_map.end())
            out += "\033[" + std::to_string(it->second) + "m";
        if (args.size() >= 3) {
            ...
            if (jt != bg_map.end())
                out += "\033[" + std::to_string(jt->second) + "m";
        }
    ...
    //   red()          -> "\033[31m"  (solo el codigo, sin reset)
    {
        ...
        for (const auto &entry : color_list) {
            ...
            builtins_[entry.first] = [seq](std::vector<Value> args) -> Value {
                return args.empty()
                           ? Value(seq)
                           : Value(seq + args[0].to_string() + "\033[0m");
            };
        }
    }
    ...
    builtins_["ansi_cursor_pos"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: ansi_cursor_pos(fila, col)");
    ...
    // strip_ansi(texto): elimina todas las secuencias de escape ANSI de un
    // string.
    ...
    builtins_["tcp_connect"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: tcp_connect(host, port)");
        ensure_wsa();
        ...
        vsh_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
        if (!vsh_sock_valid(fd))
            throw VshRuntimeError("tcp_connect: error creando socket");
        if (connect(fd, reinterpret_cast<struct sockaddr *>(&addr),
                    sizeof(addr)) != 0) {
            vsh_sock_close(fd);
            throw VshRuntimeError("tcp_connect: no se pudo conectar a " + host +
                                  ":" + std::to_string(port));
        }
    ...
    builtins_["tcp_listen"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: tcp_listen(port [, backlog])");
        ensure_wsa();
        uint16_t port = (uint16_t)args[0].as_int();
        int backlog = args.size() >= 2 ? (int)args[1].as_int() : 10;
        vsh_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
        if (!vsh_sock_valid(fd))
            throw VshRuntimeError("tcp_listen: error creando socket");
        int opt = 1;
        ...
        if (bind(fd, reinterpret_cast<struct sockaddr *>(&addr),
                 sizeof(addr)) != 0) {
            vsh_sock_close(fd);
            throw VshRuntimeError("tcp_listen: bind fallido en puerto " +
                                  std::to_string(port));
        }
        listen(fd, backlog);
        return Value(vsh_alloc_sock(fd, false));
    ...
    builtins_["tcp_accept"] = [](std::vector<Value> args) -> Value {
        ...
        vsh_sock_t client =
            accept(s->fd, reinterpret_cast<struct sockaddr *>(&remote), &len);
        if (!vsh_sock_valid(client))
            throw VshRuntimeError("tcp_accept: error al aceptar conexion");
        return Value(vsh_alloc_sock(client, false));
    };
    builtins_["socket_send"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: socket_send(handle, datos)");
        int64_t id = args[0].as_int();
        auto *s = vsh_get_sock(id);
        if (!s) throw VshRuntimeError("socket_send: handle invalido");
        std::string data = args[1].to_string();
        // perf+correctness: bucle send-all para evitar
        // envios parciales que rompen el framing length-prefix de
        // protocolos como el del debugger TCP.  TCP NORMALMENTE no
        // hace partial send para mensajes pequenos en buffer libre,
        // pero la API NO lo garantiza: bajo presion del buffer del
        // peer (clientes lentos) `send` puede devolver menos bytes
        // que los pedidos.  El loop garantiza que el frame completo
        // sale en una sola llamada VSH `socket_send`.
        size_t total = 0;
        const size_t need = data.size();
        while (total < need) {
            int chunk;
            if (s->ssl)
                chunk = SSL_write(s->ssl, data.c_str() + total,
                                  (int)(need - total));
            else
                chunk = (int)send(s->fd, data.c_str() + total,
                                  (int)(need - total), 0);
            if (chunk < 0)
                throw VshRuntimeError("socket_send: error al enviar datos");
            if (chunk == 0) break; // peer cerro
    ...
    builtins_["socket_recv"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: socket_recv(handle [, maxlen])");
        int64_t id = args[0].as_int();
        auto *s = vsh_get_sock(id);
        ...
        int n;
        if (s->ssl)
            n = SSL_read(s->ssl, &buf[0], maxlen);
        else
            n = (int)recv(s->fd, &buf[0], (size_t)maxlen, 0);
    ...
    builtins_["socket_poll"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: socket_poll(handle, ms)");
        int64_t id = args[0].as_int();
        int64_t ms = args[1].as_int();
        auto *s = vsh_get_sock(id);
        if (!s) throw VshRuntimeError("socket_poll: handle invalido");
        fd_set rfds;
        FD_ZERO(&rfds);
        FD_SET(s->fd, &rfds);
        struct timeval tv;
        ...
        int rc = select((int)(s->fd + 1), &rfds, nullptr, nullptr, &tv);
        if (rc < 0) return Value((int64_t)-1);
        return Value((int64_t)(rc > 0 ? 1 : 0));
    };
    builtins_["udp_socket"] = [](std::vector<Value> args) -> Value {
        ...
        if (!vsh_sock_valid(fd))
            throw VshRuntimeError("udp_socket: error creando socket UDP");
        return Value(vsh_alloc_sock(fd, true));
    };
    builtins_["udp_bind"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: udp_bind(handle, port)");
        int64_t id = args[0].as_int();
        auto *s = vsh_get_sock(id);
        if (!s || !s->is_udp)
            throw VshRuntimeError("udp_bind: handle invalido o no es UDP");
        uint16_t port = (uint16_t)args[1].as_int();
        struct sockaddr_in addr{};
        memset(&addr, 0, sizeof(addr));
        addr.sin_family = AF_INET;
        ...
        if (bind(s->fd, reinterpret_cast<struct sockaddr *>(&addr),
                 sizeof(addr)) != 0)
            throw VshRuntimeError("udp_bind: bind fallido en puerto " +
                                  std::to_string(port));
        return {};
    };
    builtins_["udp_sendto"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 4)
            throw VshRuntimeError("Uso: udp_sendto(handle, datos, host, port)");
        int64_t id = args[0].as_int();
        auto *s = vsh_get_sock(id);
        if (!s || !s->is_udp)
            throw VshRuntimeError("udp_sendto: handle invalido o no es UDP");
        std::string data = args[1].to_string();
    ...
    builtins_["udp_recvfrom"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: udp_recvfrom(handle [, maxlen])");
        int64_t id = args[0].as_int();
        auto *s = vsh_get_sock(id);
        if (!s || !s->is_udp)
            throw VshRuntimeError("udp_recvfrom: handle invalido o no es UDP");
        int maxlen = args.size() >= 2 ? (int)args[1].as_int() : 65536;
        std::string buf(maxlen, '\0');
        struct sockaddr_in from{};
        socklen_t fromlen = sizeof(from);
        int n =
            (int)recvfrom(s->fd, &buf[0], (size_t)maxlen, 0,
    ...
    builtins_["tls_connect"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError("Uso: tls_connect(host, port)");
        ensure_wsa();
        ensure_ssl_ctx();
        std::string host = args[0].as_string();
        uint16_t port = (uint16_t)args[1].as_int();
        ...
        vsh_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
        if (!vsh_sock_valid(fd))
            throw VshRuntimeError("tls_connect: error creando socket");
        if (connect(fd, reinterpret_cast<struct sockaddr *>(&addr),
                    sizeof(addr)) != 0) {
            vsh_sock_close(fd);
        ...
        if (SSL_connect(ssl) <= 0) {
            SSL_free(ssl);
            vsh_sock_close(fd);
            throw VshRuntimeError("tls_connect: fallo TLS handshake con " +
                                  host);
        }
    ...
    builtins_["http_post"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: http_post(url, body [, content_type])");
        bool use_tls;
        std::string path;
        auto [host, port] = vsh_parse_url(args[0].as_string(), use_tls, path);
        std::string body = args.size() >= 2 ? args[1].to_string() : "";
        std::string ct = args.size() >= 3 ? args[2].as_string()
                                          : "application/x-www-form-urlencoded";
        return Value(
            vsh_http_request("POST", host, port, path, false, body, ct).body);
    };
    builtins_["https_post"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError(
                "Uso: https_post(url, body [, content_type])");
        bool use_tls;
        std::string path;
        auto [host, port] = vsh_parse_url(args[0].as_string(), use_tls, path);
        std::string body = args.size() >= 2 ? args[1].to_string() : "";
        std::string ct = args.size() >= 3 ? args[2].as_string()
                                          : "application/x-www-form-urlencoded";
        return Value(
            vsh_http_request("POST", host, port, path, true, body, ct).body);
    };
    builtins_["http_put"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: http_put(url, body [, content_type])");
        bool use_tls;
        std::string path;
        auto [host, port] = vsh_parse_url(args[0].as_string(), use_tls, path);
        std::string body = args.size() >= 2 ? args[1].to_string() : "";
        std::string ct = args.size() >= 3 ? args[2].as_string()
                                          : "application/x-www-form-urlencoded";
        return Value(
            vsh_http_request("PUT", host, port, path, false, body, ct).body);
    };
    builtins_["https_put"] = [](std::vector<Value> args) -> Value {
        if (args.empty())
            throw VshRuntimeError("Uso: https_put(url, body [, content_type])");
        bool use_tls;
        std::string path;
        auto [host, port] = vsh_parse_url(args[0].as_string(), use_tls, path);
        std::string body = args.size() >= 2 ? args[1].to_string() : "";
        std::string ct = args.size() >= 3 ? args[2].as_string()
                                          : "application/x-www-form-urlencoded";
        return Value(
            vsh_http_request("PUT", host, port, path, true, body, ct).body);
    };
    ...
    builtins_["http_request"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2)
            throw VshRuntimeError(
                "Uso: http_request(method, url [, body [, content_type]])");
        ...
        std::string ct =
            args.size() >= 4 ? args[3].as_string() : "application/json";
    ...
    // __name__ == "__main__"   -> script ejecutado directamente (--script o
    // REPL script)
    ...
    // ===== JSON parse/stringify (anadido para soporte del cliente VSH del
    // debugger) ===== json_parse(str) -> Value:  parsea texto JSON a
    // Null/Bool/Int/Float/String/List/Map. json_stringify(value)        -> str:
    // serializa el mismo arbol. json_stringify(value, indent) -> str: con
    // indentacion (entero >= 0). En errores de parse lanza VshRuntimeError con
    // mensaje claro.
    {
        // Funcion recursiva: nlohmann::json -> Value (VSH).
        // No es lambda por la auto-referencia recursiva.
        struct JsonConv {
            static Value to_vsh(const nlohmann::json &j) {
                ...
                if (j.is_array()) {
                    auto lp = std::make_shared<std::vector<Value>>();
                    lp->reserve(j.size());
                    for (const auto &e : j)
                        lp->push_back(to_vsh(e));
                    return Value(lp);
                }
                if (j.is_object()) {
                    auto mp = std::make_shared<
                        std::unordered_map<std::string, Value>>();
                    mp->reserve(j.size());
                    for (auto it = j.begin(); it != j.end(); ++it) {
                        (*mp)[it.key()] = to_vsh(it.value());
                    }
                    return Value(mp);
                }
                return Value();
            ...
            static nlohmann::json from_vsh(const Value &v) {
                ...
                if (v.is_list()) {
                    ...
                    for (const auto &e : *v.as_list())
                        a.push_back(from_vsh(e));
                    return a;
                }
                if (v.is_map()) {
                    nlohmann::json o = nlohmann::json::object();
                    for (const auto &kv : *v.as_map())
                        o[kv.first] = from_vsh(kv.second);
                    return o;
                }
                // Function/Class/Instance -> string descriptiva (no
                // serializable).
                return v.to_string();
            }
        };

        builtins_["json_parse"] = [](std::vector<Value> args) -> Value {
            if (args.empty() || !args[0].is_string()) {
                throw VshRuntimeError("Uso: json_parse(str)");
            }
            try {
                auto j = nlohmann::json::parse(args[0].as_string());
                return JsonConv::to_vsh(j);
            } catch (const nlohmann::json::exception &e) {
                throw VshRuntimeError(std::string("json_parse: ") + e.what());
            }
        };

        builtins_["json_stringify"] = [](std::vector<Value> args) -> Value {
            if (args.empty()) {
                throw VshRuntimeError("Uso: json_stringify(value [, indent])");
            }
            int indent = -1; // -1 = sin pretty-print
            if (args.size() >= 2) {
                if (!args[1].is_int()) {
                    throw VshRuntimeError(
                        "json_stringify: 'indent' debe ser int");
                }
                indent = static_cast<int>(args[1].as_int());
                if (indent < 0) indent = -1;
            }
            try {
                auto j = JsonConv::from_vsh(args[0]);
                return Value(j.dump(indent));
            } catch (const std::exception &e) {
                throw VshRuntimeError(std::string("json_stringify: ") +
                                      e.what());
            }
        };
    }

    // ===== Helpers binario little-endian (utiles para protocolo length-prefix
    // u32) ===== u32_le_pack(n)        -> string de 4 bytes con n en LE.
    // u32_le_unpack(s)      -> int64 (lee primeros 4 bytes de s en LE).
    // bytes_concat(a, b)    -> string con a ++ b (binary safe).
    // bytes_len(s)          -> int64 con longitud en bytes (== len(s)).
    // bytes_substr(s, off, len) -> string (substring binary-safe).
    builtins_["u32_le_pack"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_int()) {
            throw VshRuntimeError("Uso: u32_le_pack(int)");
        }
        uint32_t n = static_cast<uint32_t>(args[0].as_int());
        std::string out(4, '\0');
    ...
    builtins_["u32_le_unpack"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string()) {
            throw VshRuntimeError("Uso: u32_le_unpack(str)");
        }
        const std::string &s = args[0].as_string();
        if (s.size() < 4) {
            throw VshRuntimeError("u32_le_unpack: necesita al menos 4 bytes");
        }
        uint32_t n = (static_cast<uint32_t>(static_cast<uint8_t>(s[0]))) |
    ...
    builtins_["bytes_concat"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 2 || !args[0].is_string() || !args[1].is_string()) {
            throw VshRuntimeError("Uso: bytes_concat(str_a, str_b)");
        }
        std::string r = args[0].as_string();
        r += args[1].as_string();
        return Value(r);
    };
    builtins_["bytes_len"] = [](std::vector<Value> args) -> Value {
        if (args.empty() || !args[0].is_string()) {
            throw VshRuntimeError("Uso: bytes_len(str)");
        }
        return Value(static_cast<int64_t>(args[0].as_string().size()));
    };
    builtins_["bytes_substr"] = [](std::vector<Value> args) -> Value {
        if (args.size() < 3 || !args[0].is_string() || !args[1].is_int() ||
            !args[2].is_int()) {
            throw VshRuntimeError("Uso: bytes_substr(str, off, len)");
        }
        const std::string &s = args[0].as_string();
        int64_t off = args[1].as_int();
        int64_t len = args[2].as_int();
        if (off < 0 || off > static_cast<int64_t>(s.size())) {
            throw VshRuntimeError("bytes_substr: offset fuera de rango");
        }
        if (len < 0) len = 0;
        if (off + len > static_cast<int64_t>(s.size())) {
            len = static_cast<int64_t>(s.size()) - off;
        }
        return Value(
            s.substr(static_cast<size_t>(off), static_cast<size_t>(len)));
    };
    ...
    // ---- exponer todos los builtins en el scope global como stubs (estilo
    // Python) ---- Esto permite usar 'help' sin parentesis igual que en Python
