 ...
 * Estas funciones actuan como puente entre el REPL del CLI y el motor de
 * ejecucion de la VM, encolando operaciones con soporte de cancelacion.
 */
 ...
 */
static std::string execute_on_engine(const std::string &cmd,
...
std::future<std::string> run_command_async(const std::string &cmd,
    ...
    return g_pool->submit(
        [cmd, cancel_token]() { return execute_on_engine(cmd, cancel_token); });
}
} // namespace runtime
