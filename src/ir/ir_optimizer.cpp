...
namespace ir {
            ...
            for (const auto &blk: fn.blocks) {
                for (const auto &ins: blk.instrs) {
                    ...
                    switch (ins.op) {
                        ...
                        case IrOp::BR_COND:
                        case IrOp::NOP:
                        case IrOp::CONST: safe_consumer = true;
                            break;
    ...
    namespace {
        ...
        bool sr_mem2reg_object(
            ...
            auto cur = [&](uint32_t off) -> IrValueId {
                ...
                return (it != stack.end() && !it->second.empty())
                           ? it->second.back()
                           : IR_NO_VALUE;
            };
            ...
            for (size_t bi = 0; bi < fn.blocks.size() && ok; ++bi) {
                ...
                for (size_t ii = 0; ii < b.instrs.size() && ok; ++ii) {
                    ...
                    if (in.op == IrOp::ADD && in.operands.size() == 2 &&
                        ...
                        IrValueId other = (in.operands[0] == obj)
                                              ? in.operands[1]
                                              : in.operands[0];
                        uint64_t k;
            ...
            for (size_t bi = 0; bi < fn.blocks.size() && ok; ++bi) {
                ...
                for (size_t ii = 0; ii < b.instrs.size() && ok; ++ii) {
                    ...
                    if (in.op == IrOp::ADD && in.operands.size() == 2 &&
                        ...
                        IrValueId other = (in.operands[0] == base)
                                              ? in.operands[1]
                                              : in.operands[0];
                        uint64_t k;
                        if (!const_value_of(other, k)) {
            ...
            for (auto &ins: bb.instrs) {
                switch (ins.op) {
                    ...
                    case IrOp::FTRUNC: {
                        ...
                        double     in_v   = is_f32
                                                ? static_cast<double>(bits_to_f32(
                                                    static_cast<uint32_t>(c0)))
                                                : bits_to_f64(static_cast<uint64_t>(c0));
                        double out_v = 0.0;
                        switch (ins.op) {
                            case IrOp::FSQRT: out_v = std::sqrt(in_v);
                                break;
                            case IrOp::FABS: out_v = std::fabs(in_v);
                                break;
                            case IrOp::FNEG: out_v = -in_v;
                                break;
                            case IrOp::FFLOOR: out_v = std::floor(in_v);
                                break;
                            case IrOp::FCEIL: out_v = std::ceil(in_v);
                                break;
                            case IrOp::FROUND: out_v = std::nearbyint(in_v);
                                break;
                            case IrOp::FTRUNC: out_v = std::trunc(in_v);
                                break;
                            default: break;
                        }
                        uint64_t out_bits = is_f32
                                                ? static_cast<uint64_t>(f32_to_bits(
                    ...
                    case IrOp::FMAX: {
                        ...
                        bool   ok = true;
                        switch (ins.op) {
                            case IrOp::FADD: r = a + b;
                                break;
                            case IrOp::FSUB: r = a - b;
                                break;
                            case IrOp::FMUL: r = a * b;
                                break;
                            case IrOp::FDIV:
                                ...
                                r = a / b;
                                break;
                            case IrOp::FMIN: r = std::fmin(a, b);
                                break;
                            case IrOp::FMAX: r = std::fmax(a, b);
                                break;
                            default: ok = false;
                                break;
                        }
                    ...
                    case IrOp::CMP_UGE: {
                        ...
                        bool           r  = false;
                        switch (ins.op) {
                            case IrOp::CMP_EQ: r = (c0 == c1);
                                break;
                            case IrOp::CMP_NE: r = (c0 != c1);
                                break;
                            case IrOp::CMP_LT: r = (c0 < c1);
                                break;
                            case IrOp::CMP_GT: r = (c0 > c1);
                                break;
                            case IrOp::CMP_LE: r = (c0 <= c1);
                                break;
                            case IrOp::CMP_GE: r = (c0 >= c1);
                                break;
                            case IrOp::CMP_ULT: r = (u0 < u1);
                                break;
                            case IrOp::CMP_UGT: r = (u0 > u1);
                                break;
                            case IrOp::CMP_ULE: r = (u0 <= u1);
                                break;
                            case IrOp::CMP_UGE: r = (u0 >= u1);
                                break;
                            default: break;
                        }
                    ...
                    case IrOp::FCMP_GE: {
                        ...
                        bool r = false;
                        switch (ins.op) {
                            case IrOp::FCMP_EQ: r = (a == b);
                                break;
                            case IrOp::FCMP_NE: r = (a != b);
                                break;
                            case IrOp::FCMP_LT: r = (a < b);
                                break;
                            case IrOp::FCMP_GT: r = (a > b);
                                break;
                            case IrOp::FCMP_LE: r = (a <= b);
                                break;
                            case IrOp::FCMP_GE: r = (a >= b);
                                break;
                            default: break;
                        }
                        rewrite_as_const_with_value(fn, ins, r ? 1u : 0u);
    ...
    bool ir_pass_reassoc(IrFunction &fn) {
        ...
        auto fold_consts = [](IrOp op, int64_t a, int64_t b, int64_t &out) -> bool {
            switch (op) {
                case IrOp::ADD: out = a + b;
                    return true;
                case IrOp::MUL: out = a * b;
                    return true;
                case IrOp::AND: out = a & b;
                    return true;
                case IrOp::OR: out = a | b;
                    return true;
                case IrOp::XOR: out = a ^ b;
                    return true;
                default: return false;
            }
        };
        ...
        for (auto &bb: fn.blocks) {
            for (auto &ins: bb.instrs) {
                ...
                if (ins.operands.size() == 2) {
                    ...
                    switch (ins.op) {
                        case IrOp::ADD: res = a + b;
                            break;
                        case IrOp::SUB: res = a - b;
                            break;
                        case IrOp::MUL: res = a * b;
                            break;
                        ...
                        case IrOp::MOD:
                            ...
                            res = (uint64_t) (sa % sb);
                            break;
                        case IrOp::AND: res = a & b;
                            break;
                        case IrOp::OR: res = a | b;
                            break;
                        case IrOp::XOR: res = a ^ b;
                            break;
                        case IrOp::SHL: res = a << (b & 63);
                            break;
                        case IrOp::SHR: res = a >> (b & 63);
                            break;
                        case IrOp::SAR: res = (uint64_t) (sa >> (b & 63));
                            break;
                        // comparaciones enteras -> resultado bool (0 o 1)
                        case IrOp::CMP_EQ: res = (a == b) ? 1 : 0;
                            break;
                        case IrOp::CMP_NE: res = (a != b) ? 1 : 0;
                            break;
                        case IrOp::CMP_LT: res = (sa < sb) ? 1 : 0;
                            break;
                        case IrOp::CMP_GT: res = (sa > sb) ? 1 : 0;
                            break;
                        case IrOp::CMP_LE: res = (sa <= sb) ? 1 : 0;
                            break;
                        case IrOp::CMP_GE: res = (sa >= sb) ? 1 : 0;
                            break;
                        case IrOp::CMP_ULT: res = (a < b) ? 1 : 0;
                            break;
                        case IrOp::CMP_UGT: res = (a > b) ? 1 : 0;
                            break;
                        case IrOp::CMP_ULE: res = (a <= b) ? 1 : 0;
                            break;
                        case IrOp::CMP_UGE: res = (a >= b) ? 1 : 0;
                            break;
                ...
                if (ins.operands.size() == 1) {
                    ...
                    switch (ins.op) {
                        case IrOp::NEG: res = (uint64_t) (-sa);
                            break;
                        case IrOp::NOT: res = ~a;
                            break;
                        case IrOp::ZEXT: res = a;
                            break;
                        case IrOp::TRUNC: {
                            ...
                            switch (ins.type) {
                                ...
                                case IrType::I32:
                                    ...
                                    sign_extend = true;
                                    break;
                                case IrType::U8: mask = 0xFFULL;
                                    break;
                                case IrType::U16: mask = 0xFFFFULL;
                                    break;
                                case IrType::U32: mask = 0xFFFFFFFFULL;
                                    break;
                                case IrType::BOOL: mask = 0x1ULL;
                                    break;
                                default:
            ...
            for (IrBlockId b: loop_set) {
                for (const auto &ins: fn.blocks[b].instrs) {
                    ...
                    switch (ins.op) {
                        ...
                        case IrOp::CALLITF:
                        case IrOp::CALLCLOSURE:
                        case IrOp::TAILCALL: is_write = true;
                            break;
                        default: break;
                    }
    ...
    bool ir_pass_schedule(IrFunction &                           fn, const analysis::PointsTo *pt,
        ...
        const analysis::PointsTo &sched_pt =
                pt
                    ? *pt
                    : (g_sched_alias
                           ? (sched_local_pt = analysis::compute_points_to(
                               fn, analysis::build_ir_facts(fn)))
                           : sched_local_pt);
    ...
    static Pattern carry_idiom_pattern() {
        ...
        p.legal = [](const IrFunction &, const PatternMatch &) {
            return true;
        };
