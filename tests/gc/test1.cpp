...
static void test_basic_alloc() {
    ...
    bool ok = true;
    for (int i = 0; i < 64; ++i) ok &= (p1[i] == 0xAB);
    ...
    bool ok2 = true;
    for (int i = 0; i < 128; ++i) ok2 &= (p2[i] == 0xCD);
    ...
    ASSERT_MSG(heap.stats().alloc_bytes == 64 + 128, "stats: alloc_bytes == 192");
}
...
static void test_minor_gc_survival() {
    ...
    // y resetea el bump, por lo que un bucle hasta GC_NULL_HANDLE seria infinito.
    constexpr size_t OBJ_PAYLOAD = 256;
    constexpr size_t OBJ_TOTAL   = (sizeof(gc::GcHeader) + OBJ_PAYLOAD + 7) & ~7ULL;
    ...
    for (size_t i = 0; i < max_objects; ++i) {
        ...
        if (h == gc::GC_NULL_HANDLE) break; // OOM real en OldGen (no deberia ocurrir aqui)
    ...
    for (size_t i = 0; i < handles.size(); ++i) {
        ...
        bool data_ok = true;
        for (size_t b = 0; b < OBJ_PAYLOAD; ++b) data_ok &= (p[b] == patterns[i]);
        if (data_ok) {
            ok_count++;
        } else {
...
static void test_write_barrier() {
    SECTION("Test 3: Write barrier — remembered set se actualiza correctamente");
    ...
    // Ejecutar minor GC: el remembered set debe garantizar que h_young sobrevive
    printf("  >>> minor_gc() con remembered set activo...\n");
    heap.minor_gc();

    printf("  nursery_used tras minor GC: %zu\n", heap.nursery_used());
    uint8_t *p_after = heap.deref(h_young);
    ASSERT_MSG(p_after != nullptr, "objeto YOUNG sobrevive gracias a remembered set");

    print_gc_stats(heap.stats());
    ASSERT_MSG(heap.stats().minor_gc_count == 2, "stats: 2 ciclos minor GC en este test");
}
...
static void test_major_gc_collection() {
    ...
    vm::ArenaManager mgr;
    // Nursery pequeña para promover rapido, threshold bajo para disparar major GC
    gc::GcHeap heap(mgr, 4 * 1024, 1); // threshold=1 byte -> major GC tras cada minor GC
    ...
    for (gc::GcHandle h : dead_handles) {
        ...
        // En un sistema real el GC descubriria que nadie apunta a ellos via raices.
        (void)h;
    }
    ...
    for (size_t i = 0; i < live_handles.size(); ++i) {
        ...
        if (p != nullptr) {
            bool data_ok = true;
            for (int b = 0; b < 256; ++b) data_ok &= (p[b] == (uint8_t)(0xA0 + i));
            if (data_ok) ok_count++;
            else printf("    handle=%u datos CORRUPTOS\n", live_handles[i]);
        } else {
            printf("    handle=%u → NULL (objeto vivo recolectado indebidamente!)\n",
                   live_handles[i]);
        }
    }
    ...
    print_gc_stats(heap.stats());
    ASSERT_MSG(heap.stats().major_gc_count >= 1, "stats: al menos 1 major GC ejecutado");
    (void)freed_before;
}
...
static void test_stats_tracking() {
    ...
    ASSERT_MSG(heap.stats().minor_gc_count == 0, "stats iniciales: minor_gc_count=0");
    ASSERT_MSG(heap.stats().major_gc_count == 0, "stats iniciales: major_gc_count=0");
    ...
    ASSERT_MSG(heap.stats().minor_gc_count == 1,  "minor_gc_count == 1 tras un ciclo");
    ASSERT_MSG(heap.stats().promoted_count == 3,  "promoted_count == 3 (todos promovidos)");
...
#endif
    printf("\n\033[1;35m╔══════════════════════════════════════════════╗\033[0m\n");
    printf("\033[1;35m║       TEST SUITE: GcHeap Generacional        ║\033[0m\n");
