...
static void test_fifo_single_thread() {
    ...
    CHECK(t.pop_one(100, gc::WaitKind::MONITOR) == 1,
          "pop_one() devuelve el primero (1)");
    CHECK(t.pop_one(100, gc::WaitKind::MONITOR) == 2,
          "pop_one() devuelve el segundo (2)");
    CHECK(t.pop_one(100, gc::WaitKind::MONITOR) == 3,
          "pop_one() devuelve el tercero (3)");
...
static void test_kinds_independent() {
    ...
    CHECK(t.pop_one(7, gc::WaitKind::MONITOR) == 100,
          "monitor pop devuelve 100");
    CHECK(t.pop_one(7, gc::WaitKind::CONDVAR) == 300,
          "condvar pop devuelve 300 (intacto)");
    CHECK(t.pop_one(7, gc::WaitKind::MONITOR) == 200,
          "monitor pop devuelve 200");
    CHECK(t.pop_one(7, gc::WaitKind::CONDVAR) == 400,
          "condvar pop devuelve 400");
    CHECK(t.pop_one(7, gc::WaitKind::MONITOR) == UINT64_MAX,
          "monitor cola vacia (UINT64_MAX)");
    CHECK(t.pop_one(7, gc::WaitKind::CONDVAR) == UINT64_MAX,
          "condvar cola vacia (UINT64_MAX)");
}
...
static void test_concurrent_push_same_queue() {
    std::printf("\n=== Test 5: 8 threads x 5K push sobre misma cola, drain "
                "final ===\n");
    gc::WaitTable t;
...
static void test_concurrent_push_pop_disjoint() {
    std::printf("\n=== Test 6: 8 threads x 10K ops en colas distintas (cero "
                "contention) ===\n");
    gc::WaitTable t;
    constexpr int THREADS = 8;
    ...
    for (int tid = 0; tid < THREADS; ++tid) {
        threads.emplace_back([&t, &errors, tid]() {
            uint32_t my_handle =
                (uint32_t)(tid + 1000); // handle unico por thread
            for (int i = 0; i < OPS; ++i) {
    ...
    CHECK(errors.load() == 0,
          "0 errores en ops alternadas (FIFO mantenido por thread)");
...
static void test_concurrent_mixed_kinds() {
    ...
    for (int tid = 0; tid < THREADS; ++tid) {
        threads.emplace_back([&, tid]() {
            gc::WaitKind k =
                (tid % 2 == 0) ? gc::WaitKind::MONITOR : gc::WaitKind::CONDVAR;
            uint64_t base = (uint64_t)(tid + 1) * 1000000ULL;
            for (int i = 0; i < OPS; ++i) {
                t.push(99, k, base + i);
                if (k == gc::WaitKind::MONITOR)
                    pushes_mon.fetch_add(1);
                else
                    pushes_con.fetch_add(1);
                if ((i & 1) == 0) {
                    uint64_t got = t.pop_one(99, k);
                    if (got != 0) {
                        if (k == gc::WaitKind::MONITOR)
                            pops_mon.fetch_add(1);
                        else
                            pops_con.fetch_add(1);
                    }
                }
            }
        });
    ...
    std::printf(
        "  (info) pushes_mon=%d pops_mon=%d  pushes_con=%d pops_con=%d\n",
        pushes_mon.load(), pops_mon.load(), pushes_con.load(), pops_con.load());
}
