...
static void test_basic_alloc_free() {
    ...
    bool ok = true;
    for (int i = 0; i < 128; ++i) ok &= (raw[i] == 0xFF);
    ...
    ASSERT_MSG(ra.total_allocated() == 128, "total_allocated == 128 tras un alloc");
    ASSERT_MSG(ra.block_count() == 1,       "block_count == 1 tras un alloc");

    bool freed = ra.free(p1);
    printf("  free(0x%llx)    → %s\n", (unsigned long long)p1, freed ? "OK" : "FALLO");
    ...
    for (size_t i = 0; i < SIZE; ++i) {
        if (raw[i] != static_cast<uint8_t>(i & 0xFF)) {
            printf("  FALLO en byte %zu: esperado 0x%02X, leido 0x%02X\n",
                   i, (unsigned)(i & 0xFF), (unsigned)raw[i]);
            contig = false;
            break;
        }
    }
...
static void test_realloc_grow() {
    ...
    uint8_t *raw = reinterpret_cast<uint8_t *>(p);
    for (int i = 0; i < 256; ++i) raw[i] = static_cast<uint8_t>(i);
    ...
    ASSERT_MSG(preserved, "primeros 256 bytes preservados tras realloc creciente");

    // Los bytes nuevos (256..4095) deben estar a cero
    bool zeroed = true;
    for (int i = 256; i < 4096; ++i) zeroed &= (raw2[i] == 0);
    printf("  bytes nuevos (256..4095) zero-inicializados: %s\n", zeroed ? "SI" : "NO");
    ASSERT_MSG(zeroed, "extension zero-inicializada tras realloc creciente");

    ASSERT_MSG(ra.total_allocated() == 4096, "total_allocated == 4096 tras realloc");
...
static void test_realloc_shrink() {
    ...
    uint8_t *raw = reinterpret_cast<uint8_t *>(p);
    for (int i = 0; i < 1024; ++i) raw[i] = static_cast<uint8_t>(i & 0xFF);
...
static void test_free_unknown() {
    ...
    printf("  double-free(0x%llx)     → %s\n", (unsigned long long)p, r3 ? "true" : "false");
    ASSERT_MSG(!r3, "double-free retorna false sin crash");

    ASSERT_MSG(ra.stats().alloc_count == 1, "stats: 1 alloc total en este test");
...
static void test_free_all() {
    ...
    constexpr int    N     = 10;
    constexpr size_t SIZES[N] = {64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768};
    ...
    for (int i = 0; i < N; ++i) {
        ...
        printf("    alloc(%5zu) → ptr=0x%llx\n", SIZES[i], (unsigned long long)p);
        ASSERT_MSG(p != 0, "alloc retorna puntero valido");
    }

    printf("  total_allocated = %zu (esperado %zu)\n",
           ra.total_allocated(), total_expected);
    ASSERT_MSG(ra.total_allocated() == total_expected, "total_allocated correcto");
    ...
    ASSERT_MSG(ra.stats().free_count  == N, "stats: free_count == N tras free_all");
...
static void test_realloc_to_zero() {
    ...
    ASSERT_MSG(ra.total_allocated() == 0, "total_allocated == 0 tras realloc-free");
...
#endif
    printf("\n\033[1;35m╔══════════════════════════════════════════════╗\033[0m\n");
    printf("\033[1;35m║       TEST SUITE: RawAllocator               ║\033[0m\n");
