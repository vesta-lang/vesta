...
int main() {
    ...
    //    frames nativos en este test, todo lo NO pinnado debe colectarse.
    {
        ...
        uint32_t keep = vex_gc_alloc(40);
        vex_gc_pin(keep);
        uint8_t *kp = vex_gc_deref(keep);
        if (kp) std::memset(kp, 0x5A, 40);
        // Basura: 5000 objetos sin pin -> deben colectarse.
        for (int i = 0; i < 5000; ++i) {
            uint32_t g = vex_gc_alloc(32);
            uint8_t *p = vex_gc_deref(g);
            if (p) p[0] = static_cast<uint8_t>(i);
        }
        uint64_t before = vex_gc_live_count();
        vex_gc_collect();
        uint64_t after = vex_gc_live_count();
        check(after < before, "el modo AOT colecto basura no alcanzable");
        // Los pinnados sobreviven y su payload sigue intacto.
        check(vex_gc_deref(keep) != nullptr, "objeto pinnado 'keep' sobrevive");
        uint8_t *kp2 = vex_gc_deref(keep);
        check(kp2 && kp2[0] == 0x5A && kp2[39] == 0x5A,
              "payload de 'keep' intacto tras colectar");
        check(vex_gc_deref(h1) != nullptr && vex_gc_deref(h2) != nullptr,
              "h1/h2 pinnados sobreviven");
        // Tras despinnar y colectar, 'keep' tambien se colecta.
        vex_gc_unpin(keep);
        vex_gc_collect();
        check(vex_gc_live_count() < after, "tras unpin, 'keep' se colecta");
