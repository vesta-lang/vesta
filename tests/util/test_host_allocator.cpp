...
int main() {
    ...
    // --- alineacion y no solapamiento ------------------------------------
    {
        ...
        for (size_t rep = 0; rep < 64; ++rep)
            for (size_t s : kSizes) {
                ...
                if ((reinterpret_cast<uintptr_t>(p) & 15u) != 0) aligned = false;
                // Se llena con un patron dependiente del puntero: si dos
                // reservas se solaparan, una pisaria a la otra y se veria al
                // releer.
                std::memset(p, static_cast<int>(reinterpret_cast<uintptr_t>(p) & 0xFF), s);
                live.push_back(p);
            }
        for (void *p : live) {
            const unsigned char want =
                static_cast<unsigned char>(reinterpret_cast<uintptr_t>(p) & 0xFF);
        ...
        for (void *p : live) util::host_free(p);
    }
    ...
    // --- liberar entre hilos ---------------------------------------------
    {
        ...
        for (int j = 0; j < kPer; ++j) again.push_back(util::host_alloc(64));
        const auto after = util::host_alloc_stats().chunks;
        for (void *p : again) util::host_free(p);
    ...
    // --- comparacion con el sistema --------------------------------------
    {
        ...
        for (int r = 0; r < kRounds; ++r) {
            ...
            for (int i = 0; i < kBatch; ++i) std::free(live[i]);
        }
        auto t1 = std::chrono::steady_clock::now();
        for (int r = 0; r < kRounds; ++r) {
            for (int i = 0; i < kBatch; ++i)
                live[i] = util::host_alloc(kMix[(i + r) % 9]);
            for (int i = 0; i < kBatch; ++i) util::host_free(live[i]);
        }
        auto t2 = std::chrono::steady_clock::now();
        const double ops = double(kRounds) * kBatch;
        const double sys = std::chrono::duration<double, std::nano>(t1 - t0).count() / ops;
        const double own = std::chrono::duration<double, std::nano>(t2 - t1).count() / ops;
    ...
    std::printf("  reservas=%llu  sueltas=%llu  ajenas=%llu  trozos=%llu\n",
