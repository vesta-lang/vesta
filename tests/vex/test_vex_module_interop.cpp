...
void test_typedef_roundtrip() {
    ...
    std::vector<vx::TypeChecker::VexiOnlyEntry> only;
    only.push_back({"user_id", ""});
    only.push_back({"fn_lib", ""});
    vx::import_vexi_into_typechecker(*mainmod->tc, parsed.module_, only);
...
void test_struct_roundtrip() {
    ...
    vx::VexiModule vm;
    vx::export_typechecker_to_vexi(*lib->tc, 0x1234, vm);
    ...
    std::vector<vx::TypeChecker::VexiOnlyEntry> only = {{"Point", ""}};
    vx::import_vexi_into_typechecker(*mainmod->tc, parsed.module_, only);
...
void test_only_rename() {
    ...
    vx::VexiModule vm;
    vx::export_typechecker_to_vexi(*lib->tc, 0, vm);
    auto bytes = vx::vexi_emit(vm);
    auto parsed = vx::vexi_parse(bytes.data(), bytes.size());

    auto mainmod =
        compile_to_typechecker("i32 main() { return 0; }\n", "main3.vex");
    std::vector<vx::TypeChecker::VexiOnlyEntry> only = {{"fd", "FileDesc"}};
    vx::import_vexi_into_typechecker(*mainmod->tc, parsed.module_, only);
