 ...
 *   2. El coloreado producido es SIEMPRE un coloreado PROPIO
 * (is_proper_coloring via validate_assignment), derrame o no.
 *   3. CROMATICO (interval graph): si el banco ofrece >= max_overlap(clase)
 * lanes asignables, el coloreador NO derrama ningun valor de esa clase.
 *
...
int main() {
    std::printf(
        "=== test_rbank_roundtrip (Fase 0.5: modelo abstracto fiel) ===\n");
    ...
    std::printf("\n[abstraccion: rangos que se solapan interfieren]\n");
    {
        ...
        CHECK(inter.interferes(1, 2) && inter.interferes(1, 3) &&
                  inter.interferes(2, 3),
              "clique de 3 no detectado");
        CHECK(max_overlap(p, ResourceClass::GP) == 3,
              "max_overlap del clique != 3");
    ...
    {
        // Rangos DISJUNTOS (secuenciales) no interfieren -> pueden compartir
        // lane.
        AbstractProblem p;
        p.values = {mkval(1, 0, 5, ResourceClass::GP, ViewWidth::W8),
                    mkval(2, 6, 10, ResourceClass::GP, ViewWidth::W8)};
        CHECK(!build_interference(p).interferes(1, 2),
              "rangos disjuntos interfieren");
        CHECK(max_overlap(p, ResourceClass::GP) == 1,
              "max_overlap disjuntos != 1");
    }
    ...
    std::printf("\n[coloreado: clique separado, disjuntos comparten]\n");
    {
        ...
        // 3 lanes DISTINTAS.
        CHECK(col.lane_of(1) != col.lane_of(2) &&
                  col.lane_of(1) != col.lane_of(3) &&
    ...
    {
        std::mt19937 rng(
            0xC0FFEEu); // seed FIJO -> reproducible (no Math.random).
        const size_t gp_alloc =
            bank.allocatable_count(ResourceClass::GP, false);
        const size_t fp_alloc =
            bank.allocatable_count(ResourceClass::FP_VECTOR, false);

        int idempotent_ok = 0, proper_ok = 0, chromatic_ok = 0,
            chromatic_cases = 0;
        ...
        for (int t = 0; t < TRIALS; ++t) {
            ...
            // 3. Cromatico: presion <= lanes de la clase -> 0 spills de esa
            // clase.
            for (ResourceClass cls :
                 {ResourceClass::GP, ResourceClass::FP_VECTOR}) {
                const size_t alloc =
                    (cls == ResourceClass::GP) ? gp_alloc : fp_alloc;
        ...
        CHECK(idempotent_ok == TRIALS,
              "la abstraccion no fue determinista en algun caso");
        CHECK(proper_ok == TRIALS, "algun coloreado fue invalido (BUG)");
        CHECK(chromatic_ok == chromatic_cases,
              "presion<=lanes pero derramo (coloreador NO optimo en interval "
              "graph)");
        std::printf("  idempotente=%d/%d  propio=%d/%d  cromatico=%d/%d\n",
                    idempotent_ok, TRIALS, proper_ok, TRIALS, chromatic_ok,
                    chromatic_cases);
    }
