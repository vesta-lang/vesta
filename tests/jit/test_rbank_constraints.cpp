 ...
 *        aliasing), pines, must-differ/same y satisfaccion de ValueRequirements,
...
int main() {
    ...
    std::printf("\n[interferencia]\n");
    {
        ConstraintSet cs; cs.interfere(1, 2);
        LaneAssignment a; a.assign(1, RAX); a.assign(2, RAX); // misma lane
    ...
    {
        ConstraintSet cs; cs.interfere(1, 2);
        LaneAssignment a; a.assign(1, RAX); a.assign(2, RCX); // lanes distintas
        CHECK(ok(validate_assignment(cs, a, no_reqs, bank, false)),
              "lanes distintas con interferencia viola indebidamente");
    }
    {
        ConstraintSet cs; cs.interfere(1, 2);
        LaneAssignment a; a.spill(1); a.assign(2, RAX); // uno spilled
        CHECK(ok(validate_assignment(cs, a, no_reqs, bank, false)),
              "spilled interfiere indebidamente");
    }
    {
        ConstraintSet cs; cs.interfere(1, 2);
        CHECK(cs.interferes(1, 2) && cs.interferes(2, 1) && !cs.interferes(1, 3),
    ...
    {
        ConstraintSet cs; cs.different_lane(1, 2);
        LaneAssignment a; a.assign(1, RAX); a.assign(2, RAX);
        auto v = validate_assignment(cs, a, no_reqs, bank, false);
        CHECK(!ok(v) && vk(v) == ViolationKind::DIFFERENT_LANE_VIOLATED,
              "different_lane con misma lane no viola");
    }
    {
        ConstraintSet cs; cs.same_lane(1, 2);
        LaneAssignment a; a.assign(1, RAX); a.assign(2, RCX);
        auto v = validate_assignment(cs, a, no_reqs, bank, false);
    ...
    {
        ConstraintSet cs; cs.same_lane(1, 2);
        LaneAssignment a; a.assign(1, RDX); a.assign(2, RDX);
        CHECK(ok(validate_assignment(cs, a, no_reqs, bank, false)),
    ...
    std::printf("\n[pin fijo]\n");
    {
        ConstraintSet cs; cs.fixed_lane(1, RCX);
        LaneAssignment a; a.assign(1, RAX);
        auto v = validate_assignment(cs, a, no_reqs, bank, false);
    ...
    {
        ConstraintSet cs; cs.fixed_lane(1, RCX);
        LaneAssignment a; a.assign(1, RCX);
        CHECK(ok(validate_assignment(cs, a, no_reqs, bank, false)),
    ...
    std::printf("\n[requisito del valor sobre la lane asignada]\n");
    {
        ...
        reqs[0].value_id = 1; reqs[0].cls = ResourceClass::GP; reqs[0].width = ViewWidth::W8;
        LaneAssignment a; a.assign(1, XMM0);
    ...
    }
    {
        ...
        std::vector<ValueRequirements> reqs(1);
        reqs[0].value_id = 1; reqs[0].cls = ResourceClass::FP_VECTOR; reqs[0].width = ViewWidth::W64;
        LaneAssignment a; a.assign(1, XMM0);
        auto v = validate_assignment({}, a, reqs, bank, false);
    ...
    }
    {
        ...
        std::vector<ValueRequirements> reqs(1);
        reqs[0].value_id = 1; reqs[0].cls = ResourceClass::FP_VECTOR; reqs[0].width = ViewWidth::W8;
        LaneAssignment a; a.assign(1, XMM14);
        auto v = validate_assignment({}, a, reqs, bank, false);
    ...
    std::printf("\n[solucion valida completa]\n");
    {
        ...
        reqs[0].value_id = 1; reqs[0].cls = ResourceClass::GP; reqs[0].width = ViewWidth::W8;
        reqs[1].value_id = 2; reqs[1].cls = ResourceClass::FP_VECTOR; reqs[1].width = ViewWidth::W8;
        ConstraintSet cs; cs.interfere(1, 2); // clases distintas, lanes disjuntas.
        LaneAssignment a; a.assign(1, RAX); a.assign(2, XMM0);
