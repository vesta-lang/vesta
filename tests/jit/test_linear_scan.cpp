...
static bool is_caller_saved_gp(const TargetRegInfo &t, uint8_t r) {
    for (uint8_t x : t.caller_saved[(size_t)RegClass::GP])
        if (x == r) return true;
    return false;
}
...
static void test_low_pressure() {
    ...
    CHECK(is_caller_saved_gp(tri, ra.reg_of(0)),
          "v0 en caller-saved (sin call)");
...
static void test_cross_call() {
    ...
    b.instrs.push_back(
        MInstr::make_unary(MOp::MOV, v1, v0)); // usa v0 tras call
    b.instrs.push_back(MInstr::make_ret());
    mf.blocks.push_back(std::move(b));
    ...
    bool recorded = false;
    for (uint8_t x : ra.callee_saved_used)
        if (x == r) recorded = true;
...
static void test_spill() {
    ...
    b.instrs.push_back(
        MInstr::make_unary(MOp::MOV, v3, imm(4))); // 3  <- 4 vivos
    ...
    b.instrs.push_back(
        MInstr::make_unary(MOp::MOV, v0, t2)); // 7 (usa t2; v0 reciclado)
    b.instrs.push_back(MInstr::make_ret());
    mf.blocks.push_back(std::move(b));
...
static void test_reuse() {
    ...
    b.instrs.push_back(
        MInstr::make_unary(MOp::MOV, v1, v0)); // usa v0 (muere), def v1
    b.instrs.push_back(
        MInstr::make_unary(MOp::MOV, v2, v1)); // usa v1 (muere), def v2
