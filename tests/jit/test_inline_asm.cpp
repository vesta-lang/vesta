 ...
 * layout que el codigo VM_ABI espera (safepoint_flag@0, registers@96, RBX=proc).
 */
...
struct StubAsm : vex::AsmBackend {
    vex::AsmAssembleResult assemble(const std::string &nasm,
        ...
        if (nasm == "popcnt rax, rdi") {
            ...
            r.ok = false; r.error = "stub: nasm no reconocido";
        }
        return r;
    }
};
...
static void test_popcount() {
    ...
    ir::IrFunction fn;
    fn.name = "popcount_asm"; fn.ret_type = ir::IrType::I64;
...
static void test_double_inout() {
    ...
    ir::IrFunction fn;
    fn.name = "double_asm"; fn.ret_type = ir::IrType::I64;
    auto x   = fn.new_value(ir::IrType::I64);
...
static void test_add_two() {
    ...
    ir::IrFunction fn;
    fn.name = "add_asm"; fn.ret_type = ir::IrType::I64;
