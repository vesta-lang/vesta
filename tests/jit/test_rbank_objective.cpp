...
int main() {
    ...
    CHECK(static_execution_weight(20) == static_execution_weight(9),
          "depth no capado en 9");

    // --- spill cost: peso de ejecucion (suministrado) + rematerializable ---
    std::printf("\n[coste de spill]\n");
    {
        ValueRequirements cold;
        cold.loop_depth = 0;
        ValueRequirements hot;
        hot.loop_depth = 2;
        ...
        ValueRequirements remat;
        remat.loop_depth = 2;
        remat.rematerializable = true;
        ValueRequirements plain;
        plain.loop_depth = 2;
    ...
    {
        ValueRequirements nocross;
        nocross.crosses_call = false;
        CHECK(callsave_cost_of(nocross, SavePolicy::VOLATILE, 5) == 0.0,
              "valor que no cruza call tiene callsave != 0");
        ValueRequirements cross;
        cross.crosses_call = true;
    ...
    std::printf("\n[eleccion de lane: crossing -> preferir preservado]\n");
    {
        ...
        r.crosses_call = true;
        const ObjectiveTerms tv =
            lane_choice_terms(r, *v0, 4); // volatil, 4 calls
    ...
    std::printf("\n[hot value: registro mejor que spill]\n");
    {
        ...
        const ObjectiveTerms spill =
            spill_terms(r, static_execution_weight(r.loop_depth));
