 ...
 * @brief Splitting incremento 3c: la @c FragmentationRecovery produce planes
 * REALES.
 *
 * Se prueba el ALGORITMO aislado (sin compilar nada): un problema sintetico
 * donde todas las lanes menos una estan ocupadas siempre, y esa una tiene un
 * HUECO.  Se comprueba que el plan (a) aprovecha el hueco, (b) respeta las
 * condiciones de correctitud -- no cruza bloques, deja base antes y despues --
 * y (c) obedece al cost model.
 */
...
/// Escenario sintetico: N-1 lanes GP ocupadas SIEMPRE y una (la primera
/// admisible) libre solo en [hole_from, hole_to].  El valor 0 esta derramado y
/// vive en [1, life_to].
struct Scenario {
...
Scenario make_scenario(uint32_t hole_from, uint32_t hole_to, uint32_t life_to,
    ...
    // Bloqueadores: uno por lane admisible.  La PRIMERA deja el hueco; el resto
    // ocupan toda la ventana temporal -> el valor 0 es `partially` (hay hueco,
    // pero no completo).
    uint32_t next_id = 1;
    bool first = true;
    ...
    // Intervalos reales del backend: solo el valor 0 nos interesa (los
    // bloqueadores ya estan en registro y el algoritmo ni los mira).
...
int main() {
    std::printf(
        "=== test_fragmentation_recovery (Splitting incremento 3c) ===\n");
    ...
    /* --- 1. Caso nominal: hueco [9,19], 5 usos dentro, un solo bloque --- */
    {
        ...
        const AssignmentPlan plan = build_fragmentation_plan(
            s.p, s.la, s.bank, false, s.ivs, cost, &st);
        ...
        if (plan.intervals.size() == 1) {
            ...
            CHECK(si.to.value ==
                  18 + 2); // ultimo uso + def-point de esa instr.
            CHECK(si.location.is_register());
            CHECK(si.location.register_id() ==
                  s.free_lane); // la UNICA lane con hueco.
        }
    ...
    /* --- 3. Condicion 1 (RECTILINEO): una frontera de bloque en 14 parte el
     * hueco. El tramo NO puede cruzarla, asi que el plan empieza en 14 (no en
     * 10) y solo captura los usos del segundo bloque. --- */
    {
        ...
        CHECK(st.uses_recovered ==
              3); // 14, 16, 18 (los de antes quedan fuera).
    }

    /* --- 4. Condicion 3 (VUELVE A LA BASE): el valor muere en 19, asi que el
     * tramo NO puede llegar hasta el ultimo uso -- necesitaria to=20 y ahi ya
     * no hay tramo base en memoria donde devolverlo.  CONTRATO (no "lo que
     * salga"): el algoritmo recorta hasta el penultimo uso y recupera [10,18)
     * con 4 usos.  Si una heuristica futura dejara de recuperar este caso, este
     * test lo detecta; por eso se fija el resultado exacto y no solo "to < 20".
     * --- */
    {
        ...
        if (plan.intervals.size() == 1) {
            ...
            CHECK(plan.intervals[0].to.value ==
                  18); // recortado: 20 seria el fin de la vida.
        }
        CHECK(st.uses_recovered == 4); // el uso 18 queda fuera.
    }

    /* --- 5. Valor que DEBE vivir en memoria (GC root cross-call): jamas se
     * planifica --- */
    {
        Scenario s = make_scenario(9, 19, 30, {10, 12, 14, 16, 18}, {0});
    ...
    /* --- 6. Sin usos no hay nada que ahorrar: plan vacio (y sin tocar
     * estadisticas) --- */
    {
    ...
    /* @c LaneOccupancy (invariante de normalizacion + aliasing) NO se prueba
     * aqui: es un componente independiente con sus propios consumidores ->
     * tests/jit/test_lane_occupancy.cpp. */
