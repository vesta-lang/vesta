...
void test_phi_if_else() {
    ...
    entry.name = "entry";
    ir::IrInstr br =
        mk_instr(ir::IrOp::BR_COND, ir::IrType::VOID, ir::IR_NO_VALUE);
    ...
    then_bb.name = "then";
    ir::IrInstr br_t =
        mk_instr(ir::IrOp::BR, ir::IrType::VOID, ir::IR_NO_VALUE);
    br_t.target_block = 3;
...
void test_loop_phi_sum_to_n() {
    ...
    e.id = 0;
    e.name = "entry";
    ir::IrInstr c0 = mk_instr(ir::IrOp::CONST, ir::IrType::I64, zero);
    c0.imm = 0;
    e.instrs.push_back(c0);
    ir::IrInstr c1 = mk_instr(ir::IrOp::CONST, ir::IrType::I64, one);
    c1.imm = 1;
    ...
    const double jit_mips =
        (jit_ms > 0) ? (static_cast<double>(N_BENCH) / 1e6 / (jit_ms / 1000.0))
                     : 0.0;
    ...
    /* JIT C1 template puede ser bastante lento vs C (cada op = load+op+store) -
     * aceptable hasta 30x slower. */
...
void test_invalidate() {
    ...
    comp.invalidate(res);
    CHECK(jit::JitRegistry::instance().size() == 0,
          "registry vacio tras invalidate");
