...
static void test_classify_op() {
    ...
    CHECK(aot_classify_op(IrOp::MEMCPY)   == AotOpClass::PURE_NATIVE);  // rep movsb inline
    CHECK(aot_classify_op(IrOp::CALL)     == AotOpClass::PURE_NATIVE);
    CHECK(aot_classify_op(IrOp::CALLN)    == AotOpClass::PURE_NATIVE);  // extern resuelto por linker
...
static void test_allowed_in_target() {
    ...
    AotTarget bare;        bare.tier = Tier::BARE;        bare.freestanding = false;
    AotTarget freestand;   freestand.tier = Tier::BARE;   freestand.freestanding = true;
    AotTarget embed;       embed.tier = Tier::EMBED;
    AotTarget full;        full.tier = Tier::FULL;
    ...
    CHECK(aot_op_allowed(IrOp::ALLOCA, freestand));  // stack = sub rsp, sin allocator
    CHECK(aot_op_allowed(IrOp::MEMCPY, freestand));  // rep movsb inline
    CHECK(aot_op_allowed(IrOp::CALLN,  freestand));  // extern a simbolo del kernel/usuario
    ...
    CHECK(!aot_op_allowed(IrOp::FINDCLASS, embed));  // classregistry dinamico no en embed
...
static void test_analyze_module() {
    ...
    AotTarget bare;       bare.tier = Tier::BARE;
    AotTarget freestand;  freestand.tier = Tier::BARE; freestand.freestanding = true;
    AotTarget embed;      embed.tier = Tier::EMBED;
    AotTarget full;       full.tier = Tier::FULL;
    ...
    {
        ir::IrModule mod;
        mod.functions.push_back(make_fn("fib", {IrOp::CONST, IrOp::ADD, IrOp::SUB,
                                                 IrOp::CMP_LT, IrOp::BR_COND, IrOp::RET}));
    ...
    {
        ir::IrModule mod;
        mod.functions.push_back(make_fn("alloc_buf", {IrOp::CONST, IrOp::RAW_ALLOC, IrOp::RET}));
    ...
    {
        ir::IrModule mod;
        mod.functions.push_back(make_fn("make_obj", {IrOp::NEWOBJ, IrOp::SETFIELD,
                                                     IrOp::CALLVIRT, IrOp::RET}));
        AotCompatReport rb = aot_analyze_module(mod, bare);
        CHECK(!rb.compatible);
    ...
    {
        ir::IrModule mod;
        mod.functions.push_back(make_fn("fetch", {IrOp::FUTURE, IrOp::AWAIT, IrOp::RET}));
        AotCompatReport re = aot_analyze_module(mod, embed);
        CHECK(!re.compatible);
