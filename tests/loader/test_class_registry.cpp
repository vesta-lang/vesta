...
static void test_define_and_find() {
    ...
    ClassInfo *empty =
        reg.define_class("Empty", nullptr, {}, {}, {}, CLASS_VIS_PUBLIC);
...
static void test_aop_advice_chain() {
    ...
    check(target->advice_chain != nullptr,
          "advice_chain != nullptr tras anadir");
    check(target->advice_chain->kind == ADVICE_BEFORE,
          "primer advice es BEFORE");
    check(target->advice_chain->advice_method == b1,
          "primer advice apunta a b1");
    ...
    check(target->advice_chain->next->kind == ADVICE_AFTER,
          "segundo advice es AFTER");
    check(target->advice_chain->next->advice_method == a1,
          "segundo advice apunta a a1");
    ...
    check(third != nullptr && third->kind == ADVICE_AROUND &&
              third->advice_method == r1,
          "tercer advice es AROUND -> r1");
}
...
static void test_lookup_scale() {
    ...
    fields.reserve(64);
    for (int i = 0; i < 64; ++i) {
        fields.push_back({"f" + std::to_string(i), FIELD_PRIMITIVE, nullptr, 8,
                          FIELD_PUBLIC, false});
    }
...
static void test_incremental_mutation() {
    ...
    bool ok1 = reg.add_field(
        cls, {"x", FIELD_PRIMITIVE, nullptr, 4, FIELD_PUBLIC, false});
    bool ok2 = reg.add_field(
        cls, {"y", FIELD_PRIMITIVE, nullptr, 4, FIELD_PUBLIC, false});
    bool ok3 = reg.add_field(
        cls, {"z", FIELD_PRIMITIVE, nullptr, 4, FIELD_PUBLIC, false});
    ...
    bool om1 =
        reg.add_method(cls, {"add", "(V)V", METHOD_FLAG_VIRTUAL, 0x100, 16});
    bool om2 =
        reg.add_method(cls, {"sub", "(V)V", METHOD_FLAG_VIRTUAL, 0x200, 16});
...
static void test_bytecode_shape() {
    ...
    ClassInfo *cls =
        reg.define_class("Punto", nullptr, {}, {}, {}, CLASS_VIS_PUBLIC);
    ...
    bool of1 = reg.add_field(
        cls, {"x", FIELD_PRIMITIVE, nullptr, 4, FIELD_PUBLIC, false});
    bool of2 = reg.add_field(
        cls, {"y", FIELD_PRIMITIVE, nullptr, 4, FIELD_PUBLIC, false});
    ...
    bool om = reg.add_method(
        cls, {"distance", "()f64", METHOD_FLAG_VIRTUAL, 0x1234, 64});
...
static void test_lifecycle_stress() {
    ...
    for (int iter = 0; iter < 16; ++iter) {
        ...
        for (int ci = 0; ci < 32; ++ci) {
            ...
            ClassInfo *cls =
                reg.define_class(cname, nullptr, {}, fields, methods);
            if (!cls) continue;
            ...
            for (int m = 0; m < 4; ++m) {
                ...
                MethodInfo *advice = ClassRegistry::find_method(
                    cls, "m" + std::to_string((m + 1) % 4));
                if (target && advice) {
        ...
        check(reg.class_count() == 32,
              "stress: 32 clases creadas en este ciclo");
...
static void test_inheritance_basic() {
    ...
    ClassInfo *base =
        reg.define_class("Base", nullptr, {}, base_fields, base_methods);
    ...
    check(sub->method_count == 2,
          "Sub.method_count == 2 (heredados ctor+sonido)");
    ...
    check(ms->code_vaddr == 0x2000, "Sub.sonido code_vaddr heredado del super");
