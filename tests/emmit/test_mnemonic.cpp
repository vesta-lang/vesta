 ...
 * categoria sean dos comparaciones.  Si alguien reordena la lista y la rompe,
 * las preguntas siguen dando un resultado -- el equivocado --, asi que se
 * comprueba.
 *
 * Y se comprueban mnemonicos POR NOMBRE, no solo el total: la lista se genero
 * extrayendo las claves de la tabla del emisor y el primer intento se dejo
 * SIETE fuera -- `mov` entre ellas -- porque el patron no cogia las entradas
 * escritas en varias lineas.  Un recuento que cuadra no demuestra que esten las
 * que hacen falta.
 */
...
int main() {
    ...
    for (uint8_t ci = 0; ci < static_cast<uint8_t>(Category::kCount); ++ci) {
        ...
        for (uint16_t i = r.first; i <= r.last; ++i) {
            ...
            std::printf("  FALLA    la categoria '%s' no es contigua: %s cae "
                        ...
                        category_name(c), text_of(static_cast<Mnemonic>(i)),
    ...
    exige(!is_valid(mnemonic_from_text("")),
          "la cadena vacia no es instruccion");
    exige(!is_valid(mnemonic_from_text(nullptr)),
          "un puntero nulo no revienta");
    ...
    for (uint16_t i = 0; i < mnemonic_count(); ++i) {
        ...
        std::printf(
            "  FALLA    '%s' no vuelve a su mnemonico (texto repetido?)\n",
            text_of(m));
        ++fallos;
    }
