...
static bool gen_test_cert(TestCert &tc) {
    ...
    PEM_write_bio_PrivateKey(bio_key, tc.pkey, nullptr, nullptr, 0, nullptr,
                             nullptr);
...
static void test_session_tls_no_token(uint16_t port, const TestCert &tc) {
    ...
    if (!srv_ctx) {
        printf("  [ERROR] no se pudo crear SSL_CTX para el servidor. Omitiendo "
               "test.\n");
        return;
    }
    printf("  Certificado de test generado: RSA 2048 autofirmado, "
           "CN=VestaVM-TestNode\n");
    ...
    std::thread srv_thread([&]() {
        ...
        if (ok) {
            printf("  [SERVIDOR TLS] sesion activa: remote_id=%llu "
                   "session_id=0x%llX\n",
            ...
            session.start_reader([&](const distrib::VdpHeader &hdr,
                ...
                if (static_cast<distrib::VdpMsgType>(hdr.msg_type) ==
                    ...
                    printf("  [SERVIDOR TLS] PING recibido (%u bytes), "
                           "enviando PONG\n",
                           hdr.payload_len);
    ...
    bool client_ok =
        client.connect_to("127.0.0.1", port, client_auth, "tls-test-client");
    ...
    if (!srv.received.empty())
        check(srv.received[0] ==
                  std::vector<uint8_t>({'T', 'L', 'S', '-', 'O', 'K'}),
...
static void test_session_tls_with_token(uint16_t port, const TestCert &tc,
                                        bool correct_token) {
    char titulo[128];
    std::snprintf(titulo, sizeof(titulo), "VdpSession TLS + token CRAM (%s)",
    ...
    const char *client_token =
        correct_token ? "token-tls-vesta" : "token-incorrecto";
    distrib::NodeAuthConfig client_auth{};
    client_auth.use_tls = true;
    ...
    bool client_ok =
        client.connect_to("127.0.0.1", port, client_auth, "tls-token-client");
...
int main() {
    ...
    // generar certificado RSA 2048 autofirmado una sola vez para todos los
    // tests
    ...
    printf(
        "[SETUP] Certificado generado: %zu bytes PEM cert, %zu bytes PEM key\n",
