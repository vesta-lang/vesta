 ...
 *
 * Escenario:
 *   - Hilo servidor: abre un socket TCP en 127.0.0.1:17789, acepta una
 * conexion, crea VdpSession(fd, nullptr, ...) y ejecuta server_handshake().
 *   - Hilo cliente (main): crea VdpSession(node_idx, local_id) y ejecuta
 * connect_to().
...
static void test_session_plain_no_token(uint16_t port) {
    ...
    std::thread srv_thread([&]() {
        ...
        if (lsock == TEST_INVALID_SOCK) {
            printf(
                "  [SERVIDOR] error al crear socket de escucha en puerto %u\n",
                port);
        ...
        test_sock_t cfd =
            accept(lsock, reinterpret_cast<struct sockaddr *>(&caddr), &clen);
        test_close(lsock);
        ...
        distrib::VdpSession session(static_cast<vdp_sock_t>(cfd), nullptr, 0,
                                    1001);
        ...
        if (ok) {
            printf("  [SERVIDOR] sesion activa: session_id=0x%llX "
                   "remote_node_id=%llu\n",
            ...
            session.start_reader([&](const distrib::VdpHeader &hdr,
                ...
                if (static_cast<distrib::VdpMsgType>(hdr.msg_type) ==
                    ...
                    printf(
                        "  [SERVIDOR] PING recibido con payload de %u bytes\n",
                        hdr.payload_len);
                    // responder PONG
    ...
    bool client_ok =
        client.connect_to("127.0.0.1", port, client_auth, "test-cliente");
    ...
    if (client_ok) {
        ...
        const std::vector<uint8_t> ping_payload = {'V', 'D', 'P', '-',
                                                   'T', 'E', 'S', 'T'};
    ...
    if (!srv.received_msgs.empty()) {
        ...
        check(msg.size() == 8 && std::memcmp(msg.data(), "VDP-TEST", 8) == 0,
...
static void test_session_plain_with_token(uint16_t port, bool correct_token) {
    ...
    const char *client_token =
        correct_token ? "secreto-vesta-2026" : "token-equivocado";
    distrib::NodeAuthConfig client_auth{};
    client_auth.use_tls = false;
    ...
    bool client_ok =
        client.connect_to("127.0.0.1", port, client_auth, "test-token-client");
    printf("  [CLIENTE]  connect_to() = %s\n",
    ...
    if (correct_token) {
        ...
        check(!client_ok,
              "cliente con token incorrecto: rechazado (SESSION_ERROR)");
...
int main() {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(50)); // dejar que el SO libere el puerto
