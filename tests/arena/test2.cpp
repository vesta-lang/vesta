...
int main() {
    ...
    // Crear areneros
    int arena1 =
        manager.create_arena(64, vm::MemPerm::READ | vm::MemPerm::WRITE);
    int arena2 =
        manager.create_arena(128, vm::MemPerm::READ | vm::MemPerm::EXEC);
    int arena3 = manager.create_arena(
        256, vm::MemPerm::READ | vm::MemPerm::WRITE | vm::MemPerm::EXEC);
    ...
    auto show_arena = [&](int id) {
        ...
        assert(a != nullptr);
        std::cout << "Arena ID=" << id << " | ptr=" << a->ptr
                  << " | size=" << a->size << " | perms="
    ...
    if (has_perm(a1->perms, vm::MemPerm::WRITE)) {
        ...
        assert(data[0] == 123);
        assert(data[1] == 456);
        std::cout << "[Arena ID=" << arena1
                  << "] Escritura y lectura OK: " << data[0] << ", " << data[1]
                  << "\n";
    }
    ...
    std::cout
        << "[ArenaManager Test] Todos los areneros liberados correctamente.\n";
