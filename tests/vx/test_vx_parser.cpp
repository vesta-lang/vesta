...
static void test_minimal_main() {
    ...
    VX_ASSERT(fn->name == "main", "nombre main");
    auto pt = as_node<ast::PrimitiveTypeNode>(fn->return_type.get(),
                                              ast::NodeKind::PrimitiveTypeNode);
    VX_ASSERT(pt != nullptr && pt->prim == PrimitiveKind::I32, "retorno i32");
    VX_ASSERT(fn->params.empty(), "sin params");
    VX_ASSERT(fn->body != nullptr, "tiene body");
    VX_ASSERT_EQ(fn->body->body.size(), (size_t)1, "un statement en el body");
    auto ret = as_node<ast::ReturnStmt>(fn->body->body[0].get(),
                                        ast::NodeKind::ReturnStmt);
    VX_ASSERT(ret != nullptr, "es ReturnStmt");
    auto lit =
        as_node<ast::IntLitExpr>(ret->value.get(), ast::NodeKind::IntLitExpr);
    VX_ASSERT(lit != nullptr, "valor es IntLit");
    VX_ASSERT_EQ(lit->value, (uint64_t)0, "valor 0");
}
...
static void test_global_var_decl() {
    auto out = parse_src("u8 contador = 42;\n"
                         "const f64 PI = 3.14;\n"
                         "i32 sin_init;\n");
    VX_ASSERT(!out.diags.has_errors(), "globales sin errores");
    VX_ASSERT_EQ(out.mod->decls.size(), (size_t)3, "tres globales");

    auto g0 = as_node<ast::GlobalVarDecl>(out.mod->decls[0].get(),
                                          ast::NodeKind::GlobalVarDecl);
    VX_ASSERT(g0 && g0->name == "contador", "global contador");
    VX_ASSERT(!g0->is_const, "contador no const");
    VX_ASSERT(g0->init != nullptr, "contador con init");

    auto g1 = as_node<ast::GlobalVarDecl>(out.mod->decls[1].get(),
                                          ast::NodeKind::GlobalVarDecl);
    VX_ASSERT(g1 && g1->name == "PI", "global PI");
    VX_ASSERT(g1->is_const, "PI const");

    auto g2 = as_node<ast::GlobalVarDecl>(out.mod->decls[2].get(),
                                          ast::NodeKind::GlobalVarDecl);
    VX_ASSERT(g2 && g2->name == "sin_init", "global sin_init");
    VX_ASSERT(g2->init == nullptr, "sin init");
}
...
static void test_function_with_params() {
    ...
    auto out = parse_src("uint32_t suma(int32_t a, i32 b, u8 c) {\n"
                         "    return a + b;\n"
                         "}\n");
    VX_ASSERT(!out.diags.has_errors(), "funcion con params sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    VX_ASSERT(fn && fn->name == "suma", "es funcion suma");
    VX_ASSERT_EQ(fn->params.size(), (size_t)3, "tres params");
    auto rt = as_node<ast::PrimitiveTypeNode>(fn->return_type.get(),
                                              ast::NodeKind::PrimitiveTypeNode);
    VX_ASSERT(rt && rt->prim == PrimitiveKind::U32, "retorno U32 (uint32_t)");
...
static void test_arithmetic_precedence() {
    ...
    VX_ASSERT(!out.diags.has_errors(), "precedencia sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    auto ret = as_node<ast::ReturnStmt>(fn->body->body[0].get(),
                                        ast::NodeKind::ReturnStmt);
...
static void test_left_associativity() {
    ...
    auto sub =
        as_node<ast::BinaryExpr>(ret->value.get(), ast::NodeKind::BinaryExpr);
    VX_ASSERT(sub && sub->op == ast::BinOp::Sub, "raiz Sub");
    // lhs del sub raiz debe ser otro Sub.
    auto inner =
        as_node<ast::BinaryExpr>(sub->lhs.get(), ast::NodeKind::BinaryExpr);
    VX_ASSERT(inner && inner->op == ast::BinOp::Sub, "inner es Sub");
    auto i_lhs =
    ...
    VX_ASSERT(i_lhs && i_lhs->value == 10, "10");
    VX_ASSERT(i_rhs && i_rhs->value == 4, "4");
    VX_ASSERT(r_rhs && r_rhs->value == 3, "3");
}
...
static void test_comparison_and_logical() {
    ...
    auto out = parse_src("i32 f(i32 a, i32 b, i32 c, i32 d, i32 e) {\n"
                         "    return a < b && c == d || !e;\n"
                         "}\n");
    VX_ASSERT(!out.diags.has_errors(), "comp+logical sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    auto ret = as_node<ast::ReturnStmt>(fn->body->body[0].get(),
                                        ast::NodeKind::ReturnStmt);
    auto orx =
        as_node<ast::BinaryExpr>(ret->value.get(), ast::NodeKind::BinaryExpr);
    VX_ASSERT(orx && orx->op == ast::BinOp::LogicalOr, "raiz Or");
    auto andx =
        as_node<ast::BinaryExpr>(orx->lhs.get(), ast::NodeKind::BinaryExpr);
    VX_ASSERT(andx && andx->op == ast::BinOp::LogicalAnd, "lhs And");
    auto notx =
        as_node<ast::UnaryExpr>(orx->rhs.get(), ast::NodeKind::UnaryExpr);
    VX_ASSERT(notx && notx->op == ast::UnOp::LogicalNot, "rhs Not(e)");
    auto lt =
        as_node<ast::BinaryExpr>(andx->lhs.get(), ast::NodeKind::BinaryExpr);
    auto eq =
        as_node<ast::BinaryExpr>(andx->rhs.get(), ast::NodeKind::BinaryExpr);
    VX_ASSERT(lt && lt->op == ast::BinOp::Lt, "and lhs Lt");
    VX_ASSERT(eq && eq->op == ast::BinOp::Eq, "and rhs Eq");
}
...
static void test_assignment_right_assoc() {
    ...
    auto out = parse_src("i32 f(i32 a, i32 b) {\n"
                         ...
                         "    return a;\n"
                         "}\n");
    VX_ASSERT(!out.diags.has_errors(), "assign right-assoc sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    ...
    VX_ASSERT(a1 && a1->op == ast::AssignOp::Assign, "primer assign");
    auto a2 =
        as_node<ast::AssignExpr>(a1->value.get(), ast::NodeKind::AssignExpr);
    VX_ASSERT(a2 && a2->op == ast::AssignOp::Assign, "segundo assign");
    auto target_a =
        as_node<ast::IdentExpr>(a1->target.get(), ast::NodeKind::IdentExpr);
    auto target_b =
        as_node<ast::IdentExpr>(a2->target.get(), ast::NodeKind::IdentExpr);
    VX_ASSERT(target_a && target_a->name == "a", "target a");
    VX_ASSERT(target_b && target_b->name == "b", "target b");
}
...
static void test_if_while_for() {
    ...
    VX_ASSERT(!out.diags.has_errors(), "if/while/for sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
...
static void test_function_call() {
    ...
    VX_ASSERT(!out.diags.has_errors(), "calls sin errores");
    VX_ASSERT_EQ(out.mod->decls.size(), (size_t)2, "dos decls");
    ...
    VX_ASSERT(outer && outer->args.size() == 1, "outer call con 1 arg");
    auto inner =
        as_node<ast::CallExpr>(outer->args[0].get(), ast::NodeKind::CallExpr);
    VX_ASSERT(inner && inner->args.size() == 1, "inner call con 1 arg");
    auto lit = as_node<ast::IntLitExpr>(inner->args[0].get(),
                                        ast::NodeKind::IntLitExpr);
    VX_ASSERT(lit && lit->value == 40, "literal 40");
}
...
static void test_break_continue() {
    ...
    VX_ASSERT(!out.diags.has_errors(), "break/continue sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
...
static void test_compound_assignments() {
    auto out = parse_src("i32 f(i32 a) {\n"
                         ...
                         "    return a;\n"
                         "}\n");
    VX_ASSERT(!out.diags.has_errors(), "compound assigns sin errores");
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    VX_ASSERT_EQ(fn->body->body.size(), (size_t)11, "10 ExprStmt + 1 return");
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        auto a =
            as_node<ast::AssignExpr>(es->expr.get(), ast::NodeKind::AssignExpr);
        VX_ASSERT(a && a->op == expected[i], "compound assign correcto");
    }
}
...
static void test_error_recovery() {
    ...
    auto out = parse_src("i32 a = 1\n" // <-- falta ;
                         "i32 b = 2;\n");
    VX_ASSERT(out.diags.has_errors(), "error de ; reportado");
    ...
    VX_ASSERT(found_b, "recuperacion: b detectado tras error");
}
...
static void test_position_propagated() {
    ...
    auto fn = as_node<ast::FunctionDecl>(out.mod->decls[0].get(),
                                         ast::NodeKind::FunctionDecl);
    VX_ASSERT_EQ(fn->loc.line, (uint32_t)1, "fn linea 1");
    auto ret = as_node<ast::ReturnStmt>(fn->body->body[0].get(),
                                        ast::NodeKind::ReturnStmt);
