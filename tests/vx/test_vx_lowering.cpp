...
static void test_minimal_main_lowers() {
    ...
    VX_ASSERT(out.ok, "minimal main lower ok");
    VX_ASSERT_EQ(out.irmod.functions.size(), (size_t)1, "una funcion");

    const auto &fn = out.irmod.functions[0];
    VX_ASSERT(fn.name == "main", "funcion main");
    VX_ASSERT(fn.ret_type == ir::IrType::I32, "retorno I32");
    VX_ASSERT_EQ(fn.params.size(), (size_t)0, "sin params");
    VX_ASSERT_EQ(fn.blocks.size(), (size_t)1, "un bloque");
...
static void test_add_function_lowers() {
    ...
    VX_ASSERT(out.ok, "add+main lower ok");
    VX_ASSERT_EQ(out.irmod.functions.size(), (size_t)2, "dos funciones");
    ...
    VX_ASSERT(add != nullptr, "encontrada add");
    VX_ASSERT_EQ(add->params.size(), (size_t)2, "add tiene 2 params");
    ...
    VX_ASSERT(found_add, "ADD presente en add");
    VX_ASSERT(found_ret, "RET presente en add");
    ...
    VX_ASSERT(m != nullptr, "encontrada main");
    ...
    VX_ASSERT(found_call, "CALL a add presente");
}
...
static void test_if_else_cfg() {
    ...
    VX_ASSERT(has_then, "bloque if_then presente");
    VX_ASSERT(has_else, "bloque if_else presente");
    VX_ASSERT(has_merge, "bloque if_merge presente");
    ...
    VX_ASSERT(found_cmp, "CMP_LT presente en entry");
}
...
static void test_recursion() {
    ...
    VX_ASSERT(out.ok, "factorial recursivo lower ok");
    ...
    VX_ASSERT(fact != nullptr, "encontrada fact");
    ...
    VX_ASSERT(found_recursive_call, "fact se llama a si misma (CALL fact)");
}
...
static void test_multiple_types_aliases() {
    ...
    VX_ASSERT(out.ok, "aliases lower ok");
    for (const auto &f : out.irmod.functions) {
        VX_ASSERT(f.ret_type == ir::IrType::U32, "ret_type U32 para todas");
    }
}
...
static void test_while_supported_a2() {
    ...
    auto out = compile_to_ir("i32 main() {\n"
                             ...
                             "    return i;\n"
                             "}\n");
    VX_ASSERT(out.ok, "while soportado en A.2");
    ...
    VX_ASSERT(has_phi, "while genera PHI nodes");
}
...
static void test_for_supported_a2() {
    ...
    auto out =
        compile_to_ir("i32 main() {\n"
                      ...
                      "    return sum;\n"
                      "}\n");
    VX_ASSERT(out.ok, "for soportado en A.2");
}
...
static void test_assignment_supported_a2() {
    ...
    auto out = compile_to_ir("i32 main() {\n"
                             ...
                             "    return x;\n"
                             "}\n");
    VX_ASSERT(out.ok, "asignacion soportada en A.2");
}
...
static void test_compound_assign_supported_a2() {
    ...
    auto out = compile_to_ir("i32 main() {\n"
                             ...
                             "    return acc;\n"
                             "}\n");
    VX_ASSERT(out.ok, "compound assigns soportados en A.2");
}
...
static void test_increment_supported_a2() {
    ...
    auto out = compile_to_ir("i32 main() {\n"
                             ...
                             "    return x;\n"
                             "}\n");
    VX_ASSERT(out.ok, "++/-- soportados en A.2");
}
...
int main() {
    ...
    std::printf("\n=== test_vx_lowering: %d pasos OK, %d fallidos ===\n",
                g_passed, g_failed);
