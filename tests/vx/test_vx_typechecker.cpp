...
static void test_alias_styles_equivalent() {
    auto out = check_src("u32 a = 1;\n"
                         ...
                         "    return a + b;\n"
                         "}\n");
    VX_ASSERT(out.ok, "u32 == uint32_t -> mismo tipo");
}
...
static void test_forward_reference() {
    ...
    VX_ASSERT(out.ok, "forward ref ok");
}
...
static void test_typedef_alias_resolves() {
    ...
    VX_ASSERT(out.ok, "typedef u32 Edad ok");
}
...
static void test_using_alias_resolves() {
    ...
    VX_ASSERT(out.ok, "using MyInt = i64 ok");
}
...
static void test_alias_chain() {
    ...
    VX_ASSERT(out.ok, "cadena typedef A->B->C->u32 ok");
}
...
static void test_struct_decl_only() {
    ...
    auto out = check_src("struct Punto { f64 x; f64 y; }\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(out.ok, "struct declarativo ok (sin uso)");
}
...
static void test_struct_layout_simple() {
    ...
    auto out = check_src("struct Punto { f64 x; f64 y; }\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(out.ok, "struct Punto compila");
    ...
    for (auto &d : out.mod->decls) {
        if (d && d->kind == ast::NodeKind::StructDecl) {
            ...
            if (s->name == "Punto" && s->fields.size() == 2) {
                found = true;
                VX_ASSERT(s->fields[0].name == "x", "campo x");
                VX_ASSERT(s->fields[1].name == "y", "campo y");
            }
        }
    }
    VX_ASSERT(found, "Punto encontrado con sus campos");
}
...
static void test_struct_alias_combined() {
    ...
    auto out = check_src("struct Punto { f64 x; f64 y; }\n"
                         "typedef Punto P;\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(out.ok, "alias sobre struct ok");
}

static void test_struct_field_dup_error() {
    auto out = check_src("struct Bad { i32 x; i32 x; }\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(!out.ok, "campo duplicado detectado");
}
...
static void test_struct_redef_error() {
    auto out = check_src("struct A { i32 x; }\n"
                         "struct A { i32 y; }\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(!out.ok, "struct redefinido detectado");
}
...
static void test_complex_program_ok() {
    ...
    VX_ASSERT(out.ok, "programa fibonacci ok");
}
...
static void test_undeclared_name() {
    ...
    VX_ASSERT(!out.ok, "nombre no declarado detectado");
}
...
static void test_redef_global() {
    auto out = check_src("i32 a = 1;\n"
                         "u8  a = 2;\n"
                         "i32 main() { return 0; }\n");
    VX_ASSERT(!out.ok, "redefinicion de global detectada");
}
...
static void test_call_non_function() {
    auto out = check_src("i32 main() {\n"
                         ...
                         "    return x(1);\n"
                         "}\n");
    VX_ASSERT(!out.ok, "llamada a no-funcion detectada");
}
...
static void test_arithmetic_on_bool() {
    ...
    VX_ASSERT(!out.ok, "aritmetica sobre bool detectada");
}
...
static void test_call_inside_call() {
    ...
    VX_ASSERT(out.ok, "calls anidadas ok con promocion i64->i32");
}
...
int main() {
    ...
    std::printf("\n=== test_vx_typechecker: %d pasos OK, %d fallidos ===\n",
                g_passed, g_failed);
