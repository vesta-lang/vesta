...
int main() {
    ...
    std::printf("\n[sin bucles]\n");
    {
        ir::IrFunction fn; fn.name = "noloop";
        ...
        CHECK(!f.inside(0) && !f.inside(1), "bloques marcados in_loop sin bucle");
    ...
    std::printf("\n[un bucle]\n");
    {
        ir::IrFunction fn; fn.name = "oneloop";
        fn.blocks.push_back(block(0, "entry", br(1)));
        ...
        CHECK(f.depth_of(0) == 0 && f.depth_of(3) == 0, "entry/exit no depth 0");
    ...
    std::printf("\n[bucles anidados]\n");
    {
        ...
        CHECK(f.innermost(3) == f.innermost(2), "inner_b/inner_h distinto bucle interno");
        CHECK(f.innermost(3) != f.innermost(4), "inner y outer comparten id interno");
        // parent_loop: el bucle interno tiene como padre al externo; el externo no.
        const uint32_t inner_id = f.innermost(3);
        const uint32_t outer_id = f.innermost(1);
        CHECK(f.parent_of(inner_id) == outer_id, "padre del bucle interno no es el externo");
        CHECK(f.parent_of(outer_id) == LoopFacts::NO_LOOP, "el bucle externo tiene padre");
        CHECK(f.header_block_of(inner_id) == 2 && f.header_block_of(outer_id) == 1,
