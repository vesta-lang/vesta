// @vx-snippet: vx_instrument
// @vx-requires: vx_macros
// @vx-includes: stdio.h, stdlib.h, string.h, time.h
 ...
 *       vx_user_on_enter(name, depth, pid)
 *       vx_user_on_leave(name, value, elapsed_ns, depth, pid)
 *     Por defecto no-op; el usuario los puede sobrescribir linkando su
 *     propia implementacion (mismo nombre, no @c weak).
 *
 * El mode (trace vs profile) se selecciona via macro @c VX_INSTRUMENT_MODE
...
#ifndef VX_INSTRUMENT_MODE
#define VX_INSTRUMENT_MODE 1 /* trace */
#endif

#define VX_INSTR_TRACE (VX_INSTRUMENT_MODE & 1)
#define VX_INSTR_PROFILE (VX_INSTRUMENT_MODE & 2)
...
/* ----- Timing helpers ----- */
static VX_UNUSED uint64_t vt_now_ns_(void) {
#if defined(_WIN32)
...
static VX_UNUSED int vt_pid_(void) {
#if defined(_WIN32)
...
static VX_UNUSED int vt_tid_(void) {
#if defined(_WIN32)
...
#endif
    /* Permitir override via env var VX_TRACE_NO_COLOR. */
    if (vt_color_cached_) {
...
#define VX_TRACE_MAX_DEPTH 256
typedef struct VxTraceFrame {
    const char *name;
    uint64_t start_ns;
    int depth;
} VxTraceFrame;

static VX_TLS VxTraceFrame vt_stack_[VX_TRACE_MAX_DEPTH];
static VX_TLS int vt_depth_ = 0;

/* ----- Profile stats (lock-protected, global cross-thread) ----- */
typedef struct VxProfileEntry {
    const char *name; /* puntero al string literal en .rodata */
    uint64_t count;
    uint64_t total_ns;
    uint64_t min_ns;
    uint64_t max_ns;
    struct VxProfileEntry *next;
} VxProfileEntry;
...
#endif
static VxProfileEntry *vt_profile_head_ = 0;
...
static int vt_cmp_total_desc_(const void *a, const void *b) {
    const VxProfileEntry *pa = *(VxProfileEntry *const *)a;
    const VxProfileEntry *pb = *(VxProfileEntry *const *)b;
...
static void vt_profile_report_(void) {
    ...
    vt_lock_();
    int n = 0;
    for (VxProfileEntry *e = vt_profile_head_; e; e = e->next)
        n++;
    if (n == 0) {
        vt_unlock_();
        return;
...
static VxProfileEntry *vt_profile_find_or_add_(const char *name) {
    /* Buscar por puntero (name viene de @c .rodata; mismo string literal
     * mismo puntero).  Si no se encuentra, insertar al frente. */
    for (VxProfileEntry *e = vt_profile_head_; e; e = e->next) {
        if (e->name == name) return e;
    }
    VxProfileEntry *e = (VxProfileEntry *)calloc(1, sizeof(VxProfileEntry));
    if (!e) return 0;
    e->name = name;
    e->min_ns = (uint64_t)-1;
...
static VX_UNUSED void vx_trace_enter(const char *fn_name) {
    vt_profile_init_();
    if (!vt_atexit_reg_) {
        vt_atexit_reg_ = 1;
#if VX_INSTR_PROFILE
    ...
    vx_user_on_enter(fn_name, d, vt_pid_());
}

static VX_UNUSED void vx_trace_leave(const char *fn_name, int64_t value) {
    int new_depth = vt_depth_ - 1;
    if (new_depth < 0) new_depth = 0;
    vt_depth_ = new_depth;

    uint64_t elapsed = 0;
    if (new_depth < VX_TRACE_MAX_DEPTH) {
    ...
    vt_lock_();
    VxProfileEntry *e = vt_profile_find_or_add_(fn_name);
    if (e) {
        e->count++;
        e->total_ns += elapsed;
    ...
    vx_user_on_leave(fn_name, value, elapsed, new_depth, vt_pid_());
}
