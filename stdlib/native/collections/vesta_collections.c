...
static int vlist_grow(VestaList *l) {
    ...
    uint32_t *new_data =
        (uint32_t *)realloc(l->data, new_cap * sizeof(uint32_t));
...
VESTA_PLUGIN_EXPORT uint64_t vcol_list_new(uint64_t initial_capacity) {
    uint64_t cap =
        initial_capacity < VLIST_MIN_CAP ? VLIST_MIN_CAP : initial_capacity;
...
VESTA_PLUGIN_EXPORT uint64_t vcol_list_push(uint64_t handle, uint64_t element) {
    ...
    l->data[l->size++] =
        (uint32_t)(element & 0xFFFFFFFFu); /* almacenar 32 bits bajos */
    return l->size;
}
...
static int valist_grow(VestaArrayList *al) {
    ...
    uint64_t *new_data =
        (uint64_t *)realloc(al->data, new_cap * sizeof(uint64_t));
    if (!new_data) return 0;
    al->data = new_data;
    al->cap = new_cap;
    return 1;
...
VESTA_PLUGIN_EXPORT uint64_t vcol_alist_new(uint64_t initial_capacity) {
    uint64_t cap =
        initial_capacity < VALIST_MIN_CAP ? VALIST_MIN_CAP : initial_capacity;
 ...
 *   [ ctrl[16]: 16 B | keys[16]: 128 B | vals[16]: 128 B ] = 272 B (~4.25
 * lineas). El acceso a ctrl es vectorial (16 bytes en 1 carga SSE2); luego,
 * para los 1-3 matches probables se accede a keys[i].
 *
...
typedef struct VestaHashMap {
    ...
    uint64_t
        threshold; /**< rehash cuando size >= threshold (87.5% del total). */
} VestaHashMap;
...
static int vmap_rehash(VestaHashMap *m) {
    ...
    VMapGroup *new_groups =
        (VMapGroup *)malloc(new_group_cnt * sizeof(VMapGroup));
    if (!new_groups) return 0;
    /* inicializar todos los control bytes a EMPTY (bit 7 alto). */
    memset(new_groups, VMAP_CTRL_EMPTY, new_group_cnt * sizeof(VMapGroup));
    ...
    for (uint64_t g = 0; g < m->group_cnt; ++g) {
        ...
        for (uint32_t i = 0; i < VMAP_GROUP_W; ++i) {
            ...
            if ((c & 0x80) == 0) {
                /* h2 valido (bit 7 = 0) */
                vmap_insert_fresh(new_groups, new_group_cnt, grp->keys[i],
                                  grp->vals[i]);
            }
        }
    }
...
VESTA_PLUGIN_EXPORT uint64_t vcol_map_new(uint64_t initial_capacity) {
    ...
    uint64_t cap =
        initial_capacity < VMAP_MIN_CAP ? VMAP_MIN_CAP : initial_capacity;
    cap = next_pow2(cap);
...
VESTA_PLUGIN_EXPORT uint64_t vcol_set_remove(uint64_t handle,
                                             uint64_t element) {
...
VESTA_PLUGIN_EXPORT uint64_t vcol_set_is_subset(uint64_t set_a,
                                                uint64_t set_b) {
...
VESTA_PLUGIN_EXPORT uint64_t vcol_set_intersect(uint64_t set_a,
                                                uint64_t set_b) {
    VestaHashMap *a = (VestaHashMap *)(uintptr_t)set_a;
...
VESTA_PLUGIN_EXPORT uint64_t vcol_queue_new(uint64_t initial_capacity) {
    uint64_t cap = next_pow2(
        initial_capacity < VQUEUE_MIN_CAP ? VQUEUE_MIN_CAP : initial_capacity);
 ...
 *   4. Cada camino de la raiz a una hoja NIL tiene mismo numero de nodos
 * NEGROS.
 *
...
static VTreeNode *vtree_make_node(VestaTreeMap *t, uint64_t key,
                                  uint64_t value) {
...
VESTA_PLUGIN_EXPORT uint64_t vcol_tmap_floor_key(uint64_t handle,
                                                 uint64_t key) {
    VestaTreeMap *t = (VestaTreeMap *)(uintptr_t)handle;
    if (!t) return VCOL_NOT_FOUND;
    VTreeNode *cur = t->root;
...
VESTA_PLUGIN_EXPORT uint64_t vcol_tmap_ceiling_key(uint64_t handle,
                                                   uint64_t key) {
    VestaTreeMap *t = (VestaTreeMap *)(uintptr_t)handle;
    if (!t) return VCOL_NOT_FOUND;
    VTreeNode *cur = t->root;
...
VESTA_PLUGIN_EXPORT uint64_t vstr_indexof_from(uint64_t haystack_ptr,
    ...
    const unsigned char *n = (const unsigned char *)(uintptr_t)needle_ptr;
    if (!h || !n) return VCOL_NOT_FOUND;
    const void *p = vstr_memmem(h + start, (size_t)(haystack_len - start), n,
                                (size_t)needle_len);
    return p ? (uint64_t)((const unsigned char *)p - h) : VCOL_NOT_FOUND;
}
...
VESTA_PLUGIN_EXPORT uint64_t vstr_ends_with(uint64_t haystack_ptr,
    ...
    const void *n = (const void *)(uintptr_t)needle_ptr;
    if (!h || !n) return 0;
    return memcmp(h + (haystack_len - needle_len), n, (size_t)needle_len) == 0
               ? 1u
               : 0u;
}
...
VESTA_PLUGIN_EXPORT uint64_t vstr_trim_start_offset(uint64_t ptr,
                                                    uint64_t len) {
    const unsigned char *p = (const unsigned char *)(uintptr_t)ptr;
    if (!p) return 0;
    uint64_t i = 0;
    while (i < len && vstr_is_ws(p[i]))
        ++i;
    return i;
}
...
VESTA_PLUGIN_EXPORT uint64_t vstr_trim_end_strip(uint64_t ptr, uint64_t len) {
    ...
    if (!p) return 0;
    uint64_t strip = 0;
    while (strip < len && vstr_is_ws(p[len - 1 - strip]))
        ++strip;
    return strip;
}
...
VESTA_PLUGIN_EXPORT uint64_t vstr_split_offsets(uint64_t haystack_ptr,
    ...
    if (delim_len == 0 || !d) {
        vcol_alist_push(list,
                        ((uint64_t)0 << 32) | (haystack_len & 0xFFFFFFFFu));
        return list;
    }
    uint64_t cur = 0;
    while (cur <= haystack_len) {
        ...
        if (cur < haystack_len) {
            m = (const unsigned char *)vstr_memmem(
                h + cur, (size_t)(haystack_len - cur), d, (size_t)delim_len);
        } else {
            m = NULL;
        }
        if (!m) {
...
VESTA_PLUGIN_EXPORT void vcol_set_clear_gc(uint64_t proc, uint64_t handle) {
    ...
    if (m) {
        for (uint64_t g = 0; g < m->group_cnt; ++g) {
            ...
            for (uint32_t i = 0; i < VMAP_GROUP_W; ++i) {
                if ((grp->ctrl[i] & 0x80) == 0) {
                    /* Para set, los keys SON los elementos (con value
                     * sentinel). */
