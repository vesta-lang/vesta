/*
 * Consumidor C del modulo Vex 205_c_interop.vex (Fase 4 interop C).
 *
 * Demuestra que codigo C llama a funciones Vex sin friccion: incluye el
 * header generado por `vex --emit-header` y enlaza el .c del port-C.
 *
 * Build (lo hace el e2e automaticamente):
 *   vex --vex 205_c_interop.vex --port c --emit-header -o iface
...
#include <stdio.h>
#include VEX_IFACE_HEADER /* el .h generado por vex --emit-header */
