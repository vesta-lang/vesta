...
class Platform {
  public:
    ...
    /// Mensaje informativo para el wizard ("Run as Administrator", "sudo",
    /// etc.).
    ...
    default_bin_dir(const std::filesystem::path &prefix) const = 0;
    virtual std::filesystem::path
    default_share_dir(const std::filesystem::path &prefix) const = 0;
    ...
    virtual bool register_associations(const InstallOptions &opts,
                                       Manifest &mf) = 0;
    ...
    virtual bool register_uninstaller(const InstallOptions &opts,
                                      Manifest &mf) = 0;
