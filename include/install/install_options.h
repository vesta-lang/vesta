...
struct AssocOpt {
    ...
    std::string
        command_template; ///< Argumentos: "--script \"%1\"", "--run \"%1\""
};
...
struct InstallOptions {
    ...
    std::filesystem::path
        share_dir; ///< prefix/share (Linux) o prefix (Windows)

    // ------------------------------------------------------------------
    // Origen de los ficheros (auto-detectado)
    // ------------------------------------------------------------------
    std::filesystem::path
        source_dir; ///< Donde esta el binario actual ejecutandose
    ...
    /**
     * @brief Calcula prefix, bin_dir y share_dir segun el scope si estan
     * vacios.
     *
