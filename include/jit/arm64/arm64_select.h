 ...
 *
 * Baja una @c ir::IrFunction a TEXTO ensamblador AArch64 que Keystone ensambla
 * a bytes.  Es el primer paso del backend arm64: un template slot-por-valor
 * (cada valor SSA vive en un hueco de la pila; cada op carga operandos a
 * scratch, computa y guarda el resultado), analogo al C1 de x86.  Cubre el
 * subconjunto entero de linea recta
 * (CONST/MOV/ADD/SUB/MUL/AND/OR/XOR/NEG/NOT/SHL/SHR/RET) + la ABI AAPCS64
 * basica (params en x0..x7, retorno en x0).  Las ops no cubiertas (ramas,
 * floats, llamadas, memoria) marcan @c out_unsupported para caer a otra ruta --
 * se anaden en H.2b+.
 */
 ...
 *        retorno x0, callee-saved x19-x28, LR=x30); el enum fija el seam para
 * las divergencias futuras (Windows __chkstk, Apple variadicos en pila...).
 */
enum class Arm64Abi {
 ...
 *        placeholder).  Como el control de flujo local usa @c b/@c b.cond/@c
 * cbnz (nunca @c bl), TODO @c bl del cuerpo es una llamada: el backend empareja
 ...
 */
std::string
arm64_emit_asm(const ir::IrFunction &fn, bool &out_unsupported,
