...
/**
 * @file debug_info.h
 * @brief Estructuras y clase de gestion de informacion de depuracion para VestaVM.
 *
...
namespace debug {
    ...
    static constexpr uint8_t  VAR_FLAG_CAPTURE  = 2u; ///< variable capturada (closure)
...
#   define DEBUG_PACKED
    ...
    struct DEBUG_PACKED DebugSectionHeader {
        ...
        uint32_t strings_size;  ///< tamano del blob de cadenas de depuracion en bytes
    };
     ...
     * El nombre del archivo se obtiene indexando el blob de cadenas con file_offset.
     *
    ...
    struct DEBUG_PACKED DebugLineEntry {
        ...
        uint32_t file_offset;     ///< offset al nombre del archivo en el blob de strings
    };

    // -------------------------------------------------------------------------
    //  DebugLineEntry3 - mapeo bytecode -> archivo, linea y columna (nivel 3, 16 bytes)
    // -------------------------------------------------------------------------
    ...
    struct DEBUG_PACKED DebugVarEntry {
        uint32_t name_offset;    ///< offset al nombre de la variable en el blob de strings
    ...
    // -------------------------------------------------------------------------
    //  DebugInfo - clase de consulta de informacion de depuracion en tiempo de ejecucion
    // -------------------------------------------------------------------------
     ...
     */
    struct LineInfo {
        const char *file;    ///< nombre del archivo fuente (puntero al blob de strings)
    ...
    public:
         ...
         *
         * @param data  Puntero al inicio de la seccion (comienza con DebugSectionHeader).
         ...
         * @return LineInfo con la informacion encontrada; found=false si no hay datos.
         */
    ...
    private:
        ...
        const DebugLineEntry3 *lines3_  = nullptr; ///< tabla de nivel 3 (16B/entrada)
