...
enum class DebugCmd : uint8_t {
    ...
    GC_STATS =
        18, ///< estadisticas del GC (handles vivos, nursery, oldgen, ciclos)
    GC_HANDLES =
        19, ///< lista de handles GC vivos con clase + payload addr + size
    ...
    LOAD_VELB = 36, ///< cargar un .velb desde el filesystem del server; arranca
                    ///< un nuevo proceso (opcional pause_at_start)
    KILL_PROC =
        37, ///< marcar un proceso como DEAD para que el scheduler lo libere
    SERVER_INFO = 38, ///< informacion del servidor (uptime, schedulers,
                      ///< num_procesos, persistente, etc.)
    SERVER_SHUTDOWN =
        39, ///< apagar la VM persistente y salir del modo --server-mode
...
class Debugger {
  public:
     ...
     */
    void on_message(uint64_t pid, bool is_send, uint64_t target,
                    uint64_t payload64);
     ...
     */
    void on_monitor_contention(uint64_t pid, uint64_t handle, uint64_t owner);
  ...
  private:
    ...
    bool auth_required_ =
        false; ///< exigir session_token en comandos no publicos

    std::atomic<bool> running_{false}; ///< estado del servidor
    std::atomic<bool> any_bp_{false};  ///< hay breakpoints activos (fast path)
    std::atomic<bool> any_step_{
        false}; ///< hay procesos en step/next o pausados (fast path)
    std::atomic<bool> any_watch_{
        false}; ///< hay watchpoints activos (fast path slow)
    std::atomic<bool> any_msg_trace_{
        false}; ///< hay procesos en msg-trace (fast path)
    std::atomic<bool> break_on_mon_{
        false}; ///< break-on-monitor-contention global
     ...
     */
    std::string mem_to_json(runtime::ProcessVM *proc, uint64_t addr,
                            uint32_t length);
};
