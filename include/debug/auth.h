...
/**
 * @file auth.h
 * @brief Sistema de autenticacion y autorizacion para el servidor de
 * depuracion.
 *
...
class AuthManager {
  public:
    ...
    bool create_user(const std::string &username, const std::string &password,
                     Role role, std::string *err = nullptr);
  ...
  private:
     ...
     */
    bool derive_key(const std::string &password, const uint8_t *salt,
                    size_t salt_len, uint8_t *out_hash, size_t out_len);
