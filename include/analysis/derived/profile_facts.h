 ...
 *   - TIPO B (derivados): COMBINAN varios productores.  ProfileFacts,
 * CostFacts, HotnessFacts, ExecutionFacts.  Viven en @c analysis/derived/
 * (aqui).
 ...
 * CFG (¿que rama sale?, ¿cual continua?, ¿varios exits?, switches,
 * irreducibles, bucles anidados).  Eso ya es un ANALISIS, no un fact trivial.
 * El camino CORRECTO es por capas:
 *
 *     runtime  ->  BranchProfile  ->  CFGProfile   ->  LoopProfile  ->
 * ProfileFacts (g_profile)   (conteos/linea)   (bloques/aristas)  (trip-count)
 * (por bloque)
 *
 ...
 * refinar con @c CFGProfile/@c LoopProfile cuando un consumidor exija
 * precision.
 *
...
/**
 * @brief Computa los @c ProfileFacts de una funcion (v1 aproximado, ver
 * arriba).
