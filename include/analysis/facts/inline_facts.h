...
struct InlineFacts {
    ...
    /// `@Inline`: el programador pide que entre aunque no quepa por tamano.
    /// Levanta el UMBRAL y nada mas: las reglas que dicen que no se PUEDE
    /// -- recursiva, asm crudo, @Naked -- siguen en pie, porque esas no son
    /// de conveniencia.
    ...
    /// Reserva en pila.  Ya NO prohibe inlinar: el inlinador iza la reserva al
    /// bloque de entrada del llamante, que corre una vez, asi que la pila no
    /// crece aunque la llamada este en un bucle.  Se sigue recogiendo porque
    /// el hecho describe lo que la funcion ES, y quien pregunte decidira.
