 ...
 * informacion ESTA en el modulo y solo hay que seguirla: la direccion se toma en
 * un sitio, viaja por un par de copias, y se llama en otro.  Rendirse ahi obliga
 * a cualquier consumidor a suponer lo peor sobre codigo que se puede leer
 * entero.
 *
 * Dos preguntas, que son las dos caras de lo mismo:
 *
 *   DESDE LA LLAMADA   `¿a que funcion apunta este valor?`  La usa quien esta en
 *                      un `call` indirecto y quiere el destino concreto.
 *   DESDE LA FUNCION   `¿se ven TODOS los sitios que pueden llamarme?`  La usa
 *                      quien quiere afirmar algo sobre los parametros, que solo
 *                      es licito conociendo a todos los llamantes.
 *
 * El limite esta declarado, no escondido: se sigue por copias, reinterpretaciones
 * y huecos escritos UNA sola vez.  En cuanto la direccion se guarda en un sitio
 * que se escribe mas de una vez, se pasa como argumento o se devuelve, se dice
 * que no se sabe.  Es preferible quedarse corto a afirmar de mas: sobre esto se
 * apoyan comprobaciones que pueden rechazar un programa.
 */
...
struct DireccionTomada {
    ...
    bool tomada = false;
    /// Si TODOS esos sitios acaban en llamadas indirectas que se ven.  Cuando es
    /// cierto, la lista de abajo completa el censo de llamantes.
