...
/**
 * @file cli.h
 * @brief Interfaz del interprete interactivo (REPL) y del gestor de vista de la
 * CLI de VestaVM.
 *
 * Define:
 *   - CmdParams: estructura con los campos parseados de un comando de usuario.
 *   - extract_token(): extrae un token de una cadena respetando comillas y
 * escapes.
 *   - parse_cmd_params(): parsea name + path + rest de una linea de comando.
 *   - VestaInterprete: REPL basico que ensambla y ejecuta fragmentos de codigo
 * Vesta.
 *   - Config: opciones de configuracion del REPL (historial, prompt, terminador
 * multilinea).
 *   - VestaViewManager: gestor del REPL con historial, callback de ejecucion y
 * soporte multihilo.
 */
...
/**
 * @brief Resultado del parseo de una linea de comando con nombre, ruta y
 * argumentos extra.
 *
...
struct CmdParams {
    ...
    std::string
        path; ///< Segundo token: ruta al ejecutable o argumento principal
    std::string
        rest; ///< Resto de la cadena tras name y path (puede estar vacio)
    bool valid =
        false; ///< true si se extrajeron al menos name y path correctamente
};

/**
 * @brief Extrae un token de la cadena @p s a partir de la posicion @p pos.
 *
 * Salta espacios iniciales y acumula caracteres hasta encontrar un separador de
 * token. Respeta comillas simples y dobles: los caracteres dentro de comillas
 * se toman literalmente. Soporta escapes con backslash tanto dentro como fuera
 * de comillas.
 *
 * @param s   Cadena de entrada completa.
 * @param pos Posicion de inicio en @p s (se actualiza a la posicion posterior
 * al token).
 * @return Token extraido; cadena vacia si la posicion esta al final de la
 * cadena.
 */
static std::string extract_token(const std::string &s, size_t &pos) {
    ...
    while (pos < s.size()) {
        ...
        if (quote) {
            if (c == '\\' && pos < s.size()) {
                // secuencia de escape dentro de comillas: tomar el siguiente
                // caracter literalmente
                char next = s[pos++];
                out.push_back(next);
            } else if (c == quote) {
            ...
            if (std::isspace(static_cast<unsigned char>(c))) {
                ...
                // escape fuera de comillas: permite espacios escapados dentro
                // del token
                char next = s[pos++];
                out.push_back(next);
            } else {
 ...
 * y ejecutarlas en una instancia VM interna.  Usado principalmente para
 * depuracion y pruebas rapidas de instrucciones.
 */
class VestaInterprete {
    ...
    runtime::ManageVM manager =
        runtime::ManageVM(nullptr, 0); ///< Gestor VM interno
  ...
  public:
    ...
    /**
     * @brief Ensambla el codigo acumulado en @p code y lo ejecuta en la VM
     * interna.
     *
    ...
    /**
     * @brief Inicia el bucle interactivo del interprete (bloqueante hasta que
     * el usuario sale).
     */
    void run_interprete();
};
...
/**
 * @brief Parsea la linea @p cmd extrayendo name (primer token) y path (segundo
 * token).
 *
 * El resto de la cadena (tras saltarse los espacios que siguen a path) se
 * almacena en CmdParams::rest tal cual, para que el manejador del comando lo
 * procese segun necesite.
 *
 * @code{.cpp}
 * auto p1 = parse_cmd_params(R"(myname /home/user/vm.bin)");
 * // p1.name == "myname", p1.path == "/home/user/vm.bin"
 *
 * auto p2 = parse_cmd_params(R"("My Name" "C:\Archivos de programa\VM\vm.exe"
 * --flag)");
 * // p2.name == "My Name", p2.path == "C:\Archivos de programa\VM\vm.exe",
 * p2.rest == "--flag"
 * @endcode
 *
 * @param cmd Cadena completa del comando introducido por el usuario.
 * @return CmdParams con los campos rellenos; valid=false si no se pudo extraer
 * name+path.
 */
...
struct Config {
    std::string history_file =
        "vm_history.txt"; ///< Ruta del fichero de historial de comandos
    std::string aliases_file =
        "vm_aliases.txt";      ///< Ruta del fichero de persistencia de aliases
    ...
    std::string prompt = "vm> "; ///< Cadena mostrada como prompt del REPL
                                 ///< (legacy; no usado con prompt dinamico)
 ...
 *
 * VestaViewManager implementa el bucle de lectura de comandos con historial
 * persistente, un callback de ejecucion inyectable y deteccion de modo
 * multilinea.  Internamente usa un hilo de trabajo para procesar comandos sin
...
class VestaViewManager {
  public:
     ...
     *
     * @param cfg Opciones de configuracion (prompt, historial, terminador
     * multilinea).
     */
    ...
    /**
     * @brief Registra el callback que se invoca cada vez que el usuario
     * introduce un comando.
     *
     * El callback recibe la linea completa (o el bloque multilinea completo) ya
     * validada.
     *
     ...
     *
     * Lee lineas de stdin, gestiona el historial y delega la ejecucion al
     * callback.
     */
    void run();
  ...
  private:
    ...
    Impl *
        impl_; ///< Implementacion interna (opaque pointer, definida en cli.cpp)
};
