 ...
 * @brief API de ejecucion de comandos sincrona y asincrona para el runtime de la CLI.
 *
 * Proporciona funciones para ejecutar comandos de texto en el runtime de VestaVM de forma
 * sincrona (bloqueante con timeout opcional) o asincrona (via future), junto con las
 * funciones de inicializacion y cierre del thread pool interno del runtime.
 *
 * Uso tipico:
 * @code
 *   runtime::runtime_init(4);                       // inicializar con 4 hilos
 *   std::string out = runtime::run_command_sync("run program.velb"); // ejecutar sincrono
 *   runtime::runtime_shutdown();                     // cerrar al terminar
 * @endcode
 */
...
namespace runtime {
    ...
    /**
     * @brief Token de cancelacion compartido entre el emisor y el ejecutor de un comando.
     *
     * Cuando el booleano atomico se pone a true el ejecutor debe abortar la ejecucion
     * tan pronto como sea posible.  Se crea con make_cancel_token().
     */
    using CancelToken = std::shared_ptr<std::atomic<bool>>;

    /**
     * @brief Crea un nuevo token de cancelacion inicializado a false (sin cancelar).
     *
     ...
     * @param cancel_token Token de cancelacion (puede ser nullptr para ignorar cancelacion).
    ...
    /**
     * @brief Inicializa el thread pool interno del runtime con el numero de hilos indicado.
     *
     * Si no se llama antes de usar run_command_async(), se crea automaticamente un pool
     * con std::thread::hardware_concurrency() hilos.  Llamar multiples veces no tiene efecto.
     *
     ...
     *
     * Debe llamarse antes de salir del programa para garantizar que no quedan tareas
     * en vuelo ni hilos del pool activos.
     */
