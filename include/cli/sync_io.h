 ...
 *   - La clase SyncOStream: stream con bloqueo atomico al destruirse o al
 * recibir std::endl.
 *   - La funcion helper scout() para obtener un SyncOStream temporal en una
 * expresion.
 *   - Traits has_to_string / has_print_ostream para deteccion de metodos de
 * serializacion.
 *   - component_to_string() para convertir cualquier objeto a cadena de forma
 * generica.
 *   - hex64() / hex8() para formatear enteros como cadenas hexadecimales.
 *   - dump() para volcar bloques de memoria en formato hex legible.
 *   - DebugLogger: logger de diagnostico que escribe en decode_log.txt con dump
 * de memoria.
...
/**
 * @brief Mutex global para sincronizar accesos a std::cout / std::cerr entre
 * hilos.
 *
 * Definido en sync_io.cpp.  Incluir este header en cualquier modulo que
 * necesite imprimir desde varios hilos para evitar salidas mezcladas.
 */
...
class SyncOStream {
  public:
    /**
     * @brief Construye un SyncOStream que vuelca en @p out_stream al
     * destruirse.
     *
     * @param out_stream Stream de salida destino (por defecto std::cout).
     */
    explicit SyncOStream(std::ostream &out_stream = std::cout)
        : out(out_stream) {}
    ...
    /**
     * @brief Destructor: vuelca el buffer acumulado en el stream de salida bajo
     * mutex.
     */
    ~SyncOStream();
     ...
     */
    template <typename T> SyncOStream &operator<<(T &&value) {
        oss << std::forward<T>(value); // acumular sin bloqueo
        return *this;
    }

    /**
     * @brief Sobrecarga para manipuladores de stream (std::endl, std::flush,
     * etc.).
     *
     * Aplica el manipulador al buffer interno. Si es std::endl o std::flush
     * fuerza un volcado inmediato bajo mutex para que la salida sea visible al
     * instante.
     *
...
/**
 * @brief Crea un SyncOStream temporal para uso en expresiones de una sola
 * linea.
 *
...
struct has_to_string<
    T, std::void_t<decltype(std::declval<const T &>().to_string())>>
    : std::true_type {};

/**
 * @brief Trait que detecta si el tipo T tiene un metodo print(std::ostream&)
 * const.
 *
 * @tparam T Tipo a inspeccionar.
 */
 ...
 *
 * Devuelve "<null>" si el puntero es nulo, o "ptr=<direccion>" en caso
 * contrario.
 *
 ...
 */
template <typename T> std::string component_to_string_fallback(const T *p) {
    if (!p) return "<null>";
...
/**
 * @brief Convierte un objeto a cadena usando to_string(), print(ostream&) o
 * fallback.
 *
 ...
 */
template <typename T> std::string component_to_string(const T &obj) {
 ...
 */
template <typename T> std::string component_to_string(T *p) {
    if (!p) return "<null>";
...
/**
 * @brief Formatea un entero de 64 bits como cadena hexadecimal con prefijo "0x"
 * y relleno de ceros.
 *
 * Produce siempre 16 digitos hexadecimales para representar de forma
 * consistente direcciones o valores de 64 bits.
 *
 ...
 * @return Cadena de dos caracteres con la representacion hexadecimal (p.ej.
 * "FF", "0A").
 */
...
/**
 * @brief Vuelca un bloque de memoria real como cadena hex con grupos de 8 bytes
 * por linea.
 *
 * @param code            Puntero al inicio del bloque de memoria a volcar.
 * @param size_dump_bytes Numero de bytes a incluir en el volcado.
 * @return Cadena con los bytes en hexadecimal separados por espacios y saltos
 * de linea.
 */
 ...
 * de linea.
 */
static std::string dump(vm::VirtualMemory vm_mem, uint16_t v_address,
                        uint16_t size_dump_bytes) {
    std::string dump_;
    for (int i = 1; i <= size_dump_bytes; i++) {
        uint8_t b =
            vm_mem[v_address + i - 1]; // leer byte por direccion virtual
        dump_ += hex8(b) + " ";
        if (((i % 8) == 0)) dump_ += "\n";
    }
    return dump_;
...
class DebugLogger {
  public:
    ...
    /**
     * @brief Vuelca un bloque de memoria en el log con formato hex de 16 bytes
     * por linea.
     *
     * Imprime la cabecera "[MEMORY DUMP] size=<N> bytes" y despues las filas
     * con offset en hex y los bytes de cada fila.
     *
    ...
    void dump_memory(const void *ptr, size_t size) {
        ...
        for (size_t i = 0; i < size; i += 16) {
            ...
            file_ << "    " << std::setw(4) << std::setfill('0') << std::hex
                  << i << ": ";
...
/**
 * @brief Construye una cadena concatenando todos los argumentos mediante
 * fold-expression.
 *
