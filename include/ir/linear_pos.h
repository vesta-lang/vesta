 ...
 * @brief @c ir::LinearPos: posicion lineal en el dominio del IR (la numeracion de
 *        @c compute_liveness -- 1 posicion por instruccion, PHI en block_end del
 *        predecesor).
 *
 * TIPO FUERTE, a proposito.  Es un @c uint32_t envuelto que NO se convierte
 * implicitamente a/desde @c uint32_t NI a @c codegen::LinearPos (el dominio MachineIR,
 * 2 posiciones por instruccion).  Asi el compilador RECHAZA mezclar dominios --
 * p.ej. pasar el @c now del allocator (que vive en MachineIR) a un analisis del
 * IR como @c UseDefFacts.  Mueve la regla "no cruzar niveles" del IR al MachineIR
 * de una REVISION HUMANA a una PROPIEDAD DEL SISTEMA DE TIPOS (igual que
 * @c LaneHazard hizo con las restricciones de lane).  El almacenamiento interno de
 * los Facts sigue siendo @c uint32_t crudo; el tipo fuerte vive en la INTERFAZ.
 *
 * REGLA GENERAL: un Fact PERTENECE a un dominio -- su ValueId y su Position son de
 * ese nivel.  El mismo concepto puede existir en varios (p.ej. el next-use:
 ...
 * compilador.
 *
 * El ESTADO "invalido" (sin proximo uso / sin definir / centinela) se expresa con
...
struct LinearPos {
    ...
    /// uso, sin definir, ...).  Encapsula el valor magico para que el codigo use
    ...
    uint32_t operator-(LinearPos o) const noexcept {
        assert(value >= o.value && "ir::LinearPos::operator-: precondicion this >= o");
