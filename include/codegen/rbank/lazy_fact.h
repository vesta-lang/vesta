 ...
 * @brief LazyFact<T>: celda de hecho computado UNA VEZ, cacheado y demand-driven.
 *
 * Encapsula el patron "compute-si-falta + cache" del query system del snapshot,
 * de modo que el @c mutable y el estado @c ready viven en UN SOLO SITIO (aqui) en
 * vez de estar dispersos por cada campo del snapshot.  Eso permite anadir el dia
 * de manana METRICAS (cuantas veces se pidio), INVALIDACION (@c reset) o
 ...
 * consulta no cambia el conocimiento observable, solo lo materializa).  La celda
 * se guarda POR VALOR (no @c mutable) en el snapshot -> la mutabilidad queda
 * ENCAPSULADA.  No es thread-safe por ahora (compile por-funcion single-thread);
 * la sincronizacion se anadiria AQUI sin cambiar a los consumidores.
 *
 * DIRECCION FUTURA (Query Engine): el siguiente paso natural es @c LazyFact<T,
 * Producer> o un @c Query<T> donde la dependencia NO se resuelva a mano, sino que
 * el QueryEngine conozca el productor REGISTRADO para cada T (@c loops.get(*this)
...
public:
     ...
     * @param produce  functor sin args que devuelve @c T (se llama a lo sumo 1 vez).
     */
    template <typename Producer>
    const T &get(Producer &&produce) const {
        if (!ready_) {
     ...
     *        constructor/tests deberian usarlo.  Deja claro QUIEN puede modificar.
     */
