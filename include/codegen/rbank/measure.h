 ...
 *        mejoras del asignador para decidir CUAL paga el alquiler primero
 * (medir antes de construir).  Principio: el allocator NO derrama por un PAR
 * sino por PRESION; por eso el instrumento decisivo es (c) el pico de presion.
 *
 *   (a) spills EVITABLES por rematerializacion: de los valores que rbank
 * derrama, cuantos son RECOMPUTABLES (RematFacts) -> se podrian recomputar en
 * el uso en vez de reload.  Combina IR (recomputable) con la asignacion real.
 ...
 *       (con huecos) NO solapan.  Cada una es presion inventada -- PROXY de
 * (c). (c) PICO DE PRESION envolvente vs exacto: el maximo de intervalos vivos
 * a la vez por clase.  El allocator derrama cuando el pico supera los lanes; el
 ...
 *
 * Es diagnostico: NO cambia el codigo emitido.  El numero decide el peldano
 * real.
 *
 * DIRECCIONES (documentadas, no ahora):
 *   - COSTE, no valores: (a) cuenta valores recomputables, pero el objetivo
 * ultimo son CICLOS recuperables (estimated_reload vs estimated_remat via
 *     MachineCostFacts).  Separar "es recomputable" de "merece la pena" cuando
 * el coste HW entre en la ecuacion.
 *   - GENERICO: el patron (optimizacion -> instrumento que mide -> decision)
 * sirve igual a scheduler / vectorizer / inlining / LICM / GVN.  Cuando exista
 * un segundo consumidor, extraer el molde a un sitio comun (hoy es de rbank).
 */
...
struct RematMeasure {
    ...
    uint32_t spills_rematerializable =
        0; ///< de esos, recomputables (techo bruto).
    uint32_t spills_remat_leaf =
        0; ///< de esos, HOJA (0 operandos: CONST/dir) ->
...
/** @brief (a) Mide sobre la asignacion de rbank cuantos spills son
 * recomputables, separando los HOJA (sin operandos: recompute puro, ganancia
 * casi segura) de los que necesitan mantener operandos vivos (ganancia
 * condicional al coste). */
...
/**
 * @struct RematDetail
 * @brief Dirige el esfuerzo al NIVEL correcto (no "¿construyo?" -- el Fact ya
 * vale):
 *          - HOTNESS: spills recomputables DENTRO de un loop (pagan) vs frios
 * (ruido).
 *          - RAZON de que el CONST llegue como valor separado y se derrame:
 *            imm que CABE en 32 bits -> deberia fusionarse como immediate AGUAS
 * ARRIBA (selector, nivel mas alto); imm64 -> genuinamente necesita remat/mov
 * imm64.
 */
struct RematDetail {
    uint32_t spills_in_loop =
        0; ///< META: TODOS los spills con loop_depth > 0 (hot).
    ...
    uint32_t const_imm32 =
        0; ///< CONST spilled con imm de 32 bits (fusionable arriba).
    uint32_t const_imm64 =
        0; ///< CONST spilled con imm de 64 bits (necesita remat real).

    void add(const RematDetail &o) noexcept {
        spills_in_loop += o.spills_in_loop;
        spills_cold += o.spills_cold;
        remat_in_loop += o.remat_in_loop;
        remat_cold += o.remat_cold;
        const_imm32 += o.const_imm32;
        const_imm64 += o.const_imm64;
    }
};

/**
 * @brief Detalle de los spills recomputables: hotness + por que el CONST no se
 * fusiono como immediate.  El instrumento (algoritmo) solo CONSULTA Facts, no
 * los reinventa: la hotness por valor ya es un Fact (@c
 * ValueRequirements.loop_depth, poblado desde LoopFacts) y el imm ya vive en @c
 * RematFacts.recipe.  No itera el IR ni recomputa el loop_depth -- dato (Facts)
 * separado del algoritmo.
 */
inline RematDetail
measure_remat_detail(const codegen::RegAlloc &ra,
                     const analysis::RematFacts &remat,
 ...
 *        interferencia = par que el envolvente cree interferente pero cuyos
 * rangos reales no solapan (huecos) -> presion inventada.
 */
struct EnvelopeMeasure {
    ...
    uint64_t false_interfere =
        0; ///< envolvente SI, exacto NO (deuda del envolvente).
...
/** @brief ¿Los envolventes [a.from,a.to) y [b.from,b.to) solapan?
 * (semi-abierto). */
...
/** @brief ¿Algun par de segmentos REALES de @p a y @p b solapa? (merge-walk
 * O(n+m)). */
inline bool segments_overlap(const jit::LiveInterval &a,
    ...
    while (i < a.ranges.size() && j < b.ranges.size()) {
        ...
        if (ra.to <= rb.to)
            ...
            ++j; // avanza el que acaba antes.
    }
    return false;
}

/**
 * @brief (b) Compara, por CLASE, la interferencia envolvente vs la exacta sobre
 * los intervalos reales.  O(N^2) por clase (diagnostico; el grafo eficiente es
    ...
    for (size_t i = 0; i < iv.size(); ++i) {
        ...
        for (size_t j = i + 1; j < iv.size(); ++j) {
            ...
            if (exact)
                ...
                ++m.false_interfere; // envolvente SI, exacto NO.
        }
    }
    return m;
}
 ...
 *        derrama por un par sino por PRESION -- el maximo de intervalos vivos a
 * la vez.  @c peak_env usa el envolvente [front, back); @c peak_exact usa los
 *        segmentos reales.
 *
 ...
 *     (minimo teorico; NINGUNA politica de victima los evita -- son
 * estructurales).
 *   - @c avoidable = overflow(env) - overflow(exact) = spills que el ENVOLVENTE
 *     INVENTA; cota superior de lo que se gana cerrando el envolvente
 * (segmentos). Nota: el pico es una propiedad de los INTERVALOS (liveness), no
 * de la asignacion -- no cambia al mejorar la eleccion de victima (Belady); lo
 * que esa mejora reduce es el numero de spills REALES hacia @c overflow(exact).
 */
struct PressureMeasure {
    uint32_t peak_env_gp = 0,
             peak_exact_gp = 0; ///< pico GP (envolvente / exacto).
    uint32_t peak_env_fp = 0,
             peak_exact_fp = 0; ///< pico FP (envolvente / exacto).
    ...
    /// Spills que el ENVOLVENTE inventa en GP (cota sup. de evitables
    /// cerrandolo).
...
/** @brief Pico de un sweep de eventos (pos, +-1).  Orden: en el mismo @c pos
 * las SALIDAS (-1) antes que las ENTRADAS (+1) -> dos intervalos que se tocan
 *         en un punto ([a,b)[b,c)) NO cuentan como presion simultanea
 * (semi-abierto). */
inline uint32_t
peak_from_events(std::vector<std::pair<uint32_t, int>> &ev) noexcept {
 ...
 *        eventos.  O(N log N) por clase (diagnostico).  Solo intervalos no
 * vacios.
 */
inline PressureMeasure measure_pressure(const jit::IntervalResult &ivs) {
    ...
    for (const jit::LiveInterval &iv : ivs.intervals) {
        ...
        env.emplace_back(iv.ranges.front().from,
                         +1); // envolvente [front, back).
