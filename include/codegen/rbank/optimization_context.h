 ...
 * mejor); el presupuesto son TOPES DUROS (compilacion/memoria/energia) que no
 * se pueden exceder (mas cerca de un Constraint).  Dos objetivos identicos con
...
/**
 * @struct SpillTrace
 * @brief Contador (opcional, gated por el instrumento) de POR QUE se eligio
 * cada victima de spill.  Responde la pregunta clave de Belady: ¿esta el
 *        allocator matando valores YA MUERTOS (next-use trivial) o eligiendo
 * por USO MAS LEJANO?  Si casi todas las victimas son "dead", Belady no aporta
 *        mas que la heuristica de duracion; si hay muchas "alive", Belady
 * trabaja.
 */
struct SpillTrace {
    uint64_t victims_dead =
        0; ///< victima SIN proximo uso (muerta -> Belady trivial).
    uint64_t victims_alive =
        0; ///< victima con next-use finito (Belady real: uso lejano).
    uint64_t spills_total = 0; ///< total de valores derramados.
    // Taxonomia: POR QUE existe cada spill (cierra el modelo explicativo).  La
    // rellena rbank_allocate con classify_spills sobre la asignacion
    // PRE-recovery.
    uint64_t tax_fully = 0; ///< lane libre en TODO su intervalo -> Recovery.
    uint64_t tax_partially =
        0; ///< lane libre en algun subintervalo -> Splitting.
    uint64_t tax_structural =
        0; ///< nunca hay lane libre -> inevitable (overflow).
    uint64_t tax_splitting_potential =
        0; ///< techo del splitting (area libre de los partially).
    // Recuperacion real de tax_fully.  KPI: Fully(limite superior) /
    // Recovered(greedy) / Potential = Fully - Recovered (fully que el greedy no
    // llego a recuperar).
    uint64_t rec_greedy =
        0; ///< spills recuperados por recover_spills (greedy).
    // Recuperacion real del SPLITTING (Fragmentation Recovery) sobre los
    // partially.  El KPI se lee contra tax_splitting_potential: Potential ->
    // Recovered -> Remaining.
    ...
    uint64_t split_rej_shape =
        0; ///< huecos descartados por las condiciones de forma.
    // Perfil de la decision (3c.5): sumas para derivar MEDIAS aceptado vs
    // rechazado -> permite justificar un cambio de parametro con datos, no
    // moviendo constantes.
    int64_t split_acc_gain =
        0; ///< suma de ganancia neta de los tramos aceptados.
    uint64_t split_rej_area = 0; ///< area de los rechazados por coste.
    uint64_t split_rej_uses = 0; ///< usos de los rechazados por coste.
    int64_t split_rej_gain =
        0; ///< su ganancia neta (<=0): por cuanto no llegaron.
 ...
 * allocator llama @c ctx.next_use_distance() / @c ctx.spill_terms_for() sin
 * saber de donde sale el dato -> se puede cambiar por completo el backend de
 * Facts sin tocar ninguna politica.  Esto es lo que lo mantiene respirando a
 * largo plazo.
 *
 * EVOLUCION (cuando crezca): separar CONFIGURACION (bank/constraints/objective/
 * budget) de CONOCIMIENTO (next_use/hw_cost/profile/loop/region/history).  Hoy
 * el conocimiento son 1-2 punteros sueltos; en cuanto lleguen
 * profile/region/history hay que agruparlos tras un @c ctx.knowledge.*() para
 * que esto no derive en un God Object.  El limite es CONCEPTUAL: config = "que
 * problema resolver", knowledge = "que se de del programa/hardware".
 *
...
struct OptimizationContext {
    ...
    /// Knowledge: el allocator pide @c ctx.next_use_distance(), no sabe su
    /// origen.
    const jit::MachineNextUseFacts *next_use = nullptr;
    /// Trace opcional de la razon de cada victima de spill (instrumento;
    /// nullptr = sin conteo).  El allocator lo rellena si esta presente.
    ...
    //   * Facts        : liveness / loop-info / points-to / escape (capa
    //   analysis).
    ...
    /// DEUDA (consistencia): esto reintroduce un SENTINEL (UINT32_MAX), justo
    /// lo que @c codegen::LinearPos::invalid() vino a eliminar -- "el infinito
    /// no es una distancia".  Migrar a un tipo que exprese ausencia
    /// (optional<uint32_t> o un
    /// @c DistanceToNextUse con estado no-valido) cuando se toque la interfaz;
    /// el caller ya distingue el caso muerto con @c has_next_use() + el
    /// horizonte.
    uint32_t next_use_distance(uint32_t vreg,
                               codegen::LinearPos now) const noexcept {
        return next_use ? next_use->distance_to_next_use(vreg, now)
                        : UINT32_MAX;
    }
    const PhysicalRegisterBank &get_bank() const noexcept { return *bank; }
    const ConstraintSet &get_constraints() const noexcept {
        return *constraints;
    }
     ...
     *        del PROGRAMA).
     *
     * Fase 0: devuelve el fallback estatico @c
     * static_execution_weight(loop_depth). Fase 0.25: cuando el Profile (.vprof
     * / g_profile) este cableado, devolvera la frecuencia MEDIDA del bloque de
     * la definicion; asi el modelo no queda atado a la heuristica @c 10^depth.
     */
     ...
     *        reload/store (Facts del HARDWARE via @c hw_cost).  Este metodo ES
     * el punto de convergencia del modelo:
     *
 ...
 *            defecto el fallback generico (from_hw=false); el consumidor pasa
 * la card real de @c spill_card_from(probe_machine_cost_facts(cost_model)).
 */
inline OptimizationContext
make_context(const PhysicalRegisterBank &bank, const ConstraintSet &constraints,
             ObjectiveWeights objective = {}, Budget budget = {},
             bool vec_reduction_active = false, SpillCostCard hw = {}) {
