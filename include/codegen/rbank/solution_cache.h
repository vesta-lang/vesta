 ...
 * @brief Fase 1 (optimizacion): cache de coloreados indexada por la IDENTIDAD
 * del problema (canonical_problem.h) x el banco.
 *
 ...
 *                     |                                       |
 *                   miss                              traducir canon -> ids
 * reales
 *                     |
 *               colorear + guardar
 *
 * CACHE KEY (P6/P9): la solucion depende del problema Y del BANCO
 * (capabilities)
 * -> key = @c canonical_hash x @c bank_fingerprint.
 *
 * DIRECCION (no ahora): cuando exista mas de UN allocator (linear-scan vs PBQP
 * vs graph-coloring), la misma forma canonica puede dar soluciones DISTINTAS
 * segun la estrategia.  La identidad de la SOLUCION sera entonces
 * @c ProblemShape x @c Target x @c Policy: la key ganara un @c policy_id.  Hoy
 * hay una sola policy (color_linear_scan), asi que no se anade (regla: sin
 * infra por si acaso); entra con el solver de Fase 5.  El Objective entra por
 * el mismo sitio cuando el spill inteligente lo consuma.
 *
 ...
 *        la solucion en ids CANONICOS; @c solve la traduce a los ids del
 * problema. La huella del PROBLEMA la da @c canonicalize; la del BANCO,
 *        @c bank.fingerprint() (la identidad es una pregunta del banco, no de
 * la cache).  La cache solo las COMBINA en su key interna.
 */
struct SolutionCache {
    std::unordered_map<uint64_t, LaneAssignment>
        by_key; ///< key -> sol canonica.
    uint64_t hits = 0;
    uint64_t misses = 0;
     ...
     */
    LaneAssignment solve(const AbstractProblem &p,
                         const PhysicalRegisterBank &bank, bool vec_active) {
        const CanonicalProblem cp = canonicalize(p);
        // Combinar las DOS huellas fuertes (problema x banco) en la key
        // interna.
        const uint64_t key =
