 ...
 *
 * NO DECIDE NADA -- solo materializa (la decision es de quien produjo el plan; por eso el
 * plan trae la ubicacion DESTINO de cada tramo).  Y NO es "el builder del splitting": es
 * el MATERIALIZADOR del modelo temporal.  Hoy consume un @c AssignmentPlan (de la Fragmentation
 * Recovery); el dia que existan un @c RematerializationPlan o un @c CoalescingPlan deberia
 * materializarlos igual, sin cambiar de naturaleza.  Esa es la tentacion peligrosa a
 * vigilar: que el builder empiece a DECIDIR que partir.  No.  Sigue siendo un constructor.
 *
 * Es tambien el UNICO punto donde se traduce la representacion del allocator
 * (@c RegAlloc::VAssign) a la abstraccion del timeline (@c ValueLocation): una vez, al
 * construir -- nunca en cada consulta ni en el consumidor.
 */
...
/// Traduce la representacion del ALLOCATOR (@c RegAlloc::VAssign) a la abstraccion del
/// timeline.  Unico sitio donde se mira el enum interno de la asignacion.
...
/**
 * @brief Aplica UNA afirmacion del plan sobre los segmentos de un vreg: parte los que
 *        solapan el tramo y pone dentro la ubicacion que el intervalo afirma.  Los
 *        segmentos quedan ordenados y disjuntos (el corte preserva el orden).
 *
 * No sabe -- ni le importa -- si el intervalo nacio de la Recovery, del Splitting, de una
 * rematerializacion o de un coalescing: solo transforma una representacion temporal.  Por
 * eso no se llama "apply_split".
 *
 *     base:  [----------- MEM -----------)
 *     split:        [-- REG --)
 *     out:   [ MEM )[-- REG --)[-- MEM --)
 */
inline void apply_interval(std::vector<LocationSegment> &segs, const AssignmentInterval &si) {
 ...
 *        (los rangos vivos por vreg, opcional) + @p plan (modificaciones temporales).
 * @return El modelo temporal, indexable por vreg denso.
 *
 * Sin @p ivs (caller que no construye intervals) cada vreg vivo recibe un unico segmento
 * que cubre toda la funcion: equivalente a la asignacion plana para un consumidor que
 * pregunta por posiciones reales.  Con @p plan vacio el resultado es exactamente la
 * @c RegAlloc (1 segmento por rango): el pipeline SIEMPRE existe, sin casos especiales.
 */
inline AllocationTimeline build_allocation_timeline(const RegAlloc &ra,
    ...
    if (ivs) {
        ...
        for (uint32_t vreg = 0; vreg < n; ++vreg) {
            ...
            const ValueLocation loc =
                (vreg < ra.assign.size()) ? to_resolved(ra.assign[vreg])
                                          : ValueLocation{};
        ...
        // Sin rangos: un unico segmento que cubre TODA la funcion [0, MAX) por cada vreg
        // con ubicacion real.  El fin usa el maximo (no @c invalid(): aqui MAX es un
        // extremo real "hasta el final", no un estado).
    ...
    /* (2) Aplicar el PLAN: cada tramo parte los segmentos de su vreg.  El builder no
     *     decide nada -- solo coloca la ubicacion que el plan pidio. */
    for (const AssignmentInterval &si : plan.intervals) {
        if (si.vreg >= tl.values.size()) continue; // vreg fuera de rango: ignorar.
 ...
 *        @c AllocationTimeline.  Es el unico artefacto que el Rewrite ve; cada mitad
