 ...
 *
 * POR QUE: hoy una ubicacion es REG o STACK-slot.  Cuando crezca (rematerializable,
 * constante, home location, spill-cache, TLS, frame temporal...) esos casos se anyaden como
 * predicados AQUI y ningun consumidor cambia -- el Rewrite ya preguntaba "¿es registro?
 * ¿es memoria?", no "¿que campo tiene la struct?".  Mantiene al Rewrite ignorante de COMO
 * se representa una ubicacion, igual que @c AllocationTimeline lo mantiene ignorante de la
 * representacion temporal (segments).
 *
 * ENCAPSULACION: el enum y los campos son PRIVADOS -- nadie puede @c loc.kind = ... y
 * romper el invariante.  Se construye con tags (@c Register / @c Stack); ese es el UNICO
 * modo de crear una ubicacion valida.  @c is_stack no dice "spill" a proposito: la memoria
 * podria ser home / remat / frame temporal.  (Evolucion futura probable: renombrar a
...
class ValueLocation {
  public:
    ...
    constexpr bool is_register() const noexcept { return kind_ == Kind::Register; }
    constexpr uint8_t register_id() const noexcept { return reg_; }

    /// ¿Vive en MEMORIA? (hoy = stack; manana tambien home / TLS / ...).  El consumidor
    /// que solo quiere saber "¿es memoria?" usa esto y NO enumera cada tipo de memoria.
    constexpr bool is_memory() const noexcept { return kind_ == Kind::Stack; }
    /// Sub-caso de memoria: stack.  NO "is_spill": la memoria podria ser home/remat/temp.
