...
/**
 * @file native_ffi.h
 * @brief Interfaz de funciones externas (FFI) para llamadas a codigo nativo
 * desde VestaVM.
 *
 * Proporciona la infraestructura necesaria para que el bytecode de VestaVM
 * invoque funciones nativas del sistema operativo (DLL en Windows, .so en
 * Linux) mediante la instruccion @c CALLN.
 *
 * Modelo de funcionamiento:
 *  1. Durante el ensamblado, las directivas @c @Import registran cada simbolo
 * nativo (modulo + funcion + firma) como una entrada en la tabla de
 * importaciones.
 *  2. En tiempo de carga (@c Loader), @c FFI::resolve_all() resuelve las
 * direcciones reales usando @c LoadLibrary/GetProcAddress (Windows) o @c
 * dlopen/dlsym (POSIX).
 *  3. Las direcciones resueltas se escriben directamente en el bytecode
 * (patch-sites) para que la VM pueda invocarlas con un simple call indirecto.
 *
 * Tabla de alias de puntero de funcion (@c CallFn y @c Fn00..Fn12):
 *  - Cada tipo representa una firma con 0..12 argumentos @c uint64_t mas un
 * puntero de contexto @c void* como primer parametro oculto.
...
/**
 * @brief Tipo de puntero de funcion generica para llamadas FFI con hasta 12
 * argumentos.
 *
 ...
 */
using CallFn = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
...
using Fn05 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t);
/** @brief Funcion nativa con 6 argumentos (mas contexto). */
using Fn06 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t);
/** @brief Funcion nativa con 7 argumentos (mas contexto). */
using Fn07 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t);
/** @brief Funcion nativa con 8 argumentos (mas contexto). */
using Fn08 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t, uint64_t);
/** @brief Funcion nativa con 9 argumentos (mas contexto). */
using Fn09 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);
/** @brief Funcion nativa con 10 argumentos (mas contexto). */
using Fn10 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t);
/** @brief Funcion nativa con 11 argumentos (mas contexto). */
using Fn11 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t);
/** @brief Funcion nativa con 12 argumentos (mas contexto). */
using Fn12 = uint64_t (*)(void *, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t, uint64_t, uint64_t,
                          uint64_t, uint64_t, uint64_t);

/**
 * @class FFIError
 * @brief Excepcion lanzada cuando el sistema FFI encuentra un error
 * irrecuperable.
 *
...
class FFIError : public std::runtime_error {
  public:
     ...
     */
    explicit FFIError(const std::string &msg) : std::runtime_error(msg) {}
};
 ...
 *
 * @param bytecode_base Puntero al inicio del buffer de bytecode cargado en
 * memoria.
 * @param off           Offset en bytes desde @p bytecode_base donde escribir la
 * direccion.
 * @param fn            Puntero a la funcion nativa resuelta (obtenido con dlsym
 * / GetProcAddress).
 */
static inline void patch_call(uint8_t *bytecode_base, uint32_t off, void *fn) {
    uint64_t addr = reinterpret_cast<uint64_t>(
        fn); /* convierte puntero a entero de 64 bits */
...
struct NativeImportKey {
    ...
    uint32_t function_idx =
        0; ///< Indice al nombre de la funcion en FFI::functions.
    uint32_t signature_idx =
        0; ///< Indice a la firma del metodo en FFI::signatures.
 ...
 *  - El puntero resuelto (@c resolved_ptr) tras llamar a
 * LoadLibrary+GetProcAddress o dlopen+dlsym.
 *  - Todos los @c patch_sites: offsets de bytecode donde se debe escribir @c
 * resolved_ptr para que la instruccion @c CALLN pueda invocar la funcion
 * directamente.
 *
 * El agrupamiento de patch-sites permite resolver cada simbolo nativo una sola
 * vez aunque aparezca en multiples instrucciones @c CALLN del bytecode.
 */
struct NativeImportRuntime {
    NativeImportKey key; ///< Clave que identifica el simbolo.
    void *resolved_ptr =
        nullptr; ///< Direccion nativa resuelta en tiempo de carga.
    std::vector<uint32_t>
        patch_sites; ///< Offsets de bytecode donde parchear el CALL.
};
 ...
 *
 * Combina los tres indices de la clave (module_idx, function_idx,
 * signature_idx) mediante desplazamientos XOR para obtener un valor de 64 bits
 * con baja tasa de colision, apto para @c std::unordered_map.
 */
struct NativeImportKeyHash {
    /**
...
class FFI {
  public:
     ...
     *
     * Cada entrada asocia una @c NativeImportKey (unica por triple
     * modulo/funcion/firma) con su @c NativeImportRuntime, que contiene el
     * puntero resuelto y los patch-sites correspondientes.
     */
    std::unordered_map<NativeImportKey, NativeImportRuntime,
                       NativeImportKeyHash>
        imports;
     ...
     * Si no, lo agrega al vector @c modules y registra el nuevo indice en @c
     * module_map.
     *
    ...
    /**
     * @brief Interna un nombre de funcion nativa y devuelve su indice en @c
     * functions.
     *
     * Semantica identica a @c intern_module pero aplicada a la tabla @c
     * functions.
     *
    ...
    /**
     * @brief Interna una firma de funcion y devuelve su indice en @c
     * signatures.
     *
     ...
     * RTLD_LAZY | RTLD_LOCAL.  El resultado se almacena en @c native_modules
     * para evitar cargas duplicadas en llamadas posteriores.
     *
     * @param name Nombre del modulo a cargar (p.ej. "kernel32.dll",
     * "libm.so.6").
     ...
     *
     * @param module Handle opaco del modulo cargado (obtenido con @c
     * load_native_module()).
     ...
     *
     * Recorre @c imports, llama a @c load_native_module() y @c
     * resolve_native_symbol() para cada simbolo, y luego itera sobre @c
     * patch_sites llamando a @c patch_call() por cada offset.  Tras esta
     * llamada el bytecode esta listo para ejecutarse.
     *
     * @param file_base             Puntero al inicio del buffer de bytecode en
     * memoria.
     * @param offset_real_bytecode  Offset desde @p file_base donde comienza el
     * codigo ejecutable (para calcular la posicion absoluta de cada
     * patch-site).
     * @throws FFIError si un modulo no puede cargarse o una funcion no se
     * encuentra.
     */
     ...
     *
     * Recorre @c native_modules; si un modulo exporta el simbolo @c
     * "vesta_init" y aun no fue inicializado (no esta en @c
     * initialized_plugins), lo llama pasando @p api.  Registra el modulo en @c
     * initialized_plugins para evitar llamadas duplicadas en cargas sucesivas.
     *
     * Debe invocarse despues de @c resolve_all, cuando todos los modulos
     * nativos del ejecutable ya estan cargados.
     *
     * @param api Puntero a la estructura @c VestaPluginAPI con los callbacks
     * del manager.
     */
