 ...
 *
 * ByteWriter es el complemento de ByteReader: acumula bytes en un vector de
 * salida a medida que el emisor genera codigo o datos, manteniendo el offset
 * actualizado.
 *
...
typedef struct ByteWriter {
    ...
    /**
     * @brief Numero de bytes emitidos hasta el momento (offset actual de
     * escritura).
     */
    uint64_t offset = 0;
    ...
    /**
     * @brief Emite un valor de 16 bits en little-endian y avanza el offset
     * en 2.
     *
    ...
    /**
     * @brief Emite un valor de 32 bits en little-endian y avanza el offset
     * en 4.
     *
    ...
    /**
     * @brief Emite un valor de 64 bits en little-endian y avanza el offset
     * en 8.
     *
     ...
     *
     * Copia los bytes del bloque al final del buffer de salida y avanza el
     * offset.
     *
     ...
     *
     * Util para emitir campos de 40 bits del formato .velb sin emitir los 3
     * bytes altos.
     *
     ...
     *
     * Permite rellenar huecos de 8 bytes que se dejaron a cero durante el
     * primer paso de emision y cuya direccion final se conoce en un paso
     * posterior.
     *
     * @param value Nuevo valor de 64 bits a escribir en la posicion @p pos.
     * @param pos   Offset absoluto dentro de @p output donde escribir los 8
     * bytes.
     ...
     * al opcode variante para registros especiales (p.ej. 0x09 -> 0x2E para
     * subsp).
     *
    ...
    /**
     * @brief Patch in-place de un uint32 little-endian en una posicion
     * conocida. Usado para emitir el size de la tabla de relocations en el
     * header tras conocer cuantas entries hay (write_executable patron
     * pre-calculado).
     */
