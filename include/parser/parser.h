 ...
 *  - @c remove_underscores() : limpia separadores de digitos ('_') de
 * literales.
 *  - @c is_valid_number()    : valida literales numericos (hex, bin, oct, dec).
 *  - @c parse_number()       : convierte un lexema numerico a @c uint64_t.
 *  - @c OpArity              : enum que describe el numero de operandos de una
 * instruccion.
 *  - @c InstrSet             : tabla estatica de instrucciones reconocidas por
 * el parser.
 *  - @c Parser               : clase principal del parser con todos los metodos
 * de produccion.
 */
...
/**
 * @brief Comprueba si la cadena s es un numero valido en los formatos
 * soportados.
 *
 * Soporta:
 *  - Hexadecimal: 0x... o 0X...
...
static bool is_valid_number(const std::string &s_in) {
    ...
    if (s.size() - pos >= 2 && s[pos] == '0' &&
        (s[pos + 1] == 'x' || s[pos + 1] == 'X')) {
        // hexadecimal: debe tener al menos un digito hexadecimal despues del
        // prefijo.
 ...
 * auto v6 = parse_number_safe("-10");         // std::nullopt (negativo no
 * permitido) auto ok = is_valid_number("0x1G");          // false (G no es hex)
 * @endcode
 *
...
static std::optional<uint64_t> parse_number_safe(const std::string &s_in) {
    ...
    if (s.size() - pos >= 2 && s[pos] == '0' &&
        ...
        uint64_t val = 0;
        auto [ptr, ec] =
            std::from_chars(body.data(), body.data() + body.size(), val, 16);
        if (ec == std::errc() && ptr == body.data() + body.size()) return val;
        return std::nullopt;
    }

    // binario (sin from_chars base 2), analizar manualmente con comprobacion de
    // desbordamiento
    if (s.size() - pos >= 2 && s[pos] == '0' &&
        (s[pos + 1] == 'b' || s[pos + 1] == 'B')) {
        std::string body = s.substr(pos + 2);
        ...
        for (char c : body) {
            ...
            int bit = c - '0';
            if (val > (std::numeric_limits<uint64_t>::max() >> 1))
                return std::nullopt; // overflow on shift
            val = (val << 1) | bit;
        }
        return val;
    }
    ...
    if (s[pos] == '0' && (s.size() - pos) > 1) {
        ...
        for (char c : body) {
            ...
            int d = c - '0';
            if (val > (std::numeric_limits<uint64_t>::max() >> 3))
                return std::nullopt;
            val = (val << 3) + d;
        }
        return val;
...
static uint64_t parse_number(const std::string &s) {
    ...
    // Octal
    if (s.size() > 1 && s[0] == '0') return std::stoull(s, nullptr, 8);
...
class ParseWarning : public std::runtime_error {
  public:
    ...
    ParseWarning(int l, int c, const std::string &msg,
 ...
 * ```
 * program     ::= section*
 * section     ::= "code:" statement* "end_code:" | "data:" data_decl*
 * "end_data:" statement   ::= IDENTIFIER REGISTER COMMA NUMBER | ... data_decl
...
class Parser {
  private:
    ...
    vm::Token current{vm::TokenType::EndOfFile, "", 0,
                      0}; ///< Token actual (lookahead)
     ...
     */
    [[nodiscard]] Token peek() const { return lexer.peek_token(); }
  ...
  public:
    ...
    /**
     *
     * @param type tipo de token esperado, si es el token esperado, avanza,
     * sino, muestra error sin avanzar.
