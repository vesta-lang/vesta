...
struct FieldDecl {
    ...
    ClassInfo *type_class =
        nullptr; ///< usado solo si kind == FIELD_CLASS / FIELD_STRUCT
    uint32_t size_bytes =
        8; ///< tamano del slot (8 estandar; aceptamos 1/2/4/8)
...
class ClassRegistry {
  public:
     ...
     * @param super       ClassInfo de la superclase (NULL = Object por
     * defecto).
     ...
     */
    ClassInfo *define_class(const std::string &name, ClassInfo *super,
     ...
     */
    bool add_advice(MethodInfo *target, uint8_t kind,
     ...
     */
    static FieldInfo *find_field(ClassInfo *cls, const char *name,
                                 size_t name_len);
     ...
     */
    static MethodInfo *find_method(ClassInfo *cls, const char *name,
                                   size_t name_len);
     ...
     */
    ItableEntry *get_or_build_itable(ClassInfo *cls, ClassInfo *iface,
                                     uint32_t count);
