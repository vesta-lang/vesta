...
struct GlobalPID {
    ...
    uint64_t local_pid;    ///< PID local generado por ese scheduler (monotonicamente creciente)

    /**
     * @brief Operador de igualdad: dos PIDs son iguales si ambos campos coinciden.
...
namespace std {
    ...
    struct hash<GlobalPID> {
        ...
        size_t operator()(const GlobalPID &p) const noexcept {
            // desplazar scheduler_id a los bits 48..63 y combinar con el hash de local_pid
