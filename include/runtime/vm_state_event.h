...
typedef enum vm_state {
    /**
     * @brief El proceso esta listo para ejecutarse pero no tiene la CPU
     * virtual.
     *
    ...
    /**
     * @brief El proceso esta bloqueado esperando un evento, tiempo o senal
     * externa.
     *
     * Condiciones de entrada:
     ...
     *
     * El proceso no sera ejecutado hasta que sea desbloqueado por el evento
     * esperado.
     */
    BLOCKED,

    /**
     * @brief El proceso ha terminado su ejecucion de forma permanente por error
     * fatal.
     *
     * Condiciones de entrada:
     *   - La funcion principal del proceso retorno.
     *   - El proceso encontro un error irrecuperable (SEGFAULT, instruccion
     * ilegal, etc.).
    ...
    /**
     * @brief El scheduler esta descodificando la instruccion actual del
     * proceso.
     *
     * Condiciones de entrada:
     ...
     *
     * Condiciones de entrada:
     *   - La instruccion en EXECUTE detecto que necesita esperar un dispositivo
     * externo.
     *
    ...
    /**
     * @brief El proceso detendio su ejecucion por una instruccion HALT o parada
     * explicita.
     *
     * Condiciones de entrada:
...
enum vm_event {
     ...
     *
     * Generado por el planificador cuando decide asignar la CPU virtual al
     * proceso. Transicion habitual: READY -> RUNNING.
     */
    EVT_SCHEDULED,
     ...
     *
     * Se dispara cuando se detecta un error irrecuperable (acceso de memoria
     * invalido, instruccion desconocida, division por cero, etc.). Transicion
     * habitual: cualquier estado -> DEAD.
     */
    EVT_ERROR,
 ...
 * @return  Cadena con el nombre del evento, o "UNKNOWN_EVENT" si no se
 * reconoce.
 */
static const char *event_name(vm_event e) {
    switch (e) {
    case EVT_DECODE_DONE:
        return "EVT_DECODE_DONE";               // descodificacion completada
...
typedef struct Transition {
    ...
    void (*action)(ProcessVM *); ///< Accion a ejecutar durante la transicion
                                 ///< (puede ser nullptr)
