 ...
 *
 * Define:
 *   - ManageVM: gestor que posee multiples instancias VM, un ArenaManager
 * publico, un Loader publico y un listener TCP opcional.
 *   - ManagerTLSConnection: conexion TLS gestionada por el manager (envia
 * saludo y cierra).
 *   - ManagerTCPListener: servidor TCP que crea ManagerTLSConnection al aceptar
 * clientes.
...
/**
 * @brief Genera un resumen textual de una instancia VM para impresion en
 * tablas.
 *
 ...
 */
static void
print_vm_list_table(std::ostream &out,
 ...
 * la conexion TLS de forma ordenada.  Derivar para implementar protocolos
 * reales.
 */
class ManagerTLSConnection : public TLSConnection {
  public:
    /**
     * @brief Construye una conexion TLS con el descriptor de socket y el
     * contexto SSL.
     *
    ...
    /**
     * @brief Inicia la sesion TLS: envia un mensaje de saludo y cierra la
     * conexion.
     *
     * Escribe el texto "Hola Mundo desde TLS!\n" como bytes y llama a
     * SSL_shutdown() hasta que se complete el handshake de cierre (ret != 0).
     */
    void start() override {
...
/**
 * @brief Servidor TCP que acepta conexiones y crea instancias
...
class ManagerTCPListener : public TCPServer {
  public:
    /**
     * @brief Construye el listener en el puerto indicado con el contexto TLS
     * dado.
     *
     * @param port Puerto TCP en el que el servidor escucha conexiones
     * entrantes.
  ...
  protected:
     ...
     * @return          Nueva ManagerTLSConnection, o nullptr si falla el
     * handshake.
     */
...
class ManageVM {
  public:
    ...
    /**
     * @brief ArenaManager publico compartido entre todas las instancias VM del
     * gestor.
     *
     ...
     *
     * @param num_schedulers Numero de hilos nativos que puede usar la
     * instancia.
     ...
     *
     * @return Cadena con nombre, contador, numero de VMs, listener, loader y
     * tabla de VMs.
     */
  ...
  private:
    mutable std::mutex
        vm_mutex; ///< Protege el acceso concurrente al vector vms

    /**
     * @brief Contador monotonamente creciente para asignar IDs unicos a las
     * VMs.
     *
