 ...
 * @brief Macros de despacho basado en computed-goto para el pipeline de ejecucion.
 *
 ...
 *
 * @warning Este archivo usa la extension de GCC/Clang de computed-goto (&&label).
 *          No es estandar C++17 y genera una advertencia -Wpedantic que se
...
/**
 * @brief Salta a la etiqueta correspondiente al opcode actualmente descodificado.
 *
...
/**
 * @brief Avanza el PC si no hubo salto, descodifica la siguiente instruccion y despacha.
 *
 * Secuencia: incrementar RIP (si !did_jump) -> decode_instruction() -> DISPATCH().
 * Usado al final de cada etiqueta de opcode para continuar el bucle de ejecucion
 * sin overhead de llamada a funcion.
 */
#define NEXT() \
...
// Suprimir la advertencia -Wpedantic generada por el uso de computed-goto (&&label)
 ...
 * Actualmente comentada; pendiente de integracion con el nuevo modelo de scheduler.
 *
