 ...
 *
 * Proporciona:
 *   - ansi::init()        : habilita el modo VT en Windows y detecta si stdout es un TTY.
 *   - ansi::is_enabled()  : true si los codigos de color se pueden emitir.
 *   - ansi::c(code)       : devuelve el codigo si los colores estan activos, "" si no.
 *   - ansi::byte_color(b) : color deterministico para un valor de byte (mismo valor = mismo color).
...
namespace ansi {
    ...
    inline bool init() {
        ...
        if (hOut == INVALID_HANDLE_VALUE || hOut == nullptr) return (enabled = false);
        DWORD dwMode = 0;
        if (!GetConsoleMode(hOut, &dwMode))   return (enabled = false);
        enabled = (SetConsoleMode(hOut, dwMode | ENABLE_VIRTUAL_TERMINAL_PROCESSING) != 0);
#else
        // POSIX: solo usar color si stdout va a un terminal real (no a fichero o pipe)
     ...
     */
    inline bool is_enabled() { return init(); }
    ...
    constexpr const char *ITALIC    = "\x1B[3m";   ///< Cursiva (soporte variable por terminal)
     ...
     *   - Aritmetica/logica (add, sub, mul, div, and, or, xor, not, shl, shr, sar, cmp, inc, dec): verde
     ...
     *
     * @param mnemonic Nombre mnemotecnico de la instruccion (puede estar en minusculas).
     * @return Codigo ANSI de color o "" si los colores no estan activos.
     */
    inline const char *mnem_color(const char *mnemonic) {
        ...
        auto starts = [&](const char *prefix) {
            int i = 0;
            while (prefix[i] && mnemonic[i] && mnemonic[i] == prefix[i]) i++;
            return prefix[i] == '\0';
        };
        // control de flujo
        if (starts("hlt"))                     return "\x1B[91m"; // rojo brillante
        if (starts("jmp") || starts("jcc"))    return "\x1B[91m";
        if (starts("callvm") || starts("jmpr") || starts("callvmr")) return "\x1B[91m";
        // movimiento de datos
        if (starts("mov") || starts("xchg") || starts("push") || starts("pop"))  return "\x1B[93m";
        if (starts("movc"))                    return "\x1B[93m";
        // aritmetica y logica
        if (starts("add")  || starts("sub")  || starts("mul")  || starts("div"))  return "\x1B[92m";
        if (starts("and")  || starts("or")   || starts("xor")  || starts("not"))  return "\x1B[92m";
        if (starts("shl")  || starts("shr")  || starts("sar"))                    return "\x1B[92m";
        if (starts("cmp")  || starts("inc")  || starts("dec"))                    return "\x1B[92m";
        // memoria / GC
        if (starts("alloc") || starts("free") || starts("gc"))     return "\x1B[96m"; // cian brillante
        // VM-especificas: calln, writecur, readcur, vminfo, etc.
        if (starts("calln") || starts("writecur") || starts("readcur"))  return "\x1B[94m"; // azul brillante
