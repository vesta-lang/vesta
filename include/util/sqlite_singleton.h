 ...
 * @brief Singleton thread-safe para gestion de la base de datos SQLite de
 * VestaVM.
 *
 ...
 *  - Consultas simples (execute) y consultas con resultado en JSON
 * (execute_json).
 ...
 *       "CREATE TABLE IF NOT EXISTS bytecode(id INTEGER PRIMARY KEY, code
 * BLOB)");
 *
...
class SqliteSingleton {
  private:
    ...
    static std::unique_ptr<sqlite3, SqliteDeleter>
        db_instance; ///< Instancia unica de la BD.
    static std::mutex
        db_mutex; ///< Mutex que serializa el acceso a la instancia.
    static bool
        initialized; ///< true una vez que la BD ha sido abierta con exito.
  ...
  public:
    ...
    /**
     * @brief Devuelve el puntero a la instancia unica de SQLite (inicializacion
     * lazy).
     *
     * La primera llamada abre la base de datos en @p db_path.  Las siguientes
     * llamadas devuelven directamente el puntero ya almacenado sin volver a
     * abrir.
     *
     * Thread-safe: solo un hilo inicializa la BD; los demas esperan en el mutex
     * y reutilizan la instancia ya creada.
     *
     ...
     * @param err Puntero opcional donde se almacena el mensaje de error si
     * ocurre; si es @c nullptr los errores se imprimen en @c stderr.
     * @return @c true si la ejecucion fue correcta; @c false si hubo error.
     *
     * @code
     *   SqliteSingleton::execute(
     *       "CREATE TABLE IF NOT EXISTS lexer_tokens (token TEXT, line
     * INTEGER)");
     * @endcode
     */
     ...
     * de columna y cuyos valores son cadenas (o la representacion textual del
     * tipo).
     *
     ...
     *
     * @see query_json_rows_map() para convertir la salida en un vector de mapas
     * C++.
     */
    static nlohmann::json execute_json(const std::string &sql);

    /**
     * @brief Ejecuta una consulta SQL y devuelve las filas como vector de
     * mapas.
     *
     * Convierte la salida de @c execute_json() en una estructura C++ mas
     * comoda: cada fila es un @c std::map<string,string> donde la clave es el
     * nombre de columna y el valor es la representacion textual del dato.
     *
     ...
     * @return Vector de filas; vacio si hubo error o la consulta no devolvio
     * datos.
     *
     * @note No lanza excepciones por columnas ausentes; verifica la existencia
     * de claves antes de usar @c at() si el esquema puede variar.
     *
     * @code
     *   std::string err;
 ...
 * Las columnas se ajustan al ancho maximo de sus valores (truncando a @p
 * max_width). Los valores que parecen numericos se alinean a la derecha si @p
 * right_align_numbers es @c true.
 *
 * Thread-safe: usa @c print_mutex para serializar escrituras concurrentes.
 *
 * @param out                 Stream de salida (p.ej. @c std::cout o un @c
 * ostringstream).
 * @param rows                Vector de filas; cada fila es un mapa
 * columna->valor.
 * @param columns_order       Orden explicito de columnas a mostrar; si esta
 * vacio se usa la union ordenada de todas las claves presentes.
 * @param right_align_numbers Si @c true, los valores que solo contengan
 * digitos, '.' o '-' se alinean a la derecha.
 * @param max_width           Ancho maximo en caracteres de una celda; los
 * valores mas largos se truncan con "...".
 */
static void
print_table(std::ostream &out,
            const std::vector<std::map<std::string, std::string>> &rows,
            const std::vector<std::string> &columns_order = {},
            bool right_align_numbers = true, size_t max_width = 60) {
    ...
    for (const auto &r : rows) {
        for (size_t i = 0; i < cols.size(); ++i) {
            ...
            if (it != r.end())
                width[i] =
                    std::max(width[i], std::min(max_width, it->second.size()));
        }
    }
    ...
    for (const auto &r : rows) {
        ...
        for (size_t i = 0; i < cols.size(); ++i) {
            ...
            if (val.size() > max_width)
                val = val.substr(0, max_width - 3) + "...";

            /* determinar si el valor parece un numero para alinearlo a la
             * derecha */
            bool is_number =
                !val.empty() && std::all_of(val.begin(), val.end(), [](char c) {
                    return (c >= '0' && c <= '9') || c == '.' || c == '-';
                });
 ...
 * @param columns_order       Orden explicito de columnas; vacio = orden
 * alfabetico.
 * @param right_align_numbers Alinea a la derecha los valores numericos si es @c
 * true.
 ...
 */
inline std::string
table_to_string(const std::vector<std::map<std::string, std::string>> &rows,
                const std::vector<std::string> &columns_order = {},
                bool right_align_numbers = true, size_t max_width = 60) {
