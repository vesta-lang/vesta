...
namespace vex {
     ...
     */
    struct SourceLoc {
        std::string file;       ///< Ruta o nombre logico del fichero (puede ser "<stdin>").
        uint32_t    line   = 1; ///< Numero de linea (1-based).
        uint32_t    column = 1; ///< Numero de columna (1-based, en bytes).
        uint32_t    offset = 0; ///< Offset absoluto en bytes desde el inicio del fichero.
    ...
    inline void print_all_diagnostics(std::ostream &os, const Diagnostics &diags) {
        ...
        for (const auto &d : diags.all()) print_diagnostic(os, d);
    }
    ...
    public:
        ...
        /**
         * @brief Mueve el valor fuera del Result.  Comportamiento indefinido si @c !ok().
         */
