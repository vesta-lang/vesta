...
Compilar (PE para Windows, ELF para Linux):
    ...
    vm --vx examples_codes_vx/aot/01_call.vx -m aot -o 01_call.exe        # PE (default host)
...
que aportan el crt (`_start` -> `main`) + libc + permiten linker scripts (dev OS):
    ...
    vm --vx examples_codes_vx/aot/01_call.vx -m aot --format elf --emit obj -o 01_call.o
...
aparte), asi que el emisor pre-escribe target_off en el sitio.
    ...
    vm --vx examples_codes_vx/aot/01_call.vx -m aot --format pe --emit obj -o 01_call.obj
...
sus funciones (sin main; sin _start).  Cargable con dlopen + dlsym:
    ...
    vm --vx examples_codes_vx/aot/09_shared_lib.vx -m aot --format elf --emit shared -o libfoo.so
...
exports .edata).  Cargable con LoadLibrary + GetProcAddress:
    ...
    vm --vx examples_codes_vx/aot/10_shared_dll.vx -m aot --format pe --emit shared -o foo.dll
...
- `.wstr()`   -> `u16*` UTF-16LE NUL-terminado (Win32 `*W`), con pares suplentes
  ...
  (transitorio para FFI).
    ...
    vm --vx examples_codes_vx/aot/59_string_utf8.vx -m aot --emit exe -o 59.exe
...
- **Multi-.o Vex**: una libreria `.vx` SIN `main` compila a un `.o` que EXPORTA
  sus funciones de usuario como globales (los helpers internos `__vx_*`/`__new_*`
  ...
  referencia con `extern "lib" { fn ...; }` y el linker las resuelve cross-file.
      ...
      vm --vx lib.vx -m aot --emit obj --format elf -o lib.o   # sin main
      vm --vx app.vx -m aot --emit obj --format elf -o app.o   # extern + main
...
lleva sus propios slots (`__vx_strlen_fp`, etc.) y sus inits
(`__vx_cpu_init` -> `__vx_memcpy_init` -> `__vx_strdisp_init`), que dejan
...
`__vx_premain` que los llama en orden (`cpu` -> `memcpy` -> `strdisp`) y salta
...
Con `--entry` (sin `main`/stub) el `__vx_premain` no se sintetiza: el punto de
...
(gcc -m32, ld, link.exe):
    ...
    vm --vx prog.vx -m aot --aot-arch x86-32 --emit obj --format elf -o prog.o
    gcc -m32 prog.o -o prog        # ELF32 i386, el crt llama a main

    vm --vx prog.vx -m aot --aot-arch x86-32 --emit obj --format pe -o prog.obj
...
### Script de enlace ESCRITO EN VESTA (configurable) -- `--link-script`
...
del `IMAGE_TLS_DIRECTORY`, sintetiza `__vx_tls_init` (escribe los valores
...
vm -m aot --vx 64_thread_local_basico.vx --format elf --emit exe -o t
